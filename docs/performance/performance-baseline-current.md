...
- Report-grade public auth flow latency is documented in [report-grade-auth-flow-measurement-2026-07-16.md](./report-grade-auth-flow-measurement-2026-07-16.md). The first 12-account ALB sample had `0` errors and `0` 429/A010 responses across `48` measured requests; login p95 was `151.0ms`, refresh p95 was `130.8ms`, profile read p95 was `75.5ms`, and logout p95 was `57.1ms`.
- Report-grade public recommendation flow latency is documented in [report-grade-recommendation-flow-measurement-2026-07-16.md](./report-grade-recommendation-flow-measurement-2026-07-16.md). The first one-account ALB sample had `0` errors, `0` 429/R004 responses, and `0` R003 conflicts; stored read after data existed was at most `116.4ms`, first shared refresh was `5437.4ms`, cached shared refresh was `209.5ms`, and personal refresh was `3933.2ms`. The refresh paths returned `SCORED` AI-status items, so these refresh numbers are OpenAI-influenced and cost-bearing.
- Report-grade public chatbot flow latency is documented in [report-grade-chat-flow-measurement-2026-07-16.md](./report-grade-chat-flow-measurement-2026-07-16.md). The first one-account ALB sample had `0` errors and `0` rate-limit responses across three representative questions; all three answers were `POLICY_GROUNDED`, message-send p95 was `4113.8ms`, and the slowest message was `4195.5ms`. Chat message latency is OpenAI-influenced and is excluded from ordinary interactive API p95 alerts while remaining visible as `excluded_latency`.
- Report-grade integrated user journey latency is documented in [report-grade-integrated-user-journey-measurement-2026-07-16.md](./report-grade-integrated-user-journey-measurement-2026-07-16.md). The first one-account ALB journey passed all `18` steps with `0` errors and `0` rate-limit responses; summed measured API time was `8935.8ms`, non-AI API time was `980.9ms`, AI-backed time was `7954.9ms`, and the AI-backed recommendation/chat steps accounted for about `89.0%` of measured API time.
- Final report-ready performance tables and interpretation are consolidated in [final-report-performance-summary-2026-07-16.md](./final-report-performance-summary-2026-07-16.md).
- Final functional regression after report-summary work is documented in [final-functional-regression-check-2026-07-16.md](./final-functional-regression-check-2026-07-16.md). The integrated production flow passed all `18` steps with expected values, `0` errors, and `0` rate-limit responses; ALB targets stayed `2` healthy, DB waiting locks stayed `0`, JVM restart count stayed `0`, and the secondary instance was confirmed on revision `f852c8a5`.
