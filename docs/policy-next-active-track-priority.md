...
1. 개인 캐시 기반 추천 응답 가속
2. 사용자 규모 증가 시 군집 캐시 재검토
3. 카카오 알림톡 blocked 재검토

이유:

- `검색 로그`, `대시보드` 는 현재 로컬 코드/데이터만으로 구현·검증을 끝냈다.
- 현재 프로젝트 규모에서는 군집 캐시보다 `userKey` 기준 개인 캐시가 더 단순하고 개인화 손실이 적다.
...
- 따라서 현재 phase에서 더 진행할 실용적 후보는 `개인 캐시 기반 추천 가속` 이고, 군집 캐시는 장래 확장 포인트로 남긴다.
