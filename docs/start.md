...
- chatbot baseline을 compact하게 다시 보려면 `bash deploy/smoke/run-local-chat-followup-scenario-audit.sh`, `bash deploy/smoke/run-local-chat-continuity-coaching-smoke.sh`, `bash deploy/smoke/run-local-chat-application-coaching-matrix-audit.sh` 순서로 봅니다.
- 운영 서버/RDS에서 챗봇 대화 이어짐/신청 코칭을 다시 보려면 `ENV_FILE=.env.production SMOKE_DB_MODE=postgres APP_BASE_URL='http://127.0.0.1:8082' bash deploy/smoke/run-local-chat-followup-scenario-audit.sh`, `ENV_FILE=.env.production SMOKE_DB_MODE=postgres APP_BASE_URL='http://127.0.0.1:8082' bash deploy/smoke/run-local-chat-continuity-coaching-smoke.sh`, `ENV_FILE=.env.production SMOKE_DB_MODE=postgres APP_BASE_URL='http://127.0.0.1:8082' bash deploy/smoke/run-local-chat-application-coaching-matrix-audit.sh` 를 순서대로 실행합니다.
- 실사용자 챗봇 표본은 `ENV_FILE=.env.production SMOKE_DB_MODE=postgres bash deploy/smoke/run-local-chat-real-user-quality-sample-audit.sh`, 운영 지표는 `ENV_FILE=.env.production SMOKE_DB_MODE=postgres bash deploy/smoke/run-local-chat-observability-audit.sh` 로 봅니다. 실사용자 원문 질문은 artifact에 남기지 않습니다.
...
- nightly 로그 적재형 wrapper는 `ENV_FILE=.env.production SMOKE_DB_MODE=postgres APP_BASE_URL='http://127.0.0.1:8082' bash deploy/smoke/run-nightly-policy-quality-observation.sh`
