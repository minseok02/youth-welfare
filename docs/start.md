...
- `2026-05-20` KST 기준 local generic-domain signup으로 만든 분산 `REAL_USER` 샘플은 총 30명까지 확보됐습니다. live readiness는 계속 `READY_REAL_USER_TRAFFIC / READY_REAL_USER_COHORT` 로 열려 있고, real-user-only concentration은 `top1_share_pct=10.00`, `concentration_readiness=NO_PRIORITY_DOMINANT` 로 분산됐습니다. 다만 mixed latest batch review gate는 여전히 `recommendation_review_gate=DEFERRED_NON_REAL_LEADER_SIGNAL` 이고, `REAL_USER` top-N 분포에서는 `zero_ai_rows=22`, `zero_ai_reason_buckets=AUDIENCE_MISMATCH:3,INCOME_MISMATCH:5,OTHER:7,STUDENT_AUDIENCE_MISMATCH:7` 이 다시 보입니다. latest status artifact는 계속 `VOLATILE_ONLY_DRIFT`, 기본 gate `PASS`, strict gate `LATEST_OBSERVATION_CHANGED` 이고, readiness 포함 overview에서는 `effective_operator_next_step=WAIT_FOR_REAL_USER_LEADER_SIGNAL` 로 읽는 편이 맞습니다.
