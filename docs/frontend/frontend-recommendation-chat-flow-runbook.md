...
- 마이페이지 동의 철회는 확인 모달을 거친 뒤 선택정보/민감정보 체크 해제뿐 아니라 관련 입력값 초기화까지 보여야 함
...
- `마이페이지 동의 철회는 선택정보와 민감정보 철회 API를 호출하고 체크 상태를 해제한다 @dev-only`
...
로컬 Docker stack에서 전체 `npm run test:e2e` 를 실행할 때는 먼저 `frontend/scripts/bootstrap-playwright-smoke-data.sh` 로 일반 사용자/관리자 계정과 searchable policy fixture를 준비합니다.
