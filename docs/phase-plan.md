...
남은 작업은 운영 배포/운영성 검증(운영 서버 Docker Compose, 기존 운영 DB 계정 생성 SQL 적용 및 datasource 전환, HTTPS/Nginx, Spring 다중 datasource 분리, 운영 DB에 `V2026_04_28_01__drop_runtime_legacy_user_id.sql` 적용 후 smoke 검증, CTR 표본 확충 후 재분석)과 2차 확장 기능(군집 캐시 추천, 카카오 알림톡, 검색 로그, 대시보드)입니다.
...
- [ ] HTTPS/Nginx 적용
- [ ] Spring 다중 datasource 분리로 `app_core_rw` / `app_pii_rw` / `notification_pii_ro` 실제 사용 경로 분리
