...
- 2026-05-16: 추천 편중 완화는 다시 no-priority top band 경계로 좁혔다. `ReRankingService` 는 이제 근접 top band 안에서 개별 후보 순서만 돌리지 않고, `category`(없으면 `source`) 버킷 기준으로 먼저 top1을 회전시킨다. 다만 현재 저장 추천 batch를 새로 만들지 않은 상태라 concentration audit 수치는 아직 `top1_leader_share_pct=56.41`, `CONCENTRATED_TOP1` 그대로이며, 이 단계는 “다음 refresh에서 더 자주 다른 bucket이 top1로 오르게 만드는 코드 반영”으로 읽는 것이 맞다.
