...
- `2026-05-16`: 코드를 다시 읽어보니 현재 `ClusterService` 는 항상 `youth_all` 을 반환하므로, 1차 운영 경로는 `AiScoringService -> RealtimeAiGateway` 직행이고 cluster cache를 타지 않는다. 따라서 서버 score audit에서 본 `GOV24 top2 avg rule=90.00 / avg ai=0.00 / avg final=0.27375` 는 cache drift보다 **실제 AI 결과 0점 또는 그 저장값**일 가능성이 높다. 이를 좁히기 위해 `deploy/smoke/run-local-gov24-zero-ai-audit.sh` 를 추가해 latest batch의 `GOV24 + ai_score=0` rank 분포, 대표 서비스, `ai_reason` 샘플을 바로 보게 했다.
- `2026-05-16`: zero-AI audit까지 보니 local baseline은 `gov24_zero_ai_top10_count=165` 이지만 `gov24_zero_ai_top2_count=0` 이고, 서버도 `gov24_zero_ai_top2_count=0` 이었다. 즉 Gov24 상위권 약세의 직접 원인은 `top2 zero-AI` 자체가 아니라, **같은 user에서 Gov24 top2 후보가 rank1 경쟁 후보보다 final score를 못 이기는 구조**로 읽는 편이 더 맞다. 이를 위해 `deploy/smoke/run-local-gov24-top2-competitor-audit.sh` 를 추가해 `Gov24 top2 row` 와 같은 user의 `rank1 rival` 을 `rule/ai/final/ai_reason` 기준으로 바로 비교하게 했다.
