...
- 2026-05-17: `/api/admin/dashboard/recommendation-breakdowns` 에 latest batch concentration snapshot도 올렸다. 응답은 이제 `latestBatchConcentration.latestBatchRows/users/distinctServices`, `top1Leader*`, `concentrationReadiness`, `realUserCohortGate`, `signalQuality` 를 함께 내리고, `run-local-admin-recommendation-breakdowns-smoke.sh` 도 이 계약을 검사한다. 현재 local smoke 값은 `top1LeaderServiceId=2622`, `top1LeaderSharePct=59.69`, `concentrationReadiness=CONCENTRATED_TOP1`, `realUserCohortGate=DEFERRED_NO_REAL_USER_COHORT`, `signalQuality=LOCAL_REAL_NON_EXAMPLE_SEED_WITH_NON_REAL_BATCH` 이다. 즉 이제 breakdown API만 봐도 current latest batch 편중과 real-user 부재를 shell audit 없이 바로 읽을 수 있다.
