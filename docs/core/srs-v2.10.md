...
본 문서는 "청년정책 통합포털"의 기능적·비기능적 요구사항을 정의한다.

### 1.2 프로젝트 범위

온통청년, 복지로 중앙부처, 복지로 지자체, Gov24 공공서비스(혜택) API에서 청년 복지·정책 정보를 수집하여 통합 저장하고, 사용자의 개인 상황 및 선택 우선순위에 기반한 개인화 추천(Retrieval → Re-ranking)으로 맞춤 정책을 제공한다. Cold Start 문제에 대응하기 위해 가중치 기반 테이블리드 추천을 적용하며, 확장형 MVP 구조로 단계적 고도화를 지원한다.
...
|------|------|
| 청년 | 만 18세 이상 39세 이하 |
| 군집(Cluster) | 나이대·소득구간 등으로 묶인 사용자 그룹. 현재 운영 기준은 `youth_all` 단일 군집이며, 사용자 규모가 충분히 커지기 전까지는 적극적 군집 캐시보다 개인 캐시를 우선 검토 |
...
| 시나리오 | 인프라 | 월비용 |
|---------|--------|---------|
| 졸업 데모 | EC2 t3.medium (4GB x86_64) + Docker Compose | 리전/트래픽에 따라 변동 |
| 성공 초창기 | EC2 t3.medium + RDS 또는 상위 사양 | 리전/트래픽에 따라 변동 |
...
- 따라서 현재 Compose 3컨테이너를 같은 호스트에서 운영할 때 `t3.small`은 여유가 부족하고, `t3.medium`을 권장한다
...
| ID | 요구사항 | 우선순위 | 구현 단계 |
|----|---------|:--------:|:--------:|
| FR-07-01 | **군집화**: 1차 youth_all 단일 군집. 사용자 수가 충분히 커지기 전까지는 적극적 군집 캐시보다 개인 캐시를 우선한다. 2차 후보로 나이대(4구간) × 소득구간(2구간) = 최대 8개 군집을 검토할 수 있다 | 필수 | 1차→2차 |
| FR-07-02 | 2차 군집 폴백: `min_cluster_size`(기본 3) 미만이면 나이대 단일 군집 → "전체 청년" **2단계 폴백만**. 단, 이는 사용자 규모가 실제로 커져 군집 캐시 검토 가치가 생긴 뒤에만 active로 올린다 | 필수 | 2차 |
