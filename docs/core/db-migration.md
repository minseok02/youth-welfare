...
- 기존 로컬 PostgreSQL 볼륨이 최신 `schema.sql` 보다 뒤처졌다면 `deploy/postgres/patches/*.sql` 과 `deploy/postgres/apply-local-runtime-schema-patch.sh` 로 drift를 먼저 맞춘 뒤 integration/runtime smoke를 다시 실행합니다. 현재 patch 경로에는 `recommendation_review_gate_promotion_approvals`, `recent_policy_views`, read-only/cleanup/command role-grant, `app_core_rw recent_policy_views DELETE revoke`, `app_core_rw recommendation_review_gate_promotion_approvals ALL revoke`, 주요 누락 컬럼 patch가 포함됩니다.
