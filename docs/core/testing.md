...
(`chat_messages.references_json`, `chat_retrieval_snapshots.needs_clarification`, `recommendation_review_gate_promotion_approvals`, `recent_policy_views`, `admin_dashboard_ro`, `recommendation_review_gate_command_rw`, `chat_session_cleanup_rw`, `cluster_ai_cleanup_rw`, `recommendation_retention_cleanup_rw`, `collect_execution_lock_cleanup_rw`, `web_push_subscription_cleanup_rw` role/grant, `app_core_rw DELETE revoke(chat_sessions / cluster_ai_results / collect_execution_locks / web_push_subscriptions / recent_policy_views)`, `app_core_rw ALL revoke(recommendation_review_gate_promotion_approvals)`, `users.name/birth_date` plain duplicate null patch 등)를 재적용합니다.
