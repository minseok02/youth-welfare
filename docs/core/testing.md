...
이 preflight 는 이제 포트 접근성만 보지 않고, primary / admin-ro / cluster-ai-cleanup / recommendation-retention-cleanup / pii-rw / notification-pii-ro datasource에 실제 JDBC 로그인까지 확인합니다.
...
(`chat_messages.references_json`, `chat_retrieval_snapshots.needs_clarification`, `admin_dashboard_ro`, `cluster_ai_cleanup_rw`, `recommendation_retention_cleanup_rw` role/grant 등)를 재적용합니다.
