...
(`chat_messages.references_json`, `chat_retrieval_snapshots.needs_clarification`, `recommendation_review_gate_promotion_approvals`, `recent_policy_views`, `admin_dashboard_ro`, `cluster_ai_cleanup_rw`, `recommendation_retention_cleanup_rw`, `collect_execution_lock_cleanup_rw`, `web_push_subscription_cleanup_rw` role/grant, `users.name/birth_date` plain duplicate null patch 등)를 재적용합니다.
