...
- 2026-06-20 운영 샘플 최신 1000건 재점검에서 한글 도메인 URL이 정규식 경계에서 `https://www` 로 잘리는 후보 4건을 확인했습니다. 현재 처리는 잘못된 CTA를 노출하지 않도록 단일 라벨 host를 action link에서 제외하는 보수적 차단이며, 한글 도메인을 살리는 IDN/punycode 보존은 별도 후속 범위입니다.
