...
- 해결: 새 `run-local-ops-observation-suite.sh` 를 추가해 기존 `run-local-ops-baseline-suite.sh` child artifact를 재사용하고, `decision_class`, `collect_failed_jobs_in_window`, `collect_partial_success_jobs_in_window`, `open_collect_circuits`, `recommendation_review_gate`, `recommendation_real_user_traffic_gate_in_window`, `operator_reading`, `next_action` 을 `summary/json/note` artifact로 다시 publish하게 맞췄다. 동시에 [ops-observation-runbook.md](/home/minseok/youth-welfare/docs/core/ops-observation-runbook.md:1), [ops-baseline-runbook.md](/home/minseok/youth-welfare/docs/core/ops-baseline-runbook.md:1), [start.md](/home/minseok/youth-welfare/docs/start.md:1), [current-state.md](/home/minseok/youth-welfare/docs/current-state.md:1), [local-validation-docs-index.md](/home/minseok/youth-welfare/docs/core/local-validation-docs-index.md:1) 에 compact entrypoint를 연결했다.
...
- 문제: [AuthController](/home/minseok/youth-welfare/backend/src/main/java/com/example/welfare/user/controller/AuthController.java:1) 는 이미 이메일 중복 확인, 이메일 인증 발송에는 fingerprint rate limit을 태우고 있었지만 `/api/auth/login`, `/api/auth/password-reset/request` 는 그대로 service로 내려보냈다. 계정 잠금과 이메일별 cooldown은 있어도, 같은 IP/UA fingerprint에서 여러 계정으로 로그인 시도하거나 여러 이메일로 reset request를 흩뿌리는 행위는 별도 클라이언트 축에서 막지 못했다.
- 해결: [AuthRateLimitService](/home/minseok/youth-welfare/backend/src/main/java/com/example/welfare/user/service/AuthRateLimitService.java:1) 에 `auth:rate-limit:login:*`, `auth:rate-limit:password-reset-request:*` 키를 추가하고, [AuthController](/home/minseok/youth-welfare/backend/src/main/java/com/example/welfare/user/controller/AuthController.java:1) 가 `ClientFingerprintService.build(request)` 후 이 제한을 먼저 검사하게 바꿨다. 기본값은 `login <= 20 / 300초`, `password-reset-request <= 5 / 300초` 로 두어 계정 잠금/이메일 cooldown은 그대로 유지하면서 클라이언트 축 방어만 보강한다.

## 987) admin 작업 상한은 “너무 큰 값 거부”만 볼 게 아니라 default 자체도 상한 안에 있는지 같이 맞춰야 한다
- 문제: [CollectAdminController](/home/minseok/youth-welfare/backend/src/main/java/com/example/welfare/collect/controller/CollectAdminController.java:1) 의 `bokjiro-details-gap-fill` 는 원래 `maxCallsPerRound` default가 `10000` 이었다. 여기서 상한을 `1000` 으로만 추가하면, 파라미터를 생략한 정상 호출도 default 때문에 바로 `400` 이 되어 버린다.
...
- 문제: [RecommendationController](/home/minseok/youth-welfare/backend/src/main/java/com/example/welfare/recommend/controller/RecommendationController.java:1) 의 `size` 만 검증하면 외부 HTTP 요청은 막을 수 있지만, 나중에 다른 service/scheduler/controller caller가 생기면 [RecommendationResultReadRepositoryImpl](/home/minseok/youth-welfare/backend/src/main/java/com/example/welfare/recommend/repository/RecommendationResultReadRepositoryImpl.java:1) 의 `PageRequest.of(0, size)` 에 raw 값이 다시 내려갈 수 있다.
...
- 해결: [frontend/e2e/smoke.spec.js](/home/minseok/youth-welfare/frontend/e2e/smoke.spec.js:1) 의 `issuePasswordReset()` 은 기본적으로 real API request를 유지하되, 응답이 `A009` 일 때만 새 [issue-password-reset-token.sh](/home/minseok/youth-welfare/frontend/scripts/issue-password-reset-token.sh:1) helper로 Redis에 reset token을 직접 seed해서 브라우저 flow를 계속 검증하게 바꿨다.
