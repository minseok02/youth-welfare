...
- 해결: `run-local-admin-dashboard-smoke.sh`, `run-local-admin-collect-failures-smoke.sh`, `run-local-admin-recommendation-breakdowns-smoke.sh` 가 이제 `ADMIN_ACCESS_TOKEN` 을 우선 재사용한다. 토큰이 없더라도 `ENV_FILE` 의 `JWT_SECRET` 과 DB query credential이 있으면 `auth_users.email_lookup_hash -> users(user_key,id)` 경로로 admin 계정을 찾고, short-lived `ROLE_USER,ROLE_ADMIN` access token을 bounded mint 해서 read-only admin API 호출에 쓴다. 비밀번호는 token/login 둘 다 없을 때만 fallback 전제로 남긴다.
