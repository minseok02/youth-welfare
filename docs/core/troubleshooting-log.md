...
## 1071) server/RDS smoke는 app runtime DB와 admin PII 분리 기준을 같이 맞춰야 한다

- 문제: server app은 RDS/runtime env로 떠 있는데 smoke가 기본 Docker DB 경로를 타면, admin dashboard나 chat coaching처럼 DB 상태에 의존하는 smoke가 제품 회귀처럼 실패할 수 있었다. 또 PII 분리 뒤 admin bootstrap 계정을 `users.email` 로 찾으면, 실제 lookup source인 `auth_users.email_lookup_hash` 와 user email shadow 저장 규칙을 놓쳐 admin smoke bootstrap이 깨질 수 있었다.
- 해결: server/RDS smoke는 `ENV_FILE=.env.runtime.production SMOKE_DB_MODE=postgres APP_BASE_URL='http://127.0.0.1:8082'` 로 app container와 DB를 맞춰 실행한다. 운영 admin password를 쓰지 않는 smoke 검증은 `ALLOW_ADMIN_JWT_MINT=true` 로 bounded local token mint 경로를 명시한다. [smoke-common.sh](../../deploy/smoke/smoke-common.sh) 의 admin 계정 보정은 `auth_users.email_lookup_hash` 기준으로 bootstrap 사용자를 찾고, `users.email` 은 user email shadow 규칙에 맞춰 저장하도록 고쳤다.
- 확인: `bash -n deploy/smoke/smoke-common.sh`, `run-local-admin-dashboard-smoke.sh`, `run-local-active-baseline-suite.sh`, `run-local-collect-governance-observation-suite.sh`, `run-local-notification-channel-smoke.sh`, `run-local-policy-quality-observation-suite.sh`, `run-local-chat-application-coaching-matrix-audit.sh` 를 같은 server/RDS env로 재실행해 통과했다. 최신 active baseline artifact는 `tmp/active-baseline-suite/20260627T123051Z` 이고 `active_baseline_suite=passed`, `ops_observation_decision_class=BASELINE_HEALTHY` 다.
- 데이터 보정: user profile standard-code coverage audit에서 `house_tenure_code=NONE` 인 실제 사용자 1건의 `housing_type_code=4` 가 발견되어 `users` 와 `user_profiles` 를 각각 `housing_type_code=NONE` 으로 bounded 보정했다. 재실행 결과 `users_house_tenure_none_housing_type_mismatch=0`, `profiles_house_tenure_none_housing_type_mismatch=0`, `safe_reconcile_candidate_rows=0`, `conflicting_value_gap_rows=0` 으로 닫혔다.
- 이유: PII 분리 이후에는 표시용 user row와 인증 lookup row의 source-of-truth가 다르다. smoke helper도 제품의 lookup/hash/shadow 계약을 그대로 따라야 하고, server smoke는 app이 실제로 물고 있는 DB와 같은 env를 써야 실패를 올바르게 해석할 수 있다.

## 1070) URL/route hardening은 query 제거만으로 끝내지 말고 nested router state와 저장 URL까지 같이 봐야 한다

- 문제: reset/unsubscribe/recommendation/chat 흐름에서 URL query/hash의 민감값은 대부분 제거됐지만, 로그인/회원가입/비밀번호 재설정/챗봇 왕복처럼 route state를 중첩 전달하는 경로는 `from/chatFrom` 내부에 transient 값이 남을 수 있었다. 또 관리자 `detailUrl` 보정은 사용자 화면에서 `http/https`만 렌더링하더라도 저장 단계에서는 스킴 검증이 없어 잘못된 URL이 원천 데이터로 들어갈 수 있었다.
- 해결: [safeNavigation.js](../../frontend/src/lib/safeNavigation.js) 에 `coachPolicyId`, `recommendationLogId` 제거와 nested `from/chatFrom` 검증을 묶은 공통 sanitizer를 두고, 보호 라우트/헤더/하단 내비/auth flow/정책 상세/챗봇 왕복 경로에 적용했다. 추천 클릭은 URL `log_id`를 기록하지 않고 POST body로만 처리한 뒤 state에서 제거한다. 알림/추천 메일 deeplink는 내부 경로만 absolute URL로 만들고, 관리자 `DETAIL_URL` 보정은 `http/https`, host 존재, user-info 금지 조건을 저장 전에 검증한다.
- 추가 점검: 현재 화면을 다시 누르는 Header/FloatingNav/MobileBottomNav 경로가 raw `location.state` 를 그대로 재사용하지 않도록 `returnLocation.state` 와 transient sanitizer를 거치게 했다. `postLoginAction` 도 허용 type(`toggle-bookmark`)과 양수 정수 `policyId` 만 남기는 `sanitizePostLoginAction` 으로 정규화했다. `/policies` URL query sync와 정책 상세 legacy `log_id` 제거도 sanitized state만 전달한다.
- 관리자/로그 보강: `GET /api/admin/dashboard/recommendation-diagnostics?userKey=...&serviceId=...` 는 `POST body` 로 전환하고, `POST /api/admin/users/pii-sync-replay?userKey=...` 도 optional body DTO로 옮겼다. stale 알림 hide의 `deeplinkUrl` 은 컨트롤러 DTO뿐 아니라 `AdminNotificationBacklogService` 에서도 내부 경로만 허용한다. 운영 로그의 원문 `userKey` 는 `RedisKeyHash.sha256Hex(...)` 기반 `userKeyHash` 로 바꿔 URL이 아니더라도 로그 식별자 노출을 줄였다.
- 테스트 보강: Playwright smoke의 채팅 신청 도움 테스트는 더 이상 `/chat?session=...` 를 기대하지 않고, reset-password query/hash token 테스트도 token을 읽은 뒤 `/reset-password` 로 주소창이 정리되는 계약을 기대한다. 이 테스트는 브라우저 히스토리와 새로고침에서 기능 상태가 유지되는지 확인하는 URL 노출 회귀 방지선이다.
- 2차 보강: 관리자 대시보드 목록 화면의 제보자/문의자/처리자 `userKey` 표시는 원문 대신 축약 표시로 바꿨고, 챗봇 사이드바의 `세션 #id` 노출도 제거했다. reset-password invalid token과 notification unsubscribe token E2E는 back/forward 후에도 token URL이 복구되지 않는지 확인한다. 알림/푸시/정책 보강 로그의 `e.getMessage()` 원문 기록은 `errorType` 또는 고정 실패 문구로 줄여 외부 URL/secret이 예외 메시지를 통해 남지 않게 했다.
- 3차 보강: 관리자 대시보드의 정책 오류 제보/서비스 문의 목록은 이메일을 축약 표시하고, 문의/운영 메모의 이메일·전화번호·식별번호형 문자열을 표시 직전에 마스킹한다. 지원 문의 `routePath` 는 서버 저장 전 query/hash를 제거하고 `//evil` 같은 protocol-relative 경로를 거부한다. 검색 자동완성과 실제 정책 검색은 `GET query` 대신 `POST body` 로 전환해 draft/applied keyword가 API URL에 남지 않게 했고, 검색 로그/인기어/자동완성 후보는 이메일·전화번호·주민번호·긴 번호형 키워드를 저장/반환하지 않는다. 알림 수신거부는 Redis에 해시 저장된 opaque token만 허용하고, 기존 JWT fallback은 제거했다.
- 유지한 공개 URL 상태: 정책 목록의 `category/region/subRegion/ward/income/targetGroup/sourceType/gov24*/statusFilter/sort/page/pageSize`, 마이페이지 `tab`, 관리자 운영 필터의 `limit/status/windowDays`, 공식 코드북 `q` 는 화면 복원/운영 조회 상태로 유지했다. 정책 자유 검색어는 과거 `/policies?search=...` 진입만 1회 읽고 즉시 URL에서 제거하며, 목록 복원은 `history.state.policySearchKeyword` 와 `POST /api/policies/search` body로만 처리한다. `q` 는 관리자 코드북 조회용 제한 surface로 남아 있으므로 민감 문자열 입력 가능성을 운영 문서에 남긴다.
- 4차 보강: `refresh/logout/unsubscribe` 처럼 쿠키 또는 공개 token을 쓰는 보호 POST는 `TrustedOriginFilter` 에서 허용된 `Origin` 또는 `Referer` 가 없으면 403으로 거부한다. access token은 zustand 메모리에만 두고 `auth-ui-store` localStorage persist에는 `filterSettings` 만 남긴다. 남은 localStorage 쓰기는 가이드 안내 확인 마커와 admin E2E 실패 주입 키뿐이며, token/email/search keyword 저장소 쓰기는 남기지 않는다.
- 이유: 브라우저 주소창에 보이는 query만 지워도 당장 노출은 줄지만, SPA에서는 route state가 다음 화면·로그인 복귀·뒤로가기 흐름으로 계속 전달된다. 민감/일회성 상태는 공통 sanitizer에서 제거하고, 외부 링크는 저장 단계와 렌더링 단계 양쪽에서 검증해야 같은 문제가 다른 화면으로 재발하지 않는다.

## 1069) 회원가입 저장/지역 조회/URL 민감값 노출은 기능별로 나눠 보되 최종 기록은 URL에 남는 값 기준으로 닫는다
- 문제: 회원가입 과정에서 진행한 값이 저장되지 않는 문제를 확인한 뒤, 같은 관점으로 지역 필터와 URL/URI 노출을 다시 전수 점검해야 했다. 특히 이메일/인증코드, reset/unsubscribe token, 추천 클릭 `log_id`, 채팅 `session` 같은 값은 기능은 정상이어도 주소창, 공유 URL, 이메일 링크, referrer, 브라우저 히스토리에 남을 수 있었다.
- 원인: 기능별 구현 시점이 달라 URL 상태의 성격이 섞여 있었다. 정책 검색 필터처럼 공유 가능한 공개 상태와, 추천 로그 ID/채팅 세션 ID처럼 사용자별 내부 리소스 식별자가 같은 query 방식으로 다뤄졌다. 이메일 중복확인/인증도 일부 경로는 query/params 기반으로 남아 있었고, Header 알림 딥링크는 `AlertsPage`/`MyPage` 와 달리 공통 safe navigation 검증을 통과하지 않았다.
- 해결: 지역 조회는 `PolicyListService`, `PolicySearchService`, `WelfareServiceReadRepositoryImpl`, `WelfareServiceSearchRepositoryImpl` 에서 복지로 지역명과 온통청년 `region_code` 조건이 목록/검색 모두에 반영되도록 보강했다. 회원가입은 `SignupRequest`, `UserRegistrationService`, `SignupPage.jsx` 를 정리해 core/PII dual-write 저장 누락을 막고 테스트로 고정했다. 이메일 중복확인/인증 발송/검증은 `GET/query` 대신 `POST body` 계약으로 통일했다. 알림 수신거부는 legacy `GET /api/notifications/unsubscribe?token=...` 를 제거하고 `POST body` 만 허용했다. reset/unsubscribe 화면은 `#token=` 또는 query token을 읽은 직후 `history.replaceState` 로 주소창에서 제거한다.
- 추천 클릭 추적: `/policies/{id}?log_id=...`, `/api/policies/{id}?logId=...`, 추천 이메일의 `?log_id=` 노출을 제거했다. 추천 클릭은 `POST /api/policies/{id}/recommendation-click` body `{ logId }` 로 기록하고, `RecommendationLogService` 는 `logId + userKey + serviceId` 를 함께 검증한다. 프론트는 `MainPage.jsx` 에서 추천 로그 ID를 router state로 넘기고, `PolicyDetailPage.jsx` 는 legacy `log_id` query가 들어와도 즉시 제거한다. 공유 버튼은 origin + pathname만 복사한다.
- 채팅 세션 노출: `/chat?session=...` 으로 활성 세션을 유지하던 구조를 `location.state.activeChatSessionId` 로 바꿨다. 기존 `?session=` 은 읽은 뒤 즉시 제거한다. 정책 상세에서 챗봇으로 넘기던 `/chat?coachPolicyId=...` 도 router state 전달로 바꿨다. 백엔드 API path의 `/api/chat/sessions/{sessionId}` 는 리소스 API 구조라 유지하되, 사용자에게 공유 가능한 프론트 URL에서는 세션 ID를 제거했다.
- 딥링크/외부 링크: Header 알림 클릭도 `resolveSafeInternalPath` 를 거치게 맞췄다. 관리자 stale 알림 hide 요청의 `deeplinkUrl` 은 `^/(?!/)` 정규식으로 `//evil.example/...` 같은 protocol-relative URL을 거부한다. 챗봇 답변 `actionLinks` 는 렌더링 직전에 `http:`/`https:` 만 허용하고 나머지는 버튼을 만들지 않는다. 잘못된 HTTP method는 `GlobalExceptionHandler` 에서 405로 처리한다.
- 유지한 URL 상태: 정책 검색어/필터, 정책 카테고리, 지역, 소득, 대상 그룹, 정렬, 페이지, 마이페이지 탭, 공개 정책 ID path는 화면 복원/공유 가능한 공개 상태로 보고 유지했다. 다만 검색 키워드는 사용자가 민감한 내용을 입력할 수 있으므로, privacy-first 정책을 더 강하게 잡으면 session state 또는 POST 검색으로 전환할 후속 후보로 남겼다.
- 외부 공급자 API: 공공데이터 수집 API는 공급자 규격상 `serviceKey`, `apiKeyNm` query를 사용한다. 이 경로는 public app URL이 아니라 server-to-server 호출이며, `CollectHttpRetryExecutor` 가 `serviceKey`, `apiKey`, `token`, `access_token`, `refresh_token`, `client_secret`, `secret`, `password` 값을 로그 라벨에서 `<redacted>` 처리하는 기존 방어와 테스트가 있음을 확인했다.
- 확인: `./gradlew test`, `npm run lint`, `npm run build` 통과. 관련 통합 테스트 `AuthRedisIntegrationTest`, `PolicySearchRegionQueryIntegrationTest`, `UserCoreDualWriteIntegrationTest` 도 통과했다. 처음 통합 테스트는 `127.0.0.1:5433` Postgres가 꺼져 preflight에서 실패했으나, `COMPOSE_FILE=docker-compose.yml ALLOW_LOCAL_DOCKER_DB=true docker compose up -d db` 로 테스트 DB만 올려 재실행했고 통과 후 `docker compose stop db` 로 다시 내렸다. 최종 위험 패턴 검색에서 `?log_id`, `log_id=`, `?session=`, `/chat?coachPolicyId`, 이메일/토큰/코드를 axios `params` 로 보내는 호출, legacy unsubscribe/check-email GET 매핑, 정책 상세 GET의 `logId` request parameter는 남지 않았다.
...
- 해결: [smoke.spec.js](../../frontend/e2e/smoke.spec.js) 에 정책 상세/목록/북마크 탭 readiness helper를 추가하고, 클릭과 URL 전환은 `Promise.all` 로 묶었다. 정책 목록/마이페이지 북마크 카드는 텍스트 div 대신 실제 클릭 컨테이너를 안정적으로 잡도록 [PoliciesPage.jsx](../../frontend/src/pages/PoliciesPage.jsx), [MyPage.jsx](../../frontend/src/pages/MyPage.jsx) 에 테스트 전용 `data-testid` 를 붙였다. 알림 unread 테스트는 `PATCH /api/notifications/9001/read` response 완료 후 count를 확인한다.
...
- 문제: API smoke와 backend test는 통과하는데 Playwright 브라우저 E2E에서 로그인/정책목록/reset-password 요청이 네트워크 오류처럼 실패했다. 브라우저 화면에는 [ServerErrorBanner.jsx](../../frontend/src/components/ServerErrorBanner.jsx) 의 `서버에 연결할 수 없습니다` 배너가 뜨고, Playwright request 기반 API 조회는 성공하는 패턴이었다. 이는 backend API 자체 장애보다 브라우저 origin(`http://127.0.0.1:5173`)에 대한 CORS preflight 실패 가능성이 높다.
...
- 해결: [build.gradle](../../backend/build.gradle) 의 `bootRun` 에서 `SECURITY_CORS_ALLOWED_ORIGINS` 를 다시 주입할 때 기존 값은 보존하되 `http://127.0.0.1:5173`, `http://localhost:5173` 를 반드시 포함하게 했다. [BootRunCorsContractTest.java](../../backend/src/test/java/com/example/welfare/global/config/BootRunCorsContractTest.java) 로 이 계약을 고정했다.
...
- 해결: [RecommendationAiReasonSanitizer.java](../../backend/src/main/java/com/example/welfare/recommend/support/RecommendationAiReasonSanitizer.java) 를 추가해 blank reason 은 `null`, 개행/탭/중복 공백은 단일 공백, 장문은 20 code point 이내로 정규화했다. 이 sanitizer를 `RealtimeAiGateway`, `AiScoringService`, `JpaClusterAiScoreCache`, `RecommendationPersistenceService`, `RecommendationResponse` 에 모두 적용했다.
- 문서화: 사용자 노출 계약은 [recommendation-ai-reason-memo-contract.md](../recommendation/recommendation-ai-reason-memo-contract.md) 로 분리했고, `recommendation-current-state.md`, `recommendation-docs-index.md`, `api-mapping.md` 에 연결했다.
...
- 해결: [PolicyDetailPage.jsx](../../frontend/src/pages/PolicyDetailPage.jsx) 의 `Gov24` 태그를 clickable chip으로 바꿔 `serviceField`, `userType`, `benefitType` 각각 `/policies?sourceType=GOV24&...` 로 바로 이동하게 했다. 즉 `분야/대상/유형` badge를 read-only 메타가 아니라 bounded discovery bridge로 승격한다.
...
- 해결: [policy-gov24-lane-closeout.md](../policy/policy-gov24-lane-closeout.md) 를 추가해 현재 `Gov24` lane의 active/deferred 경계를 한 장으로 고정했다. 이 문서는 public filter 3축, recommendation soft additive scoring, matcher hard condition deferred, `service_facts` deferred, `Gov24 -> YOUTH_MID` bridge deferred, stable import/backfill deferred 를 현재 시제 기준으로 다시 정리한다.
...
- 해결: [RuleScoringService.java](../../backend/src/main/java/com/example/welfare/recommend/service/RuleScoringService.java) 는 새 [Gov24RecommendationScoringSupport.java](../../backend/src/main/java/com/example/welfare/recommend/support/Gov24RecommendationScoringSupport.java) 를 통해 `Gov24` projection을 **bounded soft additive bonus** 로만 소비한다. `serviceField` 는 existing compat bridge(`주거·자립 -> HOUSING`, `고용·창업 -> JOB`, `보육·교육 -> EDUCATION`, `생활안정 -> FINANCE`, `문화·환경 -> CULTURE`, `보호·돌봄/임신·출산 -> FAMILY`)를 small bonus로 재사용하고, `benefitType` 도 `현금(장학금)`, `현금(융자)`, `서비스(일자리)`, `문화/여가지원`, `서비스(돌봄)` 같은 일부 managed token만 additive signal로 읽는다. `userType` 은 `개인`, `가구` 만 tiny audience bonus로 쓰고, `법인/시설/단체`, `소상공인` 은 hard exclusion으로 해석하지 않는다.
- 추가 해결: [DefaultPriorityMatcherTest.java](../../backend/src/test/java/com/example/welfare/recommend/service/DefaultPriorityMatcherTest.java) 에 `Gov24` taxonomy projection만으로는 `HOUSING/FINANCE` priority matcher가 열리지 않는 회귀를 추가해, scoring만 active이고 matcher hard condition은 계속 deferred 라는 경계를 테스트로 고정했다.
- 추가 해결 2: [CanonicalRecommendationReadModelRepositoryTest.java](../../backend/src/test/java/com/example/welfare/recommend/repository/CanonicalRecommendationReadModelRepositoryTest.java) 에 `Gov24` canonical term이 recommendation projection에 들어와도 `youthMajorLabel/youthMidLabel` 은 자동으로 채우지 않는 회귀를 추가했다. 즉 `Gov24 -> YOUTH_MID` bridge도 현재는 deferred 라는 경계를 read-model 단계에서 고정한다.
- 이유: 지금 필요한 것은 `Gov24` taxonomy를 recommendation에서 “전혀 안 쓰는 상태”를 벗어나는 것이지, canonical label/token 축을 곧바로 hard gate로 오해하는 것이 아니다. soft scoring으로만 좁게 열어 두면 public filter와 recommendation이 같은 canonical truth를 재사용하면서도, matcher hard condition과 `service_facts` 승격은 계속 분리할 수 있다.
...
- 이유: compact operator wrapper는 기본 서버 명령이 운영 현실에서 바로 통과해야 재사용 가치가 있다. read-only admin baseline까지 매번 비밀번호 파일이나 임시 hash reset을 요구하면 recommendation/collect/policy/auth/frontend compact surface와 운영 계약이 다시 달라진다.

## 1054) ops baseline도 raw child stdout 세 개만 남겨 두면 daily operator가 collect drift와 recommendation admin surface를 다시 해석하는 비용이 recommendation/collect compact wrapper보다 더 크다
- 문제: `run-local-ops-baseline-suite.sh` 는 health, admin dashboard, collect failures, recommendation breakdowns 를 read-only로 다시 묶어 주지만, 결과가 child stdout 경로 세 개 중심이라 운영자 handoff 관점에서는 다시 `collect failed/partial/circuit`, `recommendation review gate`, `leader signal`, `next action` 을 직접 풀어 읽어야 했다. recommendation/collect/policy/auth/frontend 는 이미 compact observation surface가 있는데 ops만 raw baseline wrapper에 머물면 운영 해석 형식이 다시 달라진다.
- 해결: 새 `run-local-ops-observation-suite.sh` 를 추가해 기존 `run-local-ops-baseline-suite.sh` child artifact를 재사용하고, `decision_class`, `collect_failed_jobs_in_window`, `collect_partial_success_jobs_in_window`, `open_collect_circuits`, `recommendation_review_gate`, `recommendation_real_user_traffic_gate_in_window`, `operator_reading`, `next_action` 을 `summary/json/note` artifact로 다시 publish하게 맞췄다. 동시에 [ops-observation-runbook.md](./ops-observation-runbook.md), [ops-baseline-runbook.md](./ops-baseline-runbook.md), [start.md](../start.md), [current-state.md](../current-state.md), [local-validation-docs-index.md](./local-validation-docs-index.md) 에 compact entrypoint를 연결했다.
- 이유: ops도 machine-readable full baseline과 human-readable daily handoff를 분리해야 recommendation/collect/policy/auth/frontend 와 같은 운영 패턴을 유지할 수 있다. raw stdout 경로만 넘기면 정보는 충분해도 daily triage 비용이 다시 커진다.
...
- 해결: observation suite가 이제 `decision_class`, `reopen_allowed`, `operator_reading` 을 summary/json에 같이 남기고, `tmp/recommendation-observation/latest-recommendation-observation-note.md` 사람용 note artifact도 publish하게 바꿨다. 동시에 [recommendation-observation-runbook.md](../recommendation/recommendation-observation-runbook.md) 를 추가해 daily operator entrypoint와 상태 해석 ladder를 한 장으로 고정했다.
...
- 해결: [run-local-wrapper-duration-baseline.sh](../../deploy/performance/run-local-wrapper-duration-baseline.sh) 가 `recommendation_observation` 과 `current_priority` 자식 실행에 각각 전용 `ARTIFACT_DIR` 를 강제로 넘기도록 바꿨다. 이제 nested smoke cleanup은 자기 하위 디렉터리만 지우고, 부모 wrapper summary/json/tsv 는 그대로 남는다.
...
- 해결: [performance-docs-index.md](../performance/performance-docs-index.md) 와 [docs/performance/README.md](../performance/README.md) 를 추가하고, active 문서 진입점 네 곳에 모두 연결했다.
...
- 해결: [openai-runtime-contract.md](./openai-runtime-contract.md) 를 추가해 recommendation/chat/semantic retrieval/embedding rebuild 경로를 같은 형식으로 정리하고, [system-docs-index.md](./system-docs-index.md), [current-state.md](../current-state.md), [chatbot-plan.md](./chatbot-plan.md) 에 연결했다.
...
- 해결: [SensitiveTextRedactor.java](../../backend/src/main/java/com/example/welfare/global/util/SensitiveTextRedactor.java) 의 alias 범위를 넓히고, 전용 [SensitiveTextRedactorTest.java](../../backend/src/test/java/com/example/welfare/global/util/SensitiveTextRedactorTest.java) 를 추가해 한글/영문 라벨형 corpus를 직접 고정했다. 같은 라운드에서 `ChatAiGatewayTest` 와 `RealtimeAiGatewayTest` 에 request-body / prompt guard도 더해 redactor와 prompt 계약을 따로 읽지 않게 맞췄다.
...
- 문제: 추천용 [RealtimeAiGateway](../../backend/src/main/java/com/example/welfare/recommend/gateway/RealtimeAiGateway.java) 는 `chat/completions` 호출을 `block()` timeout 없이 기다리고 있었고, chat/embedding은 `block(Duration)` 을 쓰는 등 경계가 제각각이었다. 또 [OpenAiChatEmbeddingGateway](../../backend/src/main/java/com/example/welfare/chat/gateway/OpenAiChatEmbeddingGateway.java) 는 OpenAI 실패 시 local fallback vector를 돌려주는데, [PolicyChunkEmbeddingService](../../backend/src/main/java/com/example/welfare/chat/service/PolicyChunkEmbeddingService.java) 가 그 값을 그대로 DB에 저장하면 운영 vector index가 “실제 OpenAI 임베딩”처럼 오염된다. 여기에 `application.yml` 의 `openai.api-key` 는 필수처럼 잡혀 있고 gateway는 blank-key fallback을 기대하고 있어 설정 계약도 어긋나 있었다.
...
## 1018) recommendation prompt는 deferred product lane이므로 바꾸지 말고 계약만 테스트로 고정해야 했다
- 문제: OpenAI 경계를 다시 점검하면서 recommendation prompt에 “local 청년 정책을 더 밀자” 같은 목적 문장을 바로 넣고 싶어질 수 있었다. 하지만 active 문서 기준 recommendation current truth는 여전히 `KEEP_OBSERVING / WAIT_FOR_REAL_USER_TRAFFIC` 이고, `INCOME_MISMATCH` / `STUDENT_AUDIENCE_MISMATCH` 같은 primary audience exclusion도 현재는 유지가 맞는 product decision이다.
- 해결: [recommendation-current-state.md](../recommendation/recommendation-current-state.md), [recommendation-primary-audience-exclusion-decision-memo.md](../recommendation/recommendation-primary-audience-exclusion-decision-memo.md) 기준으로 recommendation prompt는 직접 수정하지 않고, `RealtimeAiGatewayTest` 에서 categorical-only user prompt와 stable system prompt 계약만 고정했다. 반대로 active lane인 chat 쪽만 `모르면 모른다고 답`, `지급/자격 단정 금지`, `확인 필요 조건 명시` 원칙을 프롬프트에 더 분명히 반영했다.
- 이유: deferred product lane을 코드 수준에서 먼저 바꾸면 current-state 문서와 운영 truth가 어긋난다. 지금 필요한 것은 recommendation behavior retune이 아니라 “현재 계약을 흔들지 말 것”을 테스트로 박는 일이다.

## 1019) 자유서술 PII 최소화는 direct identifier 패턴만으로는 부족했고 label-based redaction을 공통 helper로 넓혀야 했다
- 문제: 기존 `SensitiveTextRedactor` 는 이메일/휴대폰/생년월일/주민번호/계좌번호만 가렸고, `이름 김민수`, `주소 인천광역시 ...`, `학교명 인천대학교`, `회사명 청년컴퍼니` 같이 사용자가 자기소개식으로 적는 자유서술 PII는 그대로 남았다. chat prompt와 semantic query는 이 redactor를 공통으로 타므로, 범위가 좁으면 두 경로가 같이 새어 나간다.
- 해결: redactor에 label-based `name/address/organization` 패턴과 공통 `redactLabeledValue(...)` helper를 추가하고, `ChatAiGatewayTest` / `ChatSemanticSearchServiceTest` 에서 prompt와 semantic query 양쪽 모두 `[REDACTED_NAME]`, `[REDACTED_ADDRESS]`, `[REDACTED_ORG]` 로 바뀌는지 고정했다.
- 이유: 이름/주소/학교/회사명은 이메일처럼 완벽한 generic regex가 어렵다. 그래서 false positive를 줄이면서도 재사용성을 유지하려면, “라벨이 붙은 자기소개형 값”만 공통 helper로 걷어내는 쪽이 가장 일관적이다.
...
- 해결: [smoke-common.sh](../../deploy/smoke/smoke-common.sh) 에 `smoke_resolve_env_file()` 를 추가해 `ENV_FILE` 이 상대경로면 먼저 현재 경로를 보고, 없으면 repo root 기준으로 절대경로로 승격하도록 맞췄다. 이후 `smoke_resolve_admin_credentials()`, `smoke_resolve_db_connection_url()`, `smoke_db_query()`, `smoke_db_apply_file()` 가 모두 같은 정규화 경로를 쓰게 해서 `frontend`, `deploy/smoke`, 서버 shell 어느 cwd에서 실행해도 같은 env file을 읽는다. 관련 진입 문서 [start.md](../start.md), [current-state.md](../current-state.md) 에도 current priority / observation wrapper 실행 예시를 같은 기준으로 맞췄다.

## 1014) Gov24 detail self-heal fact는 새 code set을 만들지 말고 기존 `GOV24_SUPPORT_CONDITION` key를 재사용해야 FK rollback을 피한다
- 문제: `POST /api/admin/collect/gov24-details?sourceId=...` 를 복지로와 같은 bounded self-heal lane으로 넓히는 과정에서, detail 문구의 age range를 `service_facts` 로 다시 넣도록 만들었더니 integration이 `UnexpectedRollbackException` 으로 끊겼다. 실제 root cause는 트랜잭션 자체가 아니라 `service_facts.fact_code_set_key -> normalization_code_sets.code_set_key` FK 였다. 새 detail-derived fact에 임의 `GOV24_DETAIL` code set key를 넣은 탓에 insert/update가 `fk_sfact_code_set` 에서 막히고, `Gov24DetailCollectService` 는 예외를 잡아 `failed=1` 로 돌려도 transaction은 rollback-only가 되어 commit 시점에 터졌다.
- 해결: Gov24 detail의 age fact는 새 code set을 만들지 않고 기존 `GOV24_SUPPORT_CONDITION` key를 재사용하도록 맞췄다. 동시에 [WelfareServiceMapper.java](../../backend/src/main/java/com/example/welfare/collect/mapper/WelfareServiceMapper.java) 는 `supportTarget/selectionCriteria/supportContent` 를 다시 요약해 `만 39세 이하 청년` 같은 max-only youth 문구면 `18~39` range fact로 보정한다. 그 결과 [NormalizedPolicySidecarPersistenceIntegrationTest.java](../../backend/src/test/java/com/example/welfare/integration/NormalizedPolicySidecarPersistenceIntegrationTest.java) 기준 `gov24-details?sourceId=<서비스ID>` 경로가 뒤집힌 `min_age/max_age=40/39` row를 실제로 `18/39` 로 self-heal한다. source별 bounded repair 해석도 같이 고정했다: `복지로 -> bokjiro-details-refresh?sourceId`, `Gov24 -> gov24-details?sourceId`, `YOUTH -> sourceId 단건 repair lane 없음`.
...
- 해결: current target user `bc3fa9952df14f188fa5bf03e05e77c0` 는 `age=25`, `income=5`, `household_type=1인 가구`, `employment_status=미취업`, `INTEREST_FIELD=교육` 만 있고 `TARGET_TYPE` attribute가 비어 있다. 따라서 현재 rule scoring 계약에서는 이 정책을 일반 청년 사용자와 special-target mismatch로 읽는 것이 맞다. 같은 해석이 다시 흔들리지 않게 [RuleScoringServiceTest.java](../../backend/src/test/java/com/example/welfare/recommend/service/RuleScoringServiceTest.java) 에 장애 target group mismatch 회귀 테스트를 추가했고, saved-batch-gap runbook에도 이 해석을 explicit note로 남겼다.
...
- 해결: 현재 단계에서는 `POST /api/admin/collect/bokjiro-details-refresh?sourceId=WLF00004717` 로 `3700` self-heal path를 실제로 검증했고, 같은 contract를 [BokjiroSidecarMergeIntegrationTest.java](../../backend/src/test/java/com/example/welfare/integration/BokjiroSidecarMergeIntegrationTest.java) 에 회귀 테스트로 고정했다. 추가로 local sweep 결과를 남겨 “코드 fix 완료”와 “legacy row repair/backfill 필요성”을 분리해 읽게 했다.
...
- 해결: current local baseline에서는 `ADMIN_EMAIL='<local admin email>'`, `ADMIN_PASSWORD='<local admin password>'` 를 명시해 다시 실행한다. 재실행 결과 current auto family `3516,3689,2738,2855,3605` 중 `3689/3605/2738/3516` 는 saved batch에 남고, `2855` 만 `FILTERED_BY_SPECIAL_TARGET_MISMATCH` 에서 빠지는 것이 현재 blocker로 확인됐다.
...
- 해결: [smoke-common.sh](../../deploy/smoke/smoke-common.sh) 의 auto family resolver에 `ws.min_age/max_age`, `ws.min_income/max_income` predicate를 current user context와 같은 축으로 추가했다. 재검증 결과 latest user `bc3fa9952df14f188fa5bf03e05e77c0(age=25, income=5)` 기준 auto family는 `3700` 이 빠진 `3516,3689,2738,2855,3605` 로 바뀌었고, `signal-gap`, `region-window` wrapper도 같은 기준으로 다시 통과했다.

## 1008) 복지로 age 요약은 full-width range와 참조 사업 연령대가 같이 나오면 `35/34` 같은 불가능한 bounds를 만들 수 있다
- 문제: `인천형 청년월세 지원사업(3700)` local row를 보니 `min_age=35`, `max_age=34` 로 저장돼 있었다. 원문은 `경제적으로 어려움을 겪고 있는 인천 청년(35~39세)... 국가사업(...19~34세)...` 와 detail `35세～39세이하` 조합인데, [TextConstraintExtractor.java](../../backend/src/main/java/com/example/welfare/collect/validation/TextConstraintExtractor.java) 가 full-width `～` range를 읽지 못하고 여러 age token을 전역 min/max로만 접으면 primary range와 참조 range가 뒤섞여 불가능한 조합이 나온다. 게다가 기존 [WelfareService.java](../../backend/src/main/java/com/example/welfare/policy/entity/WelfareService.java) `applyDetailFallbacks()` 는 age 값이 이미 non-null이면 detail이 sane한 bound를 줘도 복구하지 못했다.
...
- 해결: local closeout rerun에서는 [run-local-admin-forced-logout-smoke.sh](../../deploy/smoke/run-local-admin-forced-logout-smoke.sh) 의 현재 기준선인 `ADMIN_EMAIL='<local admin email>'`, `ADMIN_PASSWORD='<local admin password>'` 를 명시해 wrapper를 끝까지 다시 태운다. 현재 결과는 `old access 401/A006`, `old refresh 401/A003`, `relogin 200` 이고, auth/session wrapper 전체도 다시 통과했다.

## 1006) education replay smoke는 compose-host JDBC를 조용히 흡수하지 않고 local-host URL을 명시할 때만 다시 태우는 편이 맞다
- 문제: [run-local-education-priority-replay.sh](../../deploy/smoke/run-local-education-priority-replay.sh) 는 현재 `.env` 의 `jdbc:postgresql://db:...` 같은 compose-host URL을 만나면 `DB_URL must target local PostgreSQL host, not docker-compose service host` 로 즉시 멈춘다. 이건 코드 회귀가 아니라, compose 내부 host를 active local replay truth처럼 쓰지 않게 막는 preflight다.
...
- 문제: [PoliciesPage.jsx](../../frontend/src/pages/PoliciesPage.jsx) 는 이미 카테고리/지역/소득 같은 active filter chip은 보여 주고 있었지만, 정작 가장 강한 조건인 `검색어` 와 결과 순서를 바꾼 `정렬` 은 summary 영역에 없었다. 이 상태에서는 결과가 비거나 너무 많을 때 “현재 어떤 조합이 적용 중인가”를 사용자가 위아래로 다시 훑어야 했다.
...
- 문제: [PolicyDetailPage.jsx](../../frontend/src/pages/PolicyDetailPage.jsx) 의 기존 `비슷한 정책` 섹션은 `unifiedCategory` 하나로만 후보를 읽었다. 카테고리 데이터가 성긴 정책이나 같은 분류 내 활성 정책 수가 적은 경우에는 rail이 비거나, 실제로는 같은 출처/같은 지역으로 이어서 볼 만한 정책이 있는데도 못 보여 주는 상태가 생긴다.
...
- 해결: [MyPage.jsx](../../frontend/src/pages/MyPage.jsx) 에 recent-view를 `기본 5건 요약 + 더보기/접기`, `전체 / 진행중만` 토글, empty CTA 구조로 정리했다. API는 여전히 최대 30건을 한 번에 읽되, 화면은 기본적으로 짧게 유지하고 필요할 때만 펼치는 쪽으로 UX 비용을 낮춘다.
...
- 해결: [PolicySearchKeywordReadService.java](../../backend/src/main/java/com/example/welfare/policy/service/PolicySearchKeywordReadService.java) 는 `search_logs` 와 `searchChatCandidates()` 결과를 한 번에 모은 뒤, `exact/prefix/contains` match priority를 먼저 보고 같은 등급 안에서만 source 우선순위와 원래 order를 tie-break로 쓰게 바꿨다. 따라서 더 정확히 맞는 정책 title은 상단으로 올리되, 같은 match tier에서는 기존 로그 ranking 의미를 계속 유지한다.
...
- 해결: [PoliciesPage.jsx](../../frontend/src/pages/PoliciesPage.jsx) 에 검색창 `지우기` 액션, suggestion이 비었을 때 현재 입력어로 바로 검색하는 fallback row, 검색/필터 active 여부에 따라 달라지는 empty state CTA를 추가했다. 동시에 viewport width 기준으로 `sidebar stack`, `검색 버튼 full width`, `2열 카드 -> 1열` 전환을 넣어 기존 UI 패턴을 유지한 채 모바일에서도 무리 없이 읽히게 정리했다.
...
- 해결: [UserRecentViewedPolicyReadService.java](../../backend/src/main/java/com/example/welfare/user/service/UserRecentViewedPolicyReadService.java) 는 recent row가 비면 바로 `List.of()` 로 반환해 추가 presentation read를 건너뛴다. [PolicyViewLogService.java](../../backend/src/main/java/com/example/welfare/policy/service/PolicyViewLogService.java) 의 `recent_policy_views` upsert는 계속 `try/catch` 로 감싸 두고, 테스트도 “upsert 실패해도 상세 조회 로그 저장은 계속 진행”, “비활성 정책도 기존 북마크 패턴처럼 recent-view에 포함”까지 같이 고정했다.
...
- 해결: [PoliciesPage.jsx](../../frontend/src/pages/PoliciesPage.jsx) 에서 검색 state를 `draftSearch` 와 `appliedSearch` 로 분리했다. `draftSearch` 는 input/suggestions 전용, `appliedSearch` 는 URL sync와 실제 `/api/policies` `/api/policies/search` 조회 전용으로 쓰고, 엔터/검색 버튼/추천어 선택 시점에만 `appliedSearch` 를 갱신한다. 즉 타이핑 중에는 suggestion만 움직이고, 실제 목록은 명시적 submit 뒤에만 바뀌게 한다.
...
- 해결: 이번 단계에서는 검색엔진을 새로 들이지 않고 기존 [WelfareServiceSearchRepositoryImpl.java](../../backend/src/main/java/com/example/welfare/policy/repository/WelfareServiceSearchRepositoryImpl.java) 의 `searchChatCandidates()` read 경계를 재사용해, suggestion 응답을 `정책 title 후보 우선 + search_logs 후보 보강` 순서로 합쳤다. `LinkedHashSet` 으로 dedupe한 뒤 limit을 적용해 API shape는 그대로 유지하면서도 실제 정책명 회수율을 먼저 올린다.

## 994) 검색과 자동완성의 입력 정규화가 다르면 사용자는 “검색은 되는데 자동완성은 안 뜬다”고 느끼게 된다
- 문제: 본 검색은 [SearchKeywordSupport.java](../../backend/src/main/java/com/example/welfare/global/util/SearchKeywordSupport.java) 로 구두점/특수문자를 걷어내고 토큰화하지만, 자동완성이 별도 규칙으로 입력과 로그를 비교하면 `월세!`, `청년-지원` 같은 입력에서 검색 결과와 suggestion 결과가 서로 다른 뜻으로 동작한다.
...
- 해결: [PolicyTrafficRateLimitService.java](../../backend/src/main/java/com/example/welfare/policy/service/PolicyTrafficRateLimitService.java) 에 `search / suggestion / trending / detail` 전용 bucket을 분리했다. 현재 기본값은 `search=60/60s`, `suggestion=180/60s`, `trending=30/60s` 이고, [PolicyController.java](../../backend/src/main/java/com/example/welfare/policy/controller/PolicyController.java) 는 endpoint별로 각기 다른 checker를 탄다. 즉 permit-all 공개 계약은 유지하되, 타이핑 트래픽이 본 검색 한도를 잠식하지 않게 경계를 나눈다.
...
- 해결: [deploy/nginx/youth-welfare.conf](../../deploy/nginx/youth-welfare.conf) 와 [deploy/nginx/youth-welfare.bootstrap.conf](../../deploy/nginx/youth-welfare.bootstrap.conf) 의 server block에 `server_tokens off;` 를 추가했다. 다만 현재 로컬 작업 환경에는 host nginx가 없어서 템플릿 수정까지만 가능하고, 실제 반영 확인은 운영 서버에서 `sudo nginx -t`, `sudo systemctl reload nginx`, `curl -I https://youthmoa.kr/` 로 따로 닫아야 한다.

## 989) 컨테이너를 non-root + read-only로 바꿀 때는 Java/Redis가 실제로 쓰는 임시 경로를 먼저 열어 줘야 런타임이 안 깨진다
- 문제: [backend/Dockerfile](../../backend/Dockerfile) 을 non-root로 바꾸고 [docker-compose.prod.yml](../../docker-compose.prod.yml) 에 `read_only: true` 만 추가하면, Java는 기본 temp dir 사용 시 `/tmp` 쓰기가 필요하고 Redis도 런타임에 `/data` 와 `/tmp` 경로가 필요하다. 이걸 같이 안 열면 hardening 자체는 들어가도 컨테이너가 부팅 중 바로 실패할 수 있다.
- 해결: app는 Dockerfile entrypoint에 `-Djava.io.tmpdir=/tmp` 를 강제하고 compose에서 `/tmp` tmpfs를 열었다. redis는 현재 운영이 persistence를 기대하지 않는 구조라 `read_only: true` 를 유지하되 `/data`, `/tmp` 를 tmpfs로 열고 `no-new-privileges`, `pids_limit`, `mem_limit` 을 같이 붙였다. 즉 권한만 낮추는 게 아니라 “필요한 최소 쓰기 경로만 tmpfs로 허용”하는 방식으로 맞춘다.

## 988) 로그인 계정 잠금만 있어도 password-reset/login flood는 별도 fingerprint 축을 안 넣으면 같은 클라이언트에서 계속 두드릴 수 있다
- 문제: [AuthController](../../backend/src/main/java/com/example/welfare/user/controller/AuthController.java) 는 이미 이메일 중복 확인, 이메일 인증 발송에는 fingerprint rate limit을 태우고 있었지만 `/api/auth/login`, `/api/auth/password-reset/request` 는 그대로 service로 내려보냈다. 계정 잠금과 이메일별 cooldown은 있어도, 같은 IP/UA fingerprint에서 여러 계정으로 로그인 시도하거나 여러 이메일로 reset request를 흩뿌리는 행위는 별도 클라이언트 축에서 막지 못했다.
- 해결: [AuthRateLimitService](../../backend/src/main/java/com/example/welfare/user/service/AuthRateLimitService.java) 에 `auth:rate-limit:login:*`, `auth:rate-limit:password-reset-request:*` 키를 추가하고, [AuthController](../../backend/src/main/java/com/example/welfare/user/controller/AuthController.java) 가 `ClientFingerprintService.build(request)` 후 이 제한을 먼저 검사하게 바꿨다. 기본값은 `login <= 20 / 300초`, `password-reset-request <= 5 / 300초` 로 두어 계정 잠금/이메일 cooldown은 그대로 유지하면서 클라이언트 축 방어만 보강한다.

## 987) admin 작업 상한은 “너무 큰 값 거부”만 볼 게 아니라 default 자체도 상한 안에 있는지 같이 맞춰야 한다
- 문제: [CollectAdminController](../../backend/src/main/java/com/example/welfare/collect/controller/CollectAdminController.java) 의 `bokjiro-details-gap-fill` 는 원래 `maxCallsPerRound` default가 `10000` 이었다. 여기서 상한을 `1000` 으로만 추가하면, 파라미터를 생략한 정상 호출도 default 때문에 바로 `400` 이 되어 버린다.
- 해결: `rounds/maxCallsPerRound/maxCallsPerRun/limitPerSource` 상한을 명시하면서, `bokjiro-details-gap-fill` 의 default `maxCallsPerRound` 도 `1000` 으로 같이 낮췄다. 또 `limitPerSource=0` 은 “사실상 무제한”로 두지 않고 capped default `1000` 으로 정규화해, 파라미터를 안 넣어도 안전한 기본 실행이 되게 맞춘다.

## 986) recommendation list size는 controller에서 막아도 내부 read path에 clamp를 한 번 더 두는 편이 안전하다
- 문제: [RecommendationController](../../backend/src/main/java/com/example/welfare/recommend/controller/RecommendationController.java) 의 `size` 만 검증하면 외부 HTTP 요청은 막을 수 있지만, 나중에 다른 service/scheduler/controller caller가 생기면 [RecommendationResultReadRepositoryImpl](../../backend/src/main/java/com/example/welfare/recommend/repository/RecommendationResultReadRepositoryImpl.java) 의 `PageRequest.of(0, size)` 에 raw 값이 다시 내려갈 수 있다.
- 해결: controller에는 `@Min(1) @Max(100)` 를 걸어 oversized 요청을 `400 INVALID_INPUT(C001)` 으로 거부하고, [RecommendationAccessService](../../backend/src/main/java/com/example/welfare/recommend/service/RecommendationAccessService.java) 에도 `normalizeSize()` 를 추가해 최종적으로 `1..100` 범위를 다시 clamp 한다. 즉 external contract와 internal safety net을 둘 다 둔다.

## 985) Web Push SSRF 경계는 “등록 시 검증”만으로 닫히지 않고, 기존 저장 subscription과 테스트 dispatch 권한까지 같이 좁혀야 한다
- 문제: [NotificationController](../../backend/src/main/java/com/example/welfare/notification/controller/NotificationController.java) 의 `/push-test-send`, `/digest-test-dispatch`, `/deadline-test-dispatch` 는 일반 로그인 사용자에게 열려 있었고, [WebPushSubscriptionRequest](../../backend/src/main/java/com/example/welfare/notification/dto/WebPushSubscriptionRequest.java) 는 endpoint 길이만 봤다. [WebPushSenderClientImpl](../../backend/src/main/java/com/example/welfare/notification/service/WebPushSenderClientImpl.java) 는 저장된 endpoint로 실제 outbound 요청을 보내므로, 등록 시점만 막아도 이미 DB에 있던 unsafe endpoint는 계속 송신 시도 대상이 된다.
- 해결: 새 [WebPushEndpointPolicyService](../../backend/src/main/java/com/example/welfare/notification/service/WebPushEndpointPolicyService.java) 를 추가해 `https` 강제, push provider host allowlist, DNS resolve 결과 private/loopback/link-local/multicast 차단을 한 곳에 모았다. 이 정책을 `WebPushSubscriptionCommandService.register()` 와 `WebPushSenderClientImpl.send()` 양쪽에 같이 적용해, 신규 등록도 막고 기존 저장 subscription도 송신 직전에 disable 할 수 있게 한다. 테스트 dispatch 3개는 [SecurityConfig](../../backend/src/main/java/com/example/welfare/global/config/SecurityConfig.java) 에서 `ROLE_ADMIN` 으로만 열어 abuse surface를 줄이고, sender/dispatch warn 로그도 endpoint 전체 대신 host만 남기게 줄인다.
...
- 해결: [deploy/nginx/youth-welfare.conf](../../deploy/nginx/youth-welfare.conf) 와 [deploy/nginx/youth-welfare.bootstrap.conf](../../deploy/nginx/youth-welfare.bootstrap.conf) 에 baseline CSP를 추가하는 동시에, `/api/`, `/actuator/`, `/swagger-ui/`, `/v3/api-docs/` 프록시 경로에 `proxy_hide_header Content-Security-Policy;` 도 같이 넣었다. 이렇게 해야 CSP도 HSTS/XFO/XCTO와 같은 “edge 단일 책임” 패턴으로 유지된다.

## 983) body validation만 400으로 내려도 request parameter 누락/타입 오류는 generic 500으로 새어 나갈 수 있다
- 문제: [GlobalExceptionHandler](../../backend/src/main/java/com/example/welfare/global/exception/GlobalExceptionHandler.java) 는 `MethodArgumentNotValidException`, `ConstraintViolationException`, `HttpMessageNotReadableException` 만 직접 잡고 있었다. 그래서 `/api/auth/check-email` 처럼 `@RequestParam` 을 쓰는 경로에 `email` 없이 들어오면 `MissingServletRequestParameterException` 이 generic handler로 흘러 500처럼 보일 수 있었다.
...
- 문제: revoke 동작은 이미 정확했지만, [AccessTokenRevocationService](../../backend/src/main/java/com/example/welfare/user/service/AccessTokenRevocationService.java) 가 `access-revoked:{JWT원문}` 형식 key를 쓰고 있었다. Redis가 내부 저장소라 해도, key만 봐도 access token 원문이 드러나는 건 불필요한 노출이다.
...
- 해결: 새 [NotificationUnsubscribeTokenService](../../backend/src/main/java/com/example/welfare/notification/service/NotificationUnsubscribeTokenService.java) 가 32-byte random opaque token을 발급하고, Redis에는 `notification:unsubscribe:{sha256(token)}` key에 `userKey` 만 TTL과 함께 저장하게 바꿨다. mail body는 opaque token만 싣고, controller는 받은 token을 hash해서 lookup 후 1회성으로 delete한다. 이렇게 하면 query에 값이 남더라도 JWT claims 같은 자체 정보가 없고, 서버 저장소에도 원문 token은 남지 않는다.

## 981) unsubscribe migration 과도기가 끝나면 JWT fallback은 다시 제거해야 한다
- 문제: 새로 발송되는 메일을 opaque token으로 바꾸는 초기 단계에서는 기존 `/api/notifications/unsubscribe?token={JWT}` 링크 호환이 운영 리스크를 줄였다. 하지만 과도기가 끝난 뒤에도 fallback을 유지하면 공개 unsubscribe endpoint가 저장소에 없는 “아무 유효한 JWT”를 수신거부 토큰처럼 해석할 수 있어 토큰 용도 경계가 흐려진다.
- 해결: 현재 기준 [NotificationController](../../backend/src/main/java/com/example/welfare/notification/controller/NotificationController.java) 는 `NotificationUnsubscribeTokenService.consumeUserKey(token)` 로 Redis에 해시 저장된 opaque token만 소비한다. 저장소에 없는 값은 JWT 검증을 시도하지 않고 `INVALID_TOKEN(A001)` 로 거부한다. 새 메일 링크는 `/notifications/unsubscribe#token=...` fragment를 쓰고, 프론트는 query/hash token을 읽은 즉시 주소창에서 제거한다.
...
- 해결: `web-push` 를 `5.1.2` 로 올리는 것만으로 끝내지 않고, [build.gradle](../../backend/build.gradle) 에 dependency constraint를 추가해 `async-http-client >= 2.15.0`, `jose4j >= 0.9.6` 을 명시적으로 고정한다. 이 프로젝트는 이미 `tomcat.version` override를 쓰고 있으므로, 보안 취약 전이 의존성도 같은 식으로 “실제 해석 버전 고정 + dependencyInsight 재검증” 흐름으로 닫는 편이 맞다.
...
- 문제: `gradle.properties`의 `org.gradle.java.home=/home/ubuntu/youth-welfare/.jdk/...` 설정이 컨테이너 안에서는 존재하지 않아 `Java home supplied is invalid` 오류가 발생했음
...
- 해결: `backend/src/main/resources/db/migration-draft/V2026_04_30_02__seed_policy_normalization_codes.sql` 을 `INSERT INTO ... WITH ... SELECT ...` 순서로 수정했고, 수정 후 `service_taxonomies=2363`, `education_target_rows=110` 기준으로 실제 적용을 다시 확인했다
...
- 해결: [policy-source-code-entrypoints.md](../policy/policy-source-code-entrypoints.md) 를 추가해 [CollectAdminController.java](../../backend/src/main/java/com/example/welfare/collect/controller/CollectAdminController.java), [CollectSourceExecutionService.java](../../backend/src/main/java/com/example/welfare/collect/service/CollectSourceExecutionService.java), [RawApiPayloadService.java](../../backend/src/main/java/com/example/welfare/collect/service/RawApiPayloadService.java), [CollectItemSaver.java](../../backend/src/main/java/com/example/welfare/collect/service/CollectItemSaver.java), [DeferredNormalizedPolicySidecarWriter.java](../../backend/src/main/java/com/example/welfare/collect/normalization/DeferredNormalizedPolicySidecarWriter.java), [CanonicalRecommendationReadModelRepository.java](../../backend/src/main/java/com/example/welfare/recommend/repository/CanonicalRecommendationReadModelRepository.java) 순서로 코드 진입점을 묶었다
...
- 해결: [run-local-education-priority-replay.sh](../../deploy/smoke/run-local-education-priority-replay.sh)에 `REPLAY_SUMMARY_APPEND_FILE`, `REPLAY_SUMMARY_TS` env contract를 추가하고, 당시 `openai replay diagnostic lane plan` 문서와 [policy-normalization-education-priority-replay-procedure.md](../history/ai/policy-normalization-education-priority-replay-procedure.md)에 같은 이름으로 고정했다
...
- 해결: [cleanup-openai-replay-artifacts.sh](../../deploy/smoke/cleanup-openai-replay-artifacts.sh) 를 추가하고, 당시 `openai replay diagnostic lane plan` 문서에 `REPLAY_LOG_ROOT`, `SUMMARY_RETENTION_DAYS`, `ARTIFACT_RETENTION_DAYS`, `DRY_RUN` 계약과 실행 예시를 같이 고정했다
...
- 해결: `ApiSyncLog.status` 는 [ApiSyncLogStatusConverter](../../backend/src/main/java/com/example/welfare/collect/entity/converter/ApiSyncLogStatusConverter.java) 로 소문자 DB 값과 대문자 도메인 enum을 분리해서 읽고 쓰게 바꿨다. 동시에 `backend/src/main/resources/db/migration/V2026_04_23_01__add_api_sync_logs.sql` 과 [schema.sql](../../backend/src/main/resources/db/schema.sql) 의 `api_sync_logs.status` 타입을 `VARCHAR(30)` 으로 정리하고, 로컬 DB에도 같은 `ALTER TABLE api_sync_logs MODIFY COLUMN status VARCHAR(30)` 를 적용했다. 이후 앱 재기동과 `POST /api/admin/collect/youth` 재실행에서 최신 `api_sync_logs` row가 `success(requested=2363, saved=2363, failed=0)` 로 정상 종료되는 것을 확인했다.
...
## 703) `run-local-pii-sync-cutover-smoke` 는 secondary datasource 둘 다 PostgreSQL 기준으로 정규화하고, queue table이 이미 있으면 legacy MySQL migration replay를 건너뛰어야 한다
...
## 734) Gov24 baseline 숫자는 외부 source row 증가와 manual follow-up 적용 뒤 함께 갱신해야 한다
...
## 736) CTR readiness만으로 추천 품질을 열면, “표본은 있다”와 “추천이 덜 몰린다”를 다시 섞어 읽게 된다
...
## 745) `SignupPage` 의 “이미 계정이 있으신가요? 로그인” 버튼도 같은 auth 복귀 문맥을 실어야, 가입 화면에서 로그인으로 갈 때 챗/북마크 intent가 끊기지 않는다
...
## 747) post-login bookmark action은 “대상 row가 아직 안 보인다”와 “같은 action을 다시 읽는다”를 분리해서 다뤄야, 목록 0회/상세 2회 토글을 함께 막을 수 있다
- 문제: 이후 production preview 브라우저 재검증에서도 목록 북마크는 여전히 `POST /bookmark 0회`, 상세 북마크는 `POST /bookmark 2회` 였다. 원인을 더 좁혀 보니 목록 쪽은 action 자체가 틀린 게 아니라, 현재 결과 목록에 target policy가 보이지 않는 순간에도 `PoliciesPage` 가 action을 포기해 버리는 구조였다. 상세 쪽은 ref guard를 넣었더라도 `finally` 에서 ref를 먼저 비워 버리면, `clearPostLoginAction()` 로 state가 지워지기 전에 같은 effect가 한 번 더 돌 수 있었다. Header/FloatingNav도 챗/마이페이지 이동 target을 계산할 때 여전히 `pathname/search` 위주로 만들고 있어서, 서버 브라우저 검증에서 `/chat`, `/mypage` 최종 state가 `null` 로 남았다.
- 해결: `PoliciesPage` 는 더 이상 target row가 현재 page에 안 보인다고 해서 action을 바로 포기하지 않고, bookmark POST는 먼저 시도하되 화면 row가 있으면 그때만 local state를 토글하게 바꿨다. `PoliciesPage`, `PolicyDetailPage` 둘 다 `bookmarkActionKeyRef` 는 `postLoginAction` 이 완전히 사라질 때만 초기화하고, 실행 `finally` 에서는 즉시 null로 되돌리지 않게 바꿨다. `Header`, `FloatingNav` 는 `/chat`, `/mypage` target을 만들 때 현재 `location` 을 `from` 으로 함께 싣고, 챗은 `chatFrom` self target도 같이 구성하도록 맞췄다. 이후 `frontend lint/build` 를 다시 통과시켰다.
- 이유: 브라우저 체감 기준으로 보면 “로그인 후 북마크가 안 됨”과 “두 번 토글됨”은 서로 다른 타이밍 문제다. 하나는 visible row 의존성, 다른 하나는 one-shot action lifetime 문제이므로 같은 패턴으로 뭉뚱그리면 계속 한쪽만 고쳐진다. 네비게이션 state도 같은 라운드에서 같이 맞춰야 서버 브라우저 검증과 로컬 코드 계약이 다시 붙는다.

## 748) query canonicalization 용 `setSearchParams(..., { replace: true })` 도 `state` 를 같이 싣지 않으면, 로그인 복귀 직후의 one-shot intent가 mount 단계에서 증발할 수 있다
- 문제: 목록 북마크 post-login action이 여전히 `POST /bookmark 0회` 인 이유를 더 좁혀 보니, `PoliciesPage` 는 mount 직후 query 순서를 canonicalize 하면서 `setSearchParams(params, { replace: true })` 를 호출하고 있었다. React Router의 replace navigation은 이때 `location.state` 를 자동 보존하지 않으므로, 로그인에서 방금 넘긴 `postLoginAction` 이 fetch/effect가 실제로 처리되기 전에 같은 URL의 새 history entry로 바뀌며 사라질 수 있었다. 같은 패턴은 `ChatPage` 의 `?session=` sync, `MyPage` 의 `?tab=` sync에도 있었다.
- 해결: `PoliciesPage`, `ChatPage`, `MyPage` 의 `setSearchParams(..., { replace: true })` 호출에 모두 `state: location.state` 를 함께 넘기도록 바꿨다. 이제 query canonicalization / sync가 일어나도 현재 page state와 post-login intent가 유지된다. 이후 `frontend lint/build` 를 다시 통과시켰다.
- 이유: 현재 프론트는 URL query와 `location.state` 를 함께 계약으로 쓴다. query를 정리하는 helper가 state를 보존하지 않으면, 사용자는 같은 URL에 머무르는데도 auth intent만 사라지는 미묘한 브라우저 회귀가 생긴다.

## 749) 현재 알림 구조는 이메일 발송 이력과 사용자 읽음함을 같은 테이블로 보기 어렵기 때문에, `user_alerts` 를 별도로 두는 편이 현재 코드 경계와 맞다
...
## 780) `dispatch_key` 중복을 예외로 받는 save 기반 reservation은 scheduler loop 전체를 rollback 시킬 수 있으므로, conflict-safe insert 경계로 바꿔야 한다
...
## 783) 전용 알림함 페이지는 곧바로 서버 query를 더 늘리기보다, 클라이언트 필터로 unread/kind 구분부터 주는 편이 현재 범위에 맞다
...
## 789) direct admin smoke가 `run-local-validation-from-env.sh` 와 다른 admin credential fallback을 쓰면, 서버 allowlist truth와 무관한 로컬 기본값으로 잘못 로그인할 수 있다
- 문제: broader validation 서버 재검증에서 `admin forced logout` 이 `403 / C003` 로 실패했는데, 원인은 forced logout 자체보다 smoke login 계정과 서버 `SECURITY_ADMIN_EMAILS` allowlist 불일치 가능성이 컸다. 조사 결과 `run-local-validation-from-env.sh` 는 `.env` -> `/tmp` -> `SECURITY_ADMIN_EMAILS` 우선순위를 읽고 있었지만, direct admin smoke인 `run-local-admin-forced-logout-smoke.sh`, `run-local-admin-dashboard-smoke.sh`, `run-local-policy-quality-summary.sh` 는 여전히 `admin@example.com/password123!` 로 직접 fallback 하고 있었다.
- 해결: 공통 `smoke_resolve_admin_credentials()` helper를 `smoke-common.sh` 에 추가하고, 위 3개 admin smoke가 동일한 우선순위로 `.env`, `/tmp`, `SECURITY_ADMIN_EMAILS` 를 읽게 맞췄다. 동시에 비밀번호가 비어 있으면 로컬 기본값으로 진행하지 않고 즉시 실패하도록 바꿨다.
- 이유: 서버와 로컬의 `.env` 가 다를 때 문제는 "값이 다르다" 자체보다, 어떤 smoke는 서버 truth를 읽고 어떤 smoke는 로컬 기본값을 들고 있다는 점이었다. 이제 direct admin smoke도 wrapper와 같은 규칙을 따르므로 서버 검증 시 allowlist admin 이메일과 실제 비밀번호를 명시하면 같은 기준으로 동작한다.

## 790) Gov24 활용 품질을 보려면 수집량만 보면 안 되고, public search/list/recommendation 응답에 `sourceType` 이 실제로 노출되는지도 같이 봐야 한다
- 문제: Gov24 collect/runtime audit은 이미 닫혀 있었지만, 실제 사용자 흐름에서 Gov24가 얼마나 보이는지 확인하려고 search/list/recommendation 응답을 읽자 목록/검색 카드와 추천 응답에서 `sourceType` 계약이 들쭉날쭉했다. `PolicyDetailResponse` 는 `sourceType` 을 주고 있었지만 `PolicySummaryResponse`, `RecommendationResponse` 는 안 주고 있어서, 카드 UI와 surface audit에서는 Gov24가 섞여 있어도 출처를 안정적으로 읽을 수 없었다.
- 해결: `PolicySummaryResponse`, `RecommendationResponse` 에 `sourceType` 을 추가하고, `PoliciesPage`, `MainPage` 카드에도 출처 라벨을 노출했다. 동시에 `deploy/smoke/run-local-gov24-surface-audit.sh` 를 추가해 `keyword search`, `sourceType=GOV24 filtered search`, `policy list`, `Gov24 detail`, `fresh recommendation refresh` 의 source 분포를 한 번에 출력하게 했다.
- 이유: Gov24 품질의 다음 단계는 "row가 몇 개 있나"보다 "사용자 화면에서 실제로 보이느냐"다. 현재 baseline도 search top20에서 `GOV24 6/20`, list top20에서 `GOV24 19/20`, fresh recommendation top5에서 `GOV24 2/5` 라는 식으로 surface 분포를 봐야 의미가 있다.

## 791) Gov24 raw에 summary label 값이 있어도, 전용 list sidecar replay 경로가 없으면 예전 적재 row의 `service_taxonomy_summary_slots` drift는 계속 남는다
- 문제: server/local Gov24 surface를 다시 보니 일부 detail sample에서 `gov24ServiceFieldLabel/userType/benefitType` 이 `0` 으로 내려왔다. DB를 확인하면 Gov24 `10942`건 중 `2건` (`B55365500002`, `641000000853`) 만 `service_taxonomy_summary_slots` 의 `GOV24_SERVICE_FIELD/GOV24_USER_TYPE/GOV24_BENEFIT_TYPE` 3종이 모두 비어 있었고, 같은 서비스의 `raw_api_payloads(LIST)` 에는 각각 `보건·의료/개인/현금(감면)`, `생활안정/소상공인/현금` 값이 이미 들어 있었다.
- 해결: `CollectSourceRegistry.GOV24` 에 `toListAggregate(...)` 와 `supportsListAggregateBackfill=true` 를 연결하고, `NormalizedPolicySidecarBackfillService` 에 `backfillGov24ListSidecars()` 및 admin route `POST /api/admin/collect/gov24-sidecars-backfill?limitPerSource=0` 를 추가했다. 동시에 `backfillBokjiroListSidecars()` 는 다시 복지로 중앙/지자체만 태우도록 scope를 고정해 이름과 책임을 맞췄다. 새 smoke `deploy/smoke/run-local-gov24-sidecar-backfill-smoke.sh` 로 `before_missing=2 -> after_missing=0` 을 확인했다.
- 이유: 이건 raw 부재나 mapper 해석 실패가 아니라, "Gov24는 sidecar replay 경로 자체가 없었다"는 구조 문제였다. 그래서 소수 예전 row drift는 collect를 다시 하지 않는 한 영원히 남았고, 전용 backfill 경로를 열어야만 깔끔하게 닫힌다.

## 792) Gov24가 추천에서 약해 보여도, 실제 최신 batch를 rank별로 보면 "안 뜨는 것"과 "top1/2를 못 먹는 것"은 다르다
...
- 해결: 현재 local baseline인 `ADMIN_EMAIL='<local admin email>'`, `ADMIN_PASSWORD='<local admin password>'` 를 명시해 full wrapper를 다시 실행했다. 결과는 `local validation suite passed`, `suite_duration_seconds=75` 였고, 하위 단계도 `auth/session`, `public policy + profile/priorities + chat`, `bookmark consistency`, `recommendation click`, `admin dashboard`, `education priority replay` 전부 green 으로 끝났다. 같은 라운드에서 `run-local-notification-channel-smoke.sh`, 수정된 `run-local-deadline-reminder-smoke.sh`, `run-local-gov24-recommendation-suite.sh` 도 모두 PASS 했다.
- 이유: 이로써 알림/추천/Gov24 축 변경 이후 broader local baseline도 다시 회복됐다. 다음 broader 회귀는 admin credential만 명시하면 같은 묶음으로 재현 가능하다.

## 811) 반복되는 server drift는 migration/admin credential/date 세 축으로 분리해 한 문서에서 먼저 배제하는 편이 낫다
- 문제: 최근 서버 쪽에서 실제 기능 버그처럼 보였던 실패 상당수가 코드가 아니라 운영 drift였다. 대표적으로 기존 volume에서 `flyway_schema_history` 부재로 인한 수동 migration 필요, direct admin smoke의 credential 주입 오해, host 날짜와 app/DB 날짜 차이로 인한 deadline `NO_CANDIDATES` 오탐이 반복됐다. 이걸 troubleshooting log 흩어진 항목만 따라가면 매번 다시 찾게 된다.
- 해결: [server-runtime-drift-checklist.md](./server-runtime-drift-checklist.md) 를 새로 추가하고, 현재 active 서버 drift를 세 축으로 묶었다. 1) 기존 volume이면 `V2026_05_15_02`, `V2026_05_15_03`, `V2026_05_16_01`, `V2026_05_17_01` 같은 migration을 수동 적용해야 할 수 있다는 점, 2) admin smoke는 `ADMIN_EMAIL/ADMIN_PASSWORD -> /tmp -> SECURITY_ADMIN_EMAILS` 우선순위로 읽는다는 점, 3) deadline reminder smoke는 host shell이 아니라 DB `CURRENT_DATE` 기준으로 `apply_end_date` 를 강제해야 한다는 점을 한 페이지에 정리했다. local validation docs index와 runtime smoke 명령 문서에서도 이 체크리스트를 바로 참조하게 연결했다.
- 이유: 운영 drift를 먼저 배제해야 코드 버그와 인프라/volume 상태를 혼동하지 않는다. 다음 server/local smoke 실패부터는 기능 추적 전에 이 체크리스트를 먼저 보는 게 기준이다.

## 812) 정책 상세 요약 밀도 개선은 상단 요약 확장만으로 끝나지 않고, TDZ와 모바일 2열 레이아웃까지 같이 닫아야 브라우저 기준선이 선다
...
## 817) 운영 컨테이너가 PostgreSQL로 안전해도, root `.env` 와 active guard/smoke가 MySQL compatibility를 남겨두면 compose 밖 실행과 로컬 검증이 잘못된 경로를 조용히 탈 수 있다
- 문제: 서버 확인 결과 실제 Docker runtime env는 PostgreSQL override로 정상 기동 중이었지만, 저장소 root `.env` 의 `DB_URL`, `APP_PII_DB_URL`, `NOTIFICATION_PII_DB_URL` 은 여전히 MySQL JDBC를 가리키고 있었다. 동시에 `SecondaryDataSourceSchemaGuard`, `preflight-runtime-cutover-env.sh`, `run-local-pii-sync-cutover-smoke.sh` 는 MySQL URL을 “legacy compatibility” 로 계속 허용하거나 PostgreSQL 값으로 자동 보정하고 있었다. 이 상태에서는 compose 밖 `bootRun`·직접 `prod` 실행·로컬 smoke가 MySQL 잔재를 조용히 흡수해, 실제 운영 truth와 다른 설정을 정상처럼 보이게 만들 수 있었다.
- 해결: active `.env` 의 세 JDBC URL을 PostgreSQL mainline 값으로 교정했고, `CLAUDE.md` 의 DB 설명도 PostgreSQL 기준으로 수정했다. `SecondaryDataSourceSchemaGuard` 는 이제 `jdbc:postgresql:` 만 허용하고, `preflight-runtime-cutover-env.sh` 도 PostgreSQL URL만 valid로 본다. `run-local-pii-sync-cutover-smoke.sh` 는 빈 secondary URL은 PostgreSQL 기본값으로 채우되, MySQL URL이 명시되면 자동 보정하지 않고 즉시 fail-fast 하게 바꿨다.
- 이유: 현재 운영 truth가 PostgreSQL-only 라면, active 경로는 잘못된 MySQL 설정을 “돌아가게 만드는 것”보다 “즉시 틀렸다고 말하는 것”이 더 안전하다. legacy는 `deploy/mysql/**` 같은 명시적 경계에만 남기고, mainline guard/smoke는 운영 truth를 강하게 반영해야 drift를 빨리 잡는다.

## 818) JPQL region 정렬식에 nested enum literal을 직접 넣으면, 쿼리 의미는 맞아도 Hibernate validation 단계에서 부팅 실패를 만들 수 있다
- 문제: `fc5523a` 에서 region/sido retrieval 쿼리에 `지역 매칭 BOKJIRO_LOCAL 우선` 정렬을 넣으면서 `ws.sourceType = com.example.welfare.policy.entity.WelfareService.SourceType.BOKJIRO_LOCAL` 형태의 nested enum literal을 `ORDER BY CASE` 안에 직접 사용했다. 서버 재기동에서는 이 JPQL이 Hibernate validation 단계에서 터져 앱이 health `UP` 까지 못 올라갔다.
- 해결: 같은 조건을 string literal 비교 `ws.sourceType = 'BOKJIRO_LOCAL'` 로 바꿨다. sourceType 이 enum string 저장인 현재 모델에서는 의미는 같고, Hibernate parser/validator 쪽 부팅 리스크만 줄어든다.
- 이유: retrieval SQL 우선화처럼 운영 부팅과 직접 연결된 JPQL은 “표현상 더 타입 안전해 보이는 문법”보다 “현재 Hibernate가 검증까지 안정적으로 통과하는 문법”이 우선이다. 특히 `CASE + EXISTS + enum literal` 조합은 로컬 테스트가 통과해도 서버 parser 구현차로 부팅 실패를 만들 수 있으므로, mainline에서는 더 보수적인 literal 비교가 안전하다.

## 819) refresh 응답 정렬과 persisted rank를 같은 기준으로 다시 읽지 않으면, 클릭 smoke가 “DB top1과 다른 서비스” 를 누른 것처럼 보일 수 있다
- 문제: server `REAL_USER` 재관측 과정에서 같은 사용자에 대해 DB latest `final_score DESC` 기준 top1은 `2285` 인데, 당시 click smoke는 `3686` 을 눌렀던 흔적이 보여 `POST /api/recommendations/refresh` 응답 순서와 persisted rank 불일치 의심이 생겼다. 직접 대조해 보니 `GET /api/recommendations` 와 refresh cache reuse 응답은 DB rank와 일치했지만, refresh 생성 직후에는 저장 서비스가 반환한 순서와 DB 재조회 순서가 항상 같다고 보장하지 않았다.
- 해결: `RecommendationGenerationService` 가 `recommendationPersistenceService.save(...)` 직후 반환 리스트를 그대로 노출하지 않고, 같은 `recommendedAt` batch를 `final_score DESC, service_id DESC` 기준으로 다시 읽도록 바꿨다. `UserRecommendationRepository` 도 latest batch/read 쿼리들에 `service_id DESC` tie-break를 추가해 정렬 계약을 고정했다. 테스트도 “refresh 응답은 저장 직후 DB 재조회 순서와 같다”는 케이스를 추가했다.
- 이유: 추천 응답, click log 대상, admin diagnostics가 모두 같은 “top1” 을 가리켜야 원인 추적이 단순해진다. 저장 직후 메모리 리스트 순서와 DB persisted rank를 분리해 두면, 실제 버그가 retrieval/filter인지 응답 ordering인지 먼저 분기해야 해서 운영 분석 비용이 커진다. refresh 응답은 persisted batch order를 기준으로 다시 읽는 쪽이 더 안정적이다.

## 820) `FILTERED_BY_YOUTH_OR_AGE` 같은 묶음 라벨은 실제 탈락 원인이 `청년성 부족` 인지 `특수대상/저소득/지역 조건` 인지 가려 버린다
- 문제: `fc5523a` 반영 뒤 인천 no-priority `REAL_USER` 세그먼트에서 `3257/3281/3575/3714` 는 더 이상 `NOT_IN_SQL_RETRIEVAL` 이 아니었지만, diagnostics는 이 네 서비스를 전부 `FILTERED_BY_YOUTH_OR_AGE` 로만 보여 줬다. 서버에서 실제 서비스 조건을 읽어 보니 `3257` 은 주거급여/무주택, `3281` 은 옹진군 저소득, `3575` 는 고독사 고위험군/중장년, `3714` 는 성인 발달장애인 대상이라, 단순 “연령” 보다 `현재 사용자와 주요 대상이 안 맞는다` 쪽이 핵심이었다.
- 해결: `RetrievalService.trace(...)` 가 후보별 `primaryAudienceRelevant`, `ageConstraintMatched` 판정 맵도 같이 들고 오게 바꿨다. `AdminDashboardRecommendationDiagnosticService` 는 이 맵을 이용해 retrieval 단계 탈락 라벨을 `FILTERED_BY_PRIMARY_AUDIENCE_RELEVANCE`, `FILTERED_BY_AGE_CONSTRAINT`, `FILTERED_BY_PRIMARY_AUDIENCE_AND_AGE` 로 세분화한다. 기존 `FILTERED_BY_YOUTH_OR_AGE` 는 filter trace가 비어 있을 때만 fallback 으로 남긴다.
- 이유: 지금 recommendation 트랙의 남은 병목은 “청년 local 후보가 정말 조건상 안 맞아 떨어지는지”와 “조건은 맞는데 점수에서 밀리는지”를 구분하는 것이다. dropStage가 원인을 숨기면 다음 액션이 filter 조정인지 scoring 보정인지 결정하기 어렵다. retrieval diagnostics는 실제 경계 이름을 더 정확히 말해 주는 편이 운영/튜닝 판단에 유리하다.

## 821) active smoke가 MySQL/compose-host JDBC를 자동 보정하면, 현재 PostgreSQL-only 운영 truth와 다른 설정도 정상처럼 지나가 drift를 늦게 찾게 된다
- 문제: `preflight-runtime-cutover-env.sh` 와 `run-local-pii-sync-cutover-smoke.sh` 는 이미 PostgreSQL-only fail-fast 로 정리됐지만, `run-local-education-priority-replay.sh` 만은 여전히 `jdbc:mysql://...` 또는 `jdbc:postgresql://db:...` 값을 만나면 조용히 localhost PostgreSQL URL로 바꾸고 계속 진행했다. 이 상태에서는 active replay smoke가 실제 잘못된 `.env` 나 compose 밖 실행 drift를 흡수해, 운영 truth와 다른 경로를 “정상”처럼 보이게 만들 수 있었다.
- 해결: replay smoke에도 `assert_local_postgres_jdbc_url` 검증을 추가해 `DB_URL`, `APP_PII_DB_URL`, `NOTIFICATION_PII_DB_URL` 이 모두 `jdbc:postgresql://` 이고, 동시에 compose 내부 host `db` 가 아니라 local host를 가리키는지 먼저 확인하게 바꿨다. 이제 MySQL URL이나 `jdbc:postgresql://db:...` 값이 들어오면 자동 보정 없이 즉시 fail-fast 한다.
- 이유: 현재 운영 truth가 PostgreSQL-only 라면, active smoke는 잘못된 URL을 “대신 고쳐서 돌리는 것”보다 “즉시 틀렸다고 말하는 것”이 더 안전하다. drift를 조기에 드러내야 `.env`, shell env, compose 밖 bootRun 경로를 같은 기준으로 관리할 수 있다.

## 822) retrieval/filter 문제가 닫힌 뒤에도 local 청년 후보가 왜 8위인지 모르면, 남은 병목이 `rule/AI` 인지 `rerank/diversity` 인지 다시 추측하게 된다
- 문제: `2736` 은 server 재관측 기준으로 이미 `PRESENT_IN_SAVED_BATCH(savedRank=8)` 였고, `interest/priority/specialTarget mismatch` 도 없었다. 동시에 `3686/3282/3252/2736` 는 `ruleBaseScore=23`, `ruleWeightedScore=23`, `aiScore=NULL` 로 같아 보였기 때문에, “그럼 왜 `2736` 만 8위인가”를 기존 diagnostics 필드만으로는 설명할 수 없었다.
- 해결: `ReRankingService` 에 diagnostics 전용 `trace(...)` 경로를 추가해 candidate별 `normRule`, `baseBlendScore`, `priorityMultiplier`, `scoreAfterPriorityMultiplier`, `diversityBucket`, `diversityPenalty`, `noPriorityTopBandEligible`, `noPriorityAdjustment`, `currentFinalScore`, `currentRank` 를 계산하게 했다. `AdminDashboardRecommendationDiagnosticService` 는 이를 `recommendation-diagnostics` 응답에 그대로 싣는다.
- 이유: 추천 생성 경로를 다시 실행하지 않고도 “retrieval/filter는 통과했는데 final score 단계에서 왜 밀렸는지”를 같은 API로 읽을 수 있어야 다음 액션이 `filter 완화` 가 아니라 `rerank/diversity 조정` 임을 바로 확정할 수 있다. 특히 `rule=23, ai=NULL` 같은 동점 그룹에서는 diversity penalty와 no-priority top-band 보정이 사실상 순위를 가르는 축이므로, 이 값을 직접 노출하는 편이 운영/튜닝 판단에 유리하다.

## 823) `POST /api/recommendations/refresh` 는 기본적으로 `personal=false` 라서, rerank diagnostics와 saved batch를 직접 비교할 때는 cache hit을 먼저 의심해야 한다
- 문제: server `b4000c0` diagnostics 기준으로 `2736` 은 `rerankCurrentRank=3`, `rerankCurrentFinalScore=1.03` 이었지만, 직후 `POST /api/recommendations/refresh` 응답과 saved batch는 여전히 `latestSavedRank=8`, `latestSavedFinalScore=0.64` 를 돌려줬다. 얼핏 보면 diagnostics rerank와 실제 저장 경로가 다른 후보 집합을 쓰는 것처럼 보이지만, `RecommendationController.refresh` 의 기본 `personal=false` 는 먼저 refresh cache reuse를 시도한다.
- 해결: 이 경계는 우선 코드 버그로 단정하지 않고, “saved batch를 최신 rerank와 맞춰 비교하려면 `POST /api/recommendations/refresh?personal=true` 로 cache bypass 재계산을 강제한다”는 운영 절차로 정리했다. 즉 non-personal refresh 결과가 과거 batch를 재사용하는 한, diagnostics의 current rerank trace와 saved rank를 1:1로 놓고 보면 안 된다.
- 이유: 현재 recommendation API는 `refresh` 라는 이름과 달리 non-personal 모드에서 cache reuse를 허용한다. 따라서 troubleshooting 순서도 “saved rank와 diagnostics rerank가 다르다 -> 생성 경로 버그” 가 아니라, 먼저 “이 호출이 진짜 재계산이었는가”를 확인하는 쪽이 맞다. `personal=true` 재실행으로도 차이가 유지될 때만 생성 경로와 diagnostics trace 불일치를 의심해야 한다.

## 824) `personal=true` 뒤에도 diagnostics rerank와 saved rank가 다르면, 다음으로 봐야 할 것은 cache가 아니라 diagnostics가 AI 이전 단계만 재현한다는 점이다
...
## 894) Gov24 canonical promotion 1차는 `service_facts` 가 아니라 `service_taxonomy_terms` 에 bounded 하게 여는 편이 맞다
- 문제: 문서로는 `Gov24 canonical promotion` 범위를 정리했지만, 코드가 여전히 summary label만 저장하면 다음 단계에서 다시 “term으로도 올릴 건지, 아니면 parser 관찰만 할 건지”가 흐려질 수 있었다. 반대로 바로 `service_facts` 나 scoring으로 가면 `사용자구분/지원유형` 을 eligibility 축처럼 오해할 위험이 컸다.
- 해결:
  - `NormalizationKeySupport` 에
    - `GOV24_SERVICE_FIELD`
    - `GOV24_USER_TYPE_TOKEN`
    - `GOV24_BENEFIT_TYPE_TOKEN`
    term group과 source field 상수를 추가했다.
  - 새 `Gov24NormalizationSupport` 를 만들어
    - `서비스분야` 는 exact raw label term 1개
    - `사용자구분` 은 split token allowlist term
    - `지원유형` 은 split token allowlist term
    으로만 조립하게 했다.
  - `WelfareServiceMapper.toNormalizedGov24(...)` 는 이제 summary label과 함께 위 taxonomy terms를 같이 만든다.
  - 테스트도 `Gov24NormalizationSupportTest`, `NormalizedPolicyAggregateTest`, `NormalizedPolicySidecarPersistenceIntegrationTest` 로 추가해, 실제 sidecar save path에서 `service_taxonomies` summary는 그대로 유지되면서 `service_taxonomy_terms` 에만 Gov24 canonical term이 저장되는 계약을 고정했다.
- 이유:
  - 이 단계의 목적은 raw exact label을 버리거나 `service_facts` 를 여는 것이 아니라, **label-first canonical term 저장층을 먼저 만드는 것**이다.
...
## 892) 하이브리드 완화는 system prompt 수준에서 먼저 시도했지만, 핵심 zero cohort를 줄이지 못해 mainline에 유지하지 않는다
...
- 서버 재검증:
  - `3257`: 여전히 `savedAi=0`, reason=`저소득층을 대상으로 한 정책으로, 현재 사용자의 소득 수준과 맞지 않는다.`
  - `3209`: 여전히 `savedAi=0`, reason=`신혼부부 전용 정책으로, 현재 사용자의 특성과 맞지 않는다.`
  - `3287`: 여전히 `savedAi=0`
  - `3288`: 직전 `savedAi=70` 에서 다시 `savedAi=0` 으로 하락
  - `fresh_top_ai_zero_count`: `3 -> 4`
- 이유:
...
- 문제: 현재 앱 코드는 env key(`DB_URL`, `APP_PII_DB_URL`, `REDIS_HOST`)만 보면 되는데, 기존 [docker-compose.yml](../../docker-compose.yml) 은 `db` 컨테이너와 `docker-entrypoint-initdb.d` 자동 init을 전제로 한다. 이 상태로는 "코드를 EC2+RDS 형태로 바꿔야 하나?"라는 혼선이 계속 생긴다.
- 해결: 로컬 compose는 그대로 두고, 운영은 [docker-compose.prod.yml](../../docker-compose.prod.yml) 과 [env.production.example](../../env.production.example) 로 분리했다. 운영형 compose는 `app + redis` 만 띄우고 DB는 RDS endpoint를 직접 보게 하며, bootstrap은 [bootstrap-rds-runtime.sh](../../deploy/postgres/bootstrap-rds-runtime.sh) 로 `schema.sql -> runtime roles/grants -> patch` 순서를 EC2 shell에서 수행하게 고정했다.
...
- 문제: 이 프로젝트는 [AuthController.java](../../backend/src/main/java/com/example/welfare/user/controller/AuthController.java) 에서 refresh token을 `HttpOnly + SameSite=Lax` cookie로 내려 주고 있었지만, 프론트 [axios.js](../../frontend/src/lib/axios.js) 와 [authStore.js](../../frontend/src/store/authStore.js) 는 access token을 `localStorage("token")` 에 따로 저장해 Bearer로 붙였다. 그래서 refresh cookie 자체는 읽지 못해도, XSS가 나면 access token을 브라우저 영구 저장소에서 바로 꺼내 쓸 수 있는 경계가 남아 있었다.
- 해결: access token은 더 이상 persist/localStorage에 저장하지 않고 zustand 메모리 state에만 유지하게 바꿨다. 앱 시작 시 [AuthBootstrap.jsx](../../frontend/src/components/AuthBootstrap.jsx) 가 refresh cookie로 `/api/auth/refresh` 를 먼저 호출해 세션을 복구한 뒤 라우터를 렌더링하고, refresh 실패/만료 시에는 `logout` 과 별도인 `clearSession` 으로 세션만 비운다. 또한 legacy `token` / `auth-store` localStorage 흔적은 초기화해 이전 브라우저 저장소가 남아도 새 코드가 다시 읽지 않게 맞췄다.
...
- 문제: [AesEncryptUtil.java](../../backend/src/main/java/com/example/welfare/global/util/AesEncryptUtil.java) 는 기존에 `AES/CBC/PKCS5Padding` 으로 `Base64(iv+ciphertext)` 형태를 저장하고 있었고, integration tests도 이 포맷으로 누적된 `user_pii` 데이터를 전제로 돌고 있다. 여기서 encrypt/decrypt를 GCM으로만 단순 교체하면, 이미 저장된 CBC payload를 더 이상 복호화하지 못해 로그인/프로필/알림 관련 기존 row read가 깨질 수 있다.
...
- 문제: `/api/auth/email-verification/send` 는 `permitAll` 공개 엔드포인트인데, 기존 [EmailVerificationService.java](../../backend/src/main/java/com/example/welfare/user/service/EmailVerificationService.java) 는 같은 이메일에 대한 cooldown만 걸고 있었다. 이 상태에선 공격자가 이메일 주소만 계속 바꿔 같은 IP/UA에서 반복 호출해 메일 발송 비용과 평판을 소모시킬 수 있다.
- 해결: [AuthController.java](../../backend/src/main/java/com/example/welfare/user/controller/AuthController.java) 에서 `ClientFingerprintService.build(request)` 값을 먼저 구해 [AuthRateLimitService.java](../../backend/src/main/java/com/example/welfare/user/service/AuthRateLimitService.java) 의 `checkEmailVerificationSendLimit()` 를 태우고, 기존 `EmailVerificationService.sendCode(email)` 의 이메일별 cooldown은 그대로 유지하게 맞췄다. 동시에 `@Validated` + `@RequestParam @Email` 로 형식이 잘못된 주소는 서비스 진입 전에 400으로 거른다.
...
- 문제: [RecommendationExecutionGuard.java](../../backend/src/main/java/com/example/welfare/recommend/service/RecommendationExecutionGuard.java) 는 userKey 기준 lock으로 "동시에 두 번" 실행되는 것만 막는다. 하지만 `/api/recommendations/refresh?personal=true` 는 [RecommendationGenerationService.java](../../backend/src/main/java/com/example/welfare/recommend/service/RecommendationGenerationService.java) 에서 refresh cache를 즉시 비우고 retrieval/rule/AI/rerank/save를 다시 태우므로, 사용자가 순차적으로 짧은 간격으로 여러 번 누르면 lock이 풀릴 때마다 비싼 파이프라인이 계속 돈다.
- 해결: 새 [RecommendationRefreshRateLimitService.java](../../backend/src/main/java/com/example/welfare/recommend/service/RecommendationRefreshRateLimitService.java) 를 추가해 userKey 기준 Redis TTL 키를 두고, `personal=true` 는 도입 당시 기본 `1회 / 600초`(2026-06-01 현재 `3회 / 600초`), `personal=false` 는 기본 `3회 / 60초` 로 제한했다. 검사는 generation service 초입, 즉 execution lock 전에 수행하고 초과 시 새 429 코드 `R004` 를 반환한다.
...
- 문제: 기존 [WelfareServiceMapper.java](../../backend/src/main/java/com/example/welfare/collect/mapper/WelfareServiceMapper.java) 는 `www.` 를 `https://` 로 보정하는 정도였고, 일부 source는 `RawFieldValidator.normalize(servDtlLink)` 값을 그대로 `detailUrl` 에 넣었다. 프론트 [PolicyDetailPage.jsx](../../frontend/src/pages/PolicyDetailPage.jsx) 도 `href` 와 `window.open()` 에 그 값을 거의 그대로 넘겼다. 이 상태에선 `javascript:`, `data:`, `intent:` 같은 비정상 scheme이 수집 데이터로 들어오면 렌더링/오픈 경계가 느슨해진다.
...
- 문제: [NotificationController.java](../../backend/src/main/java/com/example/welfare/notification/controller/NotificationController.java) 는 공개 링크 `/api/notifications/unsubscribe` 에서 `jwtUtil.getSubjectAllowExpired(token)` 으로 userKey를 읽고 있었다. 이 메서드는 [JwtUtil.java](../../backend/src/main/java/com/example/welfare/global/util/JwtUtil.java) 에서 `ExpiredJwtException` 이어도 claims에서 subject를 꺼내 반환하므로, 서명만 맞으면 만료된 unsubscribe 링크도 계속 성공한다.
...
- 문제: 기존 [frontend/public/sw.js](../../frontend/public/sw.js) 는 notification payload의 `url` 을 그대로 `navigate()` / `openWindow()` 에 넘겼고, [WebPushDispatchService.java](../../backend/src/main/java/com/example/welfare/notification/service/WebPushDispatchService.java) 의 test send도 `request.url` 을 그대로 payload에 실었다. 이 상태에선 외부 origin URL이나 비정상 scheme이 push payload에 들어오면, 사용자는 "앱 알림"을 눌렀는데 외부 사이트로 이동할 수 있다.
...
- 해결: [SecurityConfig.java](../../backend/src/main/java/com/example/welfare/global/config/SecurityConfig.java) 의 public 정책 조회 그룹에 두 endpoint를 명시적으로 추가했다.
...
- 해결: integration은 endpoint 대신 [PolicySearchKeywordReadRepository.java](../../backend/src/main/java/com/example/welfare/policy/repository/PolicySearchKeywordReadRepository.java) 를 직접 호출하고, `windowStart` 를 짧게 잡아 fixture만 읽는 방식으로 바꿨다. HTTP contract는 별도 WebMvc 테스트로 유지했다.
...
- 문제: [policy-next-active-track-priority.md](../policy/policy-next-active-track-priority.md) 는 `Gov24 canonical promotion` 을 다음 active track처럼 서술하고 있었지만, 실제 코드와 [phase-plan.md](../phase-plan.md) 은 이미 label-first canonical 경계가 read-model/admin/presentation/test까지 닫힌 상태였다. 이 차이 때문에 “다음 로컬 작업”을 고를 때 이미 끝난 구현을 또 열어야 하는 것처럼 읽힐 수 있었다.
...
## 1040) `recent_policy_views` 는 write surface지만 `DELETE` 가 필요 없는 bounded privilege였다
- 문제: `recent_policy_views` 는 최근 본 정책 upsert/read 모델이라 앱 코드가 직접 `DELETE` 를 호출하지 않는데도, 초기 patch와 runtime bootstrap은 `app_core_rw` 에 `SELECT, INSERT, UPDATE, DELETE` 를 통째로 줬다. 이 상태는 cleanup role까지 만들 정도로 큰 경계는 아니지만, public write role에 불필요한 `DELETE` 가 남는 drift였다.
- 해결: 코드 경로를 다시 확인해 [PolicyViewLogCommandRepositoryImpl.java](../../backend/src/main/java/com/example/welfare/policy/repository/PolicyViewLogCommandRepositoryImpl.java) 는 `INSERT ... ON CONFLICT DO UPDATE` 만, [UserRecentViewedPolicyReadService.java](../../backend/src/main/java/com/example/welfare/user/service/UserRecentViewedPolicyReadService.java) 는 read만 쓰는 bounded surface임을 확정했다. 그 뒤 `app_core_rw` 에서 `recent_policy_views DELETE` 만 revoke하는 migration/patch를 추가하고, fresh init([z90-create-runtime-db-users.sh](../../deploy/postgres/init/z90-create-runtime-db-users.sh)), RDS bootstrap([bootstrap-rds-runtime.sh](../../deploy/postgres/bootstrap-rds-runtime.sh)), 운영 verify([verify-rds-runtime-privileges.sh](../../deploy/postgres/verify-rds-runtime-privileges.sh)) 까지 같이 맞췄다. 즉 이 경계는 전용 cleanup role을 늘리지 않고도 bounded revoke로 닫는 편이 더 단순하다.

## 1041) `recommendation_review_gate_promotion_approvals` 는 admin 전용 단일 테이블이라 별도 command role로 떼는 편이 bounded했다
- 문제: 이 테이블은 recommendation lane reopen 판단을 위한 admin-only approval record인데, 초기 migration/patch는 편의상 `app_core_rw` 에 `SELECT/INSERT/UPDATE/DELETE` 를 통째로 줬다. 하지만 실제 write 경로는 [AdminRecommendationReviewGatePromotionApprovalRecordService.java](../../backend/src/main/java/com/example/welfare/admin/dashboard/service/AdminRecommendationReviewGatePromotionApprovalRecordService.java) 의 upsert/delete 두 경로뿐이고, read도 이미 admin read-only datasource를 타므로 primary runtime role에 남길 이유가 약했다.
- 해결: [RecommendationReviewGatePromotionApprovalCommandRepositoryImpl.java](../../backend/src/main/java/com/example/welfare/admin/dashboard/repository/RecommendationReviewGatePromotionApprovalCommandRepositoryImpl.java) 를 `recommendation_review_gate_command_rw` 전용 datasource로 옮기고, `app_core_rw` 에서는 `recommendation_review_gate_promotion_approvals` `ALL` 권한을 제거했다. 동시에 fresh init/로컬 patch/RDS bootstrap/runtime verify/preflight/env example을 같이 맞춰, “코드는 secondary datasource를 기대하지만 실제 role/grant/login은 없는 drift” 까지 한 번에 닫는다.

## 1042) 배포용 patch는 `psql -f` 직접 실행 계약이면 `:'var'` 치환에 기대지 않는 편이 안전하다
- 문제: `deploy/postgres/patches/V2026_05_30_04__add_recommendation_review_gate_command_role.sql` 첫 버전은 `:'recommendation_review_gate_command_username'` 같은 psql 변수 치환을 가정했다. 하지만 서버 운영에서 patch를 `psql -f` 로 직접 실행하면 해당 변수를 넘기지 않는 경우가 자연스럽고, 이때 `DO $$` 블록 안에서 syntax error처럼 보이는 실패가 난다.
- 해결: patch 파일은 direct `psql -f` 실행을 기준 계약으로 다시 맞췄다. 현재 파일은 표준 runtime role 이름(`app_core_rw`, `recommendation_review_gate_command_rw`, `migration_admin`)을 직접 사용하므로 추가 `-v` 없이도 실행된다. 반대로 bootstrap/init 경로는 여전히 env 기반 커스텀 값을 받을 수 있게 두어, “one-off patch direct apply” 와 “fresh/bootstrap automation” 의 실행 문맥을 분리한다.
## 1043) `user_recommendations DELETE` 는 retention cleanup role과 섞지 말고 refresh replace command 경계로 분리하는 편이 안전하다
- 문제: `app_core_rw` 는 이미 `recommendation_retention_cleanup_rw` 가 따로 있는데도, refresh replace 경로의 `deleteAllByUserKey()` 때문에 `user_recommendations DELETE` 를 계속 직접 들고 있었다. 이 상태는 least-privilege 경계가 어색하고, 단순 revoke만 하면 recommendation refresh replace가 바로 깨진다.
- 해결: `recommendation_persistence_command_rw` 전용 datasource를 추가하고, `RecommendationPersistenceCommandRepositoryImpl.replaceAllForUser()` 는 이 경계에서 `DELETE + batch INSERT` 를 같은 JDBC transaction으로 수행하게 바꿨다. 동시에 `app_core_rw` 에서는 `user_recommendations DELETE` 를 revoke 하고, 새 role에는 `INSERT/DELETE` 와 `user_recommendations_id_seq USAGE` 만 부여했다.
- 이유: retention cleanup role에 refresh replace까지 얹으면 역할 이름과 운영 해석이 흐려지고, 반대로 `DELETE` 만 secondary datasource로 빼면 rollback 경계를 깨뜨린다. refresh replace 자체를 command role로 묶는 편이 권한/트랜잭션/문서 설명이 가장 일관적이다.

## 1044) secondary `PlatformTransactionManager` 를 추가하면 Boot 기본 `transactionManager` 가 사라질 수 있다
- 문제: `recommendation_persistence_command_rw` 전용 datasource를 추가하면서 [RecommendationPersistenceCommandDataSourceConfig.java](../../backend/src/main/java/com/example/welfare/global/config/RecommendationPersistenceCommandDataSourceConfig.java) 에 `recommendationPersistenceCommandTransactionManager` bean이 생겼다. 이 뒤 서버 signup/login sanity에서 `NoSuchBeanDefinitionException: No bean named 'transactionManager' available` 가 발생했고, 일반 `@Transactional` 서비스가 500으로 깨졌다.
- 해결: [PrimaryDataSourceConfig.java](../../backend/src/main/java/com/example/welfare/global/config/PrimaryDataSourceConfig.java) 에 primary JPA transaction manager를 `@Bean(name = {"transactionManager", "primaryTransactionManager"})` + `@Primary` 로 명시했다. 이제 기본 JPA write/read 서비스는 다시 `transactionManager` 이름을 해석하고, bounded secondary command datasource는 자기 전용 transaction manager만 명시적으로 쓴다.
- 이유: Spring Boot는 `PlatformTransactionManager` bean이 하나도 없을 때만 기본 `transactionManager` auto-config를 제공한다. secondary datasource를 늘리는 저장소에서는 "새 manager 추가" 와 "기본 primary manager 이름 보존"을 같이 해 줘야 런타임 API가 깨지지 않는다.

## 1045) PostgreSQL에서는 `DELETE ... WHERE column = ?` 에도 해당 column `SELECT` 권한이 필요하다
...
- 해결: fresh init([z90-create-runtime-db-users.sh](../../deploy/postgres/init/z90-create-runtime-db-users.sh)), RDS bootstrap([bootstrap-rds-runtime.sh](../../deploy/postgres/bootstrap-rds-runtime.sh)), runtime drift patch([V2026_05_30_06__grant_recommendation_persistence_user_key_select.sql](../../deploy/postgres/patches/V2026_05_30_06__grant_recommendation_persistence_user_key_select.sql)), verifier([verify-rds-runtime-privileges.sh](../../deploy/postgres/verify-rds-runtime-privileges.sh)) 기준을 모두 `SELECT(user_key)` bounded grant 포함 형태로 맞췄다.
...
- 문제: `frontend-browser-smoke` job은 `npm ci` 단계에서 먼저 실패하면 아직 `docker compose up` 을 하지 않았더라도 마지막 cleanup step의 `docker compose down -v` 는 그대로 실행된다. 이 저장소의 [docker-compose.yml](../../docker-compose.yml) `app` 서비스는 `env_file: .env` 를 참조하므로, CI workspace 루트에 `.env` 가 없으면 cleanup 자체가 또 한 번 빨갛게 남는다.
- 해결: [.github/workflows/ci.yml](../../.github/workflows/ci.yml) 에 `Prepare CI compose env file` step을 추가해 early stage에서 `touch .env` 를 수행하도록 했다. 이렇게 하면 install 단계가 먼저 실패해도 cleanup의 compose parse는 더 이상 `.env not found` 로 깨지지 않는다.
...
- 문제: `frontend-browser-smoke` job의 전역 `APP_BASE_URL` 은 Playwright와 Vite용 `http://127.0.0.1:5173` 이었다. 그런데 [bootstrap-playwright-smoke-data.sh](../../frontend/scripts/bootstrap-playwright-smoke-data.sh) 는 `APP_BASE_URL` 을 백엔드 API base와 health endpoint 기준으로 읽어 `HEALTH_URL=${APP_BASE_URL}/actuator/health` 를 만들기 때문에, CI에서 `127.0.0.1:5173/actuator/health` 를 60회 재시도하다가 실패했다.
- 해결: [.github/workflows/ci.yml](../../.github/workflows/ci.yml) 의 `Bootstrap frontend smoke data` step에만 `APP_BASE_URL=http://127.0.0.1:8082`, `HEALTH_URL=http://127.0.0.1:8082/actuator/health` 를 명시적으로 덮어썼다. 브라우저 smoke 자체는 계속 `PLAYWRIGHT_BASE_URL=5173`, `VITE_API_BASE_URL=8082` 계약을 유지한다.
...
- 문제: `frontend-browser-smoke` job의 backend `bootRun` 이 health 대기 전에 바로 죽었는데, backend log를 보면 `JWT_SECRET` 이 `1.23456789012346E+30` 으로 전달되어 [JwtUtil.java](../../backend/src/main/java/com/example/welfare/global/util/JwtUtil.java) 초기화에서 `WeakKeyException` 이 났다. workflow YAML에 `012345...` 값을 따옴표 없이 썼기 때문에 GitHub Actions env가 이를 숫자로 해석해 과학 표기 문자열로 바꿨다.
- 해결: [.github/workflows/ci.yml](../../.github/workflows/ci.yml) 의 `backend-unit`, `frontend-browser-smoke` 두 job 모두 `JWT_SECRET` 을 `"01234567890123456789012345678901"` 처럼 명시적으로 문자열 quoting 했다.
...
## 1050) reset-password 브라우저 smoke는 메일 인프라가 없는 CI에서 `A009` 일 때 direct token seed fallback이 필요하다
- 문제: `frontend-browser-smoke` job은 fake Gmail credential로 backend를 띄우므로 `POST /api/auth/password-reset/request` 가 [PasswordResetService.java](../../backend/src/main/java/com/example/welfare/user/service/PasswordResetService.java) 내부 `EmailClient.send(...)` 실패 후 `A009` 로 끝난다. Playwright smoke는 그 응답을 무조건 200으로 기대해 query/hash reset-password 2개가 계속 깨졌다.
- 해결: [frontend/e2e/smoke.spec.js](../../frontend/e2e/smoke.spec.js) 의 `issuePasswordReset()` 은 기본적으로 real API request를 유지하되, 응답이 `A009` 일 때만 새 [issue-password-reset-token.sh](../../frontend/scripts/issue-password-reset-token.sh) helper로 Redis에 reset token을 직접 seed해서 브라우저 flow를 계속 검증하게 바꿨다.
- 이유: 이 smoke의 목적은 메일 provider 통합이 아니라 `/reset-password` query/hash 진입과 confirm/login flow 검증이다. 메일 인프라가 없는 CI에서만 bounded fallback을 허용하고, 그 외 error는 그대로 실패시키는 편이 목적과 회귀 감도를 모두 지킨다.

## 1051) auth/session도 compact operator surface가 없으면 recommendation/collect/policy와 진입 패턴이 다시 달라진다
- 문제: auth는 `run-local-auth-session-smoke.sh` 와 current-state/checklist는 있었지만, daily operator가 바로 읽는 compact handoff surface는 없었다. 이 상태면 다른 문서군은 `observation suite -> latest note` 패턴으로 보는데 auth만 raw smoke stdout을 직접 해석해야 했다.
- 해결: `deploy/smoke/run-local-auth-observation-suite.sh` 를 추가해 기존 auth/session smoke 묶음을 다시 읽고 `decision_class`, `enabled_smoke_steps`, `operator_reading`, `next_action` 을 `summary/json/note` artifact로 publish하게 맞췄다. 동시에 `docs/auth/auth-observation-runbook.md` 를 추가하고 `auth-docs-index`, `auth-session-revocation-current-state`, `start`, `current-state` 도 같은 entrypoint 규칙으로 갱신했다.
- 이유: baseline 유지 단계에서는 개별 revoke/lockout smoke의 세부보다 “현재 auth/session baseline이 healthy 한가”를 먼저 읽는 surface가 중요하다. 문서군마다 진입 패턴을 맞춰야 운영자 handoff와 반복 검증 비용이 줄어든다.

## 1052) frontend QA도 current-state와 checklist만 있으면 daily operator는 lint/build/Playwright 결과를 다시 raw stdout으로 읽어야 한다
...
## 1054) `Gov24 userType` public filter는 exact raw label이 아니라 additive token이므로, legacy fallback도 `||` split semantics로 맞춰야 한다
