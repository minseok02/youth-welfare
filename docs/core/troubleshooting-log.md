...
- 해결: [PolicyDetailPage.jsx](/home/ubuntu/youth-welfare/frontend/src/pages/PolicyDetailPage.jsx:1) 의 `Gov24` 태그를 clickable chip으로 바꿔 `serviceField`, `userType`, `benefitType` 각각 `/policies?sourceType=GOV24&...` 로 바로 이동하게 했다. 즉 `분야/대상/유형` badge를 read-only 메타가 아니라 bounded discovery bridge로 승격한다.
...
- 해결: [RuleScoringService.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/recommend/service/RuleScoringService.java:1) 는 새 [Gov24RecommendationScoringSupport.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/recommend/support/Gov24RecommendationScoringSupport.java:1) 를 통해 `Gov24` projection을 **bounded soft additive bonus** 로만 소비한다. `serviceField` 는 existing compat bridge(`주거·자립 -> HOUSING`, `고용·창업 -> JOB`, `보육·교육 -> EDUCATION`, `생활안정 -> FINANCE`, `문화·환경 -> CULTURE`, `보호·돌봄/임신·출산 -> FAMILY`)를 small bonus로 재사용하고, `benefitType` 도 `현금(장학금)`, `현금(융자)`, `서비스(일자리)`, `문화/여가지원`, `서비스(돌봄)` 같은 일부 managed token만 additive signal로 읽는다. `userType` 은 `개인`, `가구` 만 tiny audience bonus로 쓰고, `법인/시설/단체`, `소상공인` 은 hard exclusion으로 해석하지 않는다.
- 추가 해결: [DefaultPriorityMatcherTest.java](/home/ubuntu/youth-welfare/backend/src/test/java/com/example/welfare/recommend/service/DefaultPriorityMatcherTest.java:1) 에 `Gov24` taxonomy projection만으로는 `HOUSING/FINANCE` priority matcher가 열리지 않는 회귀를 추가해, scoring만 active이고 matcher hard condition은 계속 deferred 라는 경계를 테스트로 고정했다.
- 추가 해결 2: [CanonicalRecommendationReadModelRepositoryTest.java](/home/ubuntu/youth-welfare/backend/src/test/java/com/example/welfare/recommend/repository/CanonicalRecommendationReadModelRepositoryTest.java:1) 에 `Gov24` canonical term이 recommendation projection에 들어와도 `youthMajorLabel/youthMidLabel` 은 자동으로 채우지 않는 회귀를 추가했다. 즉 `Gov24 -> YOUTH_MID` bridge도 현재는 deferred 라는 경계를 read-model 단계에서 고정한다.
...
- 해결: 새 `run-local-ops-observation-suite.sh` 를 추가해 기존 `run-local-ops-baseline-suite.sh` child artifact를 재사용하고, `decision_class`, `collect_failed_jobs_in_window`, `collect_partial_success_jobs_in_window`, `open_collect_circuits`, `recommendation_review_gate`, `recommendation_real_user_traffic_gate_in_window`, `operator_reading`, `next_action` 을 `summary/json/note` artifact로 다시 publish하게 맞췄다. 동시에 [ops-observation-runbook.md](/home/ubuntu/youth-welfare/docs/core/ops-observation-runbook.md:1), [ops-baseline-runbook.md](/home/ubuntu/youth-welfare/docs/core/ops-baseline-runbook.md:1), [start.md](/home/ubuntu/youth-welfare/docs/start.md:1), [current-state.md](/home/ubuntu/youth-welfare/docs/current-state.md:1), [local-validation-docs-index.md](/home/ubuntu/youth-welfare/docs/core/local-validation-docs-index.md:1) 에 compact entrypoint를 연결했다.
...
- 해결: observation suite가 이제 `decision_class`, `reopen_allowed`, `operator_reading` 을 summary/json에 같이 남기고, `tmp/recommendation-observation/latest-recommendation-observation-note.md` 사람용 note artifact도 publish하게 바꿨다. 동시에 [recommendation-observation-runbook.md](/home/ubuntu/youth-welfare/docs/recommendation/recommendation-observation-runbook.md:1) 를 추가해 daily operator entrypoint와 상태 해석 ladder를 한 장으로 고정했다.
...
- 해결: [run-local-wrapper-duration-baseline.sh](/home/ubuntu/youth-welfare/deploy/performance/run-local-wrapper-duration-baseline.sh:1) 가 `recommendation_observation` 과 `current_priority` 자식 실행에 각각 전용 `ARTIFACT_DIR` 를 강제로 넘기도록 바꿨다. 이제 nested smoke cleanup은 자기 하위 디렉터리만 지우고, 부모 wrapper summary/json/tsv 는 그대로 남는다.
...
- 해결: [performance-docs-index.md](/home/ubuntu/youth-welfare/docs/performance/performance-docs-index.md:1) 와 [docs/performance/README.md](/home/ubuntu/youth-welfare/docs/performance/README.md:1) 를 추가하고, active 문서 진입점 네 곳에 모두 연결했다.
...
- 해결: [openai-runtime-contract.md](/home/ubuntu/youth-welfare/docs/core/openai-runtime-contract.md:1) 를 추가해 recommendation/chat/semantic retrieval/embedding rebuild 경로를 같은 형식으로 정리하고, [system-docs-index.md](/home/ubuntu/youth-welfare/docs/core/system-docs-index.md:1), [current-state.md](/home/ubuntu/youth-welfare/docs/current-state.md:1), [chatbot-plan.md](/home/ubuntu/youth-welfare/docs/core/chatbot-plan.md:1) 에 연결했다.
...
- 해결: [recommendation-current-state.md](/home/ubuntu/youth-welfare/docs/recommendation/recommendation-current-state.md:1), [recommendation-primary-audience-exclusion-decision-memo.md](/home/ubuntu/youth-welfare/docs/recommendation/recommendation-primary-audience-exclusion-decision-memo.md:1) 기준으로 recommendation prompt는 직접 수정하지 않고, `RealtimeAiGatewayTest` 에서 categorical-only user prompt와 stable system prompt 계약만 고정했다. 반대로 active lane인 chat 쪽만 `모르면 모른다고 답`, `지급/자격 단정 금지`, `확인 필요 조건 명시` 원칙을 프롬프트에 더 분명히 반영했다.
...
- 해결: Gov24 detail의 age fact는 새 code set을 만들지 않고 기존 `GOV24_SUPPORT_CONDITION` key를 재사용하도록 맞췄다. 동시에 [WelfareServiceMapper.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/collect/mapper/WelfareServiceMapper.java:457) 는 `supportTarget/selectionCriteria/supportContent` 를 다시 요약해 `만 39세 이하 청년` 같은 max-only youth 문구면 `18~39` range fact로 보정한다. 그 결과 [NormalizedPolicySidecarPersistenceIntegrationTest.java](/home/ubuntu/youth-welfare/backend/src/test/java/com/example/welfare/integration/NormalizedPolicySidecarPersistenceIntegrationTest.java:291) 기준 `gov24-details?sourceId=<서비스ID>` 경로가 뒤집힌 `min_age/max_age=40/39` row를 실제로 `18/39` 로 self-heal한다. source별 bounded repair 해석도 같이 고정했다: `복지로 -> bokjiro-details-refresh?sourceId`, `Gov24 -> gov24-details?sourceId`, `YOUTH -> sourceId 단건 repair lane 없음`.
...
- 문제: `인천형 청년월세 지원사업(3700)` local row를 보니 `min_age=35`, `max_age=34` 로 저장돼 있었다. 원문은 `경제적으로 어려움을 겪고 있는 인천 청년(35~39세)... 국가사업(...19~34세)...` 와 detail `35세～39세이하` 조합인데, [TextConstraintExtractor.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/collect/validation/TextConstraintExtractor.java:1) 가 full-width `～` range를 읽지 못하고 여러 age token을 전역 min/max로만 접으면 primary range와 참조 range가 뒤섞여 불가능한 조합이 나온다. 게다가 기존 [WelfareService.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/policy/entity/WelfareService.java:1) `applyDetailFallbacks()` 는 age 값이 이미 non-null이면 detail이 sane한 bound를 줘도 복구하지 못했다.
...
- 문제: [PoliciesPage.jsx](/home/ubuntu/youth-welfare/frontend/src/pages/PoliciesPage.jsx:1) 는 이미 카테고리/지역/소득 같은 active filter chip은 보여 주고 있었지만, 정작 가장 강한 조건인 `검색어` 와 결과 순서를 바꾼 `정렬` 은 summary 영역에 없었다. 이 상태에서는 결과가 비거나 너무 많을 때 “현재 어떤 조합이 적용 중인가”를 사용자가 위아래로 다시 훑어야 했다.
...
- 문제: [PolicyDetailPage.jsx](/home/ubuntu/youth-welfare/frontend/src/pages/PolicyDetailPage.jsx:1) 의 기존 `비슷한 정책` 섹션은 `unifiedCategory` 하나로만 후보를 읽었다. 카테고리 데이터가 성긴 정책이나 같은 분류 내 활성 정책 수가 적은 경우에는 rail이 비거나, 실제로는 같은 출처/같은 지역으로 이어서 볼 만한 정책이 있는데도 못 보여 주는 상태가 생긴다.
...
- 해결: [MyPage.jsx](/home/ubuntu/youth-welfare/frontend/src/pages/MyPage.jsx:1) 에 recent-view를 `기본 5건 요약 + 더보기/접기`, `전체 / 진행중만` 토글, empty CTA 구조로 정리했다. API는 여전히 최대 30건을 한 번에 읽되, 화면은 기본적으로 짧게 유지하고 필요할 때만 펼치는 쪽으로 UX 비용을 낮춘다.
...
- 해결: [PoliciesPage.jsx](/home/ubuntu/youth-welfare/frontend/src/pages/PoliciesPage.jsx:1) 에 검색창 `지우기` 액션, suggestion이 비었을 때 현재 입력어로 바로 검색하는 fallback row, 검색/필터 active 여부에 따라 달라지는 empty state CTA를 추가했다. 동시에 viewport width 기준으로 `sidebar stack`, `검색 버튼 full width`, `2열 카드 -> 1열` 전환을 넣어 기존 UI 패턴을 유지한 채 모바일에서도 무리 없이 읽히게 정리했다.
...
- 해결: 이번 단계에서는 검색엔진을 새로 들이지 않고 기존 [WelfareServiceSearchRepositoryImpl.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/policy/repository/WelfareServiceSearchRepositoryImpl.java:107) 의 `searchChatCandidates()` read 경계를 재사용해, suggestion 응답을 `정책 title 후보 우선 + search_logs 후보 보강` 순서로 합쳤다. `LinkedHashSet` 으로 dedupe한 뒤 limit을 적용해 API shape는 그대로 유지하면서도 실제 정책명 회수율을 먼저 올린다.

## 994) 검색과 자동완성의 입력 정규화가 다르면 사용자는 “검색은 되는데 자동완성은 안 뜬다”고 느끼게 된다
- 문제: 본 검색은 [SearchKeywordSupport.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/global/util/SearchKeywordSupport.java:1) 로 구두점/특수문자를 걷어내고 토큰화하지만, 자동완성이 별도 규칙으로 입력과 로그를 비교하면 `월세!`, `청년-지원` 같은 입력에서 검색 결과와 suggestion 결과가 서로 다른 뜻으로 동작한다.
...
- 해결: [PolicyTrafficRateLimitService.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/policy/service/PolicyTrafficRateLimitService.java:1) 에 `search / suggestion / trending / detail` 전용 bucket을 분리했다. 현재 기본값은 `search=60/60s`, `suggestion=180/60s`, `trending=30/60s` 이고, [PolicyController.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/policy/controller/PolicyController.java:141) 는 endpoint별로 각기 다른 checker를 탄다. 즉 permit-all 공개 계약은 유지하되, 타이핑 트래픽이 본 검색 한도를 잠식하지 않게 경계를 나눈다.
...
- 해결: [deploy/nginx/youth-welfare.conf](/home/ubuntu/youth-welfare/deploy/nginx/youth-welfare.conf:1) 와 [deploy/nginx/youth-welfare.bootstrap.conf](/home/ubuntu/youth-welfare/deploy/nginx/youth-welfare.bootstrap.conf:1) 의 server block에 `server_tokens off;` 를 추가했다. 다만 현재 로컬 작업 환경에는 host nginx가 없어서 템플릿 수정까지만 가능하고, 실제 반영 확인은 운영 서버에서 `sudo nginx -t`, `sudo systemctl reload nginx`, `curl -I https://youthmoa.kr/` 로 따로 닫아야 한다.

## 989) 컨테이너를 non-root + read-only로 바꿀 때는 Java/Redis가 실제로 쓰는 임시 경로를 먼저 열어 줘야 런타임이 안 깨진다
- 문제: [backend/Dockerfile](/home/ubuntu/youth-welfare/backend/Dockerfile:1) 을 non-root로 바꾸고 [docker-compose.prod.yml](/home/ubuntu/youth-welfare/docker-compose.prod.yml:1) 에 `read_only: true` 만 추가하면, Java는 기본 temp dir 사용 시 `/tmp` 쓰기가 필요하고 Redis도 런타임에 `/data` 와 `/tmp` 경로가 필요하다. 이걸 같이 안 열면 hardening 자체는 들어가도 컨테이너가 부팅 중 바로 실패할 수 있다.
...
- 문제: [AuthController](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/user/controller/AuthController.java:1) 는 이미 이메일 중복 확인, 이메일 인증 발송에는 fingerprint rate limit을 태우고 있었지만 `/api/auth/login`, `/api/auth/password-reset/request` 는 그대로 service로 내려보냈다. 계정 잠금과 이메일별 cooldown은 있어도, 같은 IP/UA fingerprint에서 여러 계정으로 로그인 시도하거나 여러 이메일로 reset request를 흩뿌리는 행위는 별도 클라이언트 축에서 막지 못했다.
- 해결: [AuthRateLimitService](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/user/service/AuthRateLimitService.java:1) 에 `auth:rate-limit:login:*`, `auth:rate-limit:password-reset-request:*` 키를 추가하고, [AuthController](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/user/controller/AuthController.java:1) 가 `ClientFingerprintService.build(request)` 후 이 제한을 먼저 검사하게 바꿨다. 기본값은 `login <= 20 / 300초`, `password-reset-request <= 5 / 300초` 로 두어 계정 잠금/이메일 cooldown은 그대로 유지하면서 클라이언트 축 방어만 보강한다.

## 987) admin 작업 상한은 “너무 큰 값 거부”만 볼 게 아니라 default 자체도 상한 안에 있는지 같이 맞춰야 한다
- 문제: [CollectAdminController](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/collect/controller/CollectAdminController.java:1) 의 `bokjiro-details-gap-fill` 는 원래 `maxCallsPerRound` default가 `10000` 이었다. 여기서 상한을 `1000` 으로만 추가하면, 파라미터를 생략한 정상 호출도 default 때문에 바로 `400` 이 되어 버린다.
...
- 문제: [RecommendationController](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/recommend/controller/RecommendationController.java:1) 의 `size` 만 검증하면 외부 HTTP 요청은 막을 수 있지만, 나중에 다른 service/scheduler/controller caller가 생기면 [RecommendationResultReadRepositoryImpl](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/recommend/repository/RecommendationResultReadRepositoryImpl.java:1) 의 `PageRequest.of(0, size)` 에 raw 값이 다시 내려갈 수 있다.
- 해결: controller에는 `@Min(1) @Max(100)` 를 걸어 oversized 요청을 `400 INVALID_INPUT(C001)` 으로 거부하고, [RecommendationAccessService](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/recommend/service/RecommendationAccessService.java:1) 에도 `normalizeSize()` 를 추가해 최종적으로 `1..100` 범위를 다시 clamp 한다. 즉 external contract와 internal safety net을 둘 다 둔다.

## 985) Web Push SSRF 경계는 “등록 시 검증”만으로 닫히지 않고, 기존 저장 subscription과 테스트 dispatch 권한까지 같이 좁혀야 한다
- 문제: [NotificationController](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/notification/controller/NotificationController.java:1) 의 `/push-test-send`, `/digest-test-dispatch`, `/deadline-test-dispatch` 는 일반 로그인 사용자에게 열려 있었고, [WebPushSubscriptionRequest](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/notification/dto/WebPushSubscriptionRequest.java:1) 는 endpoint 길이만 봤다. [WebPushSenderClientImpl](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/notification/service/WebPushSenderClientImpl.java:1) 는 저장된 endpoint로 실제 outbound 요청을 보내므로, 등록 시점만 막아도 이미 DB에 있던 unsafe endpoint는 계속 송신 시도 대상이 된다.
- 해결: 새 [WebPushEndpointPolicyService](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/notification/service/WebPushEndpointPolicyService.java:1) 를 추가해 `https` 강제, push provider host allowlist, DNS resolve 결과 private/loopback/link-local/multicast 차단을 한 곳에 모았다. 이 정책을 `WebPushSubscriptionCommandService.register()` 와 `WebPushSenderClientImpl.send()` 양쪽에 같이 적용해, 신규 등록도 막고 기존 저장 subscription도 송신 직전에 disable 할 수 있게 한다. 테스트 dispatch 3개는 [SecurityConfig](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/global/config/SecurityConfig.java:1) 에서 `ROLE_ADMIN` 으로만 열어 abuse surface를 줄이고, sender/dispatch warn 로그도 endpoint 전체 대신 host만 남기게 줄인다.
...
- 문제: revoke 동작은 이미 정확했지만, [AccessTokenRevocationService](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/user/service/AccessTokenRevocationService.java:1) 가 `access-revoked:{JWT원문}` 형식 key를 쓰고 있었다. Redis가 내부 저장소라 해도, key만 봐도 access token 원문이 드러나는 건 불필요한 노출이다.
...
- 해결: `web-push` 를 `5.1.2` 로 올리는 것만으로 끝내지 않고, [build.gradle](/home/ubuntu/youth-welfare/backend/build.gradle:1) 에 dependency constraint를 추가해 `async-http-client >= 2.15.0`, `jose4j >= 0.9.6` 을 명시적으로 고정한다. 이 프로젝트는 이미 `tomcat.version` override를 쓰고 있으므로, 보안 취약 전이 의존성도 같은 식으로 “실제 해석 버전 고정 + dependencyInsight 재검증” 흐름으로 닫는 편이 맞다.
...
- 해결: [policy-source-code-entrypoints.md](../policy/policy-source-code-entrypoints.md) 를 추가해 [CollectAdminController.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/collect/controller/CollectAdminController.java), [CollectService.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/collect/service/CollectService.java), [RawApiPayloadService.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/collect/service/RawApiPayloadService.java), [CollectItemSaver.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/collect/service/CollectItemSaver.java), [DeferredNormalizedPolicySidecarWriter.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/collect/normalization/DeferredNormalizedPolicySidecarWriter.java), [CanonicalRecommendationReadModelRepository.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/recommend/repository/CanonicalRecommendationReadModelRepository.java) 순서로 코드 진입점을 묶었다
...
- 해결: [cleanup-openai-replay-artifacts.sh](/home/ubuntu/youth-welfare/deploy/smoke/cleanup-openai-replay-artifacts.sh) 를 추가하고, 당시 `openai replay diagnostic lane plan` 문서에 `REPLAY_LOG_ROOT`, `SUMMARY_RETENTION_DAYS`, `ARTIFACT_RETENTION_DAYS`, `DRY_RUN` 계약과 실행 예시를 같이 고정했다
...
- 문제: 현재 앱 코드는 env key(`DB_URL`, `APP_PII_DB_URL`, `REDIS_HOST`)만 보면 되는데, 기존 [docker-compose.yml](/home/ubuntu/youth-welfare/docker-compose.yml:1) 은 `db` 컨테이너와 `docker-entrypoint-initdb.d` 자동 init을 전제로 한다. 이 상태로는 "코드를 EC2+RDS 형태로 바꿔야 하나?"라는 혼선이 계속 생긴다.
- 해결: 로컬 compose는 그대로 두고, 운영은 [docker-compose.prod.yml](/home/ubuntu/youth-welfare/docker-compose.prod.yml:1) 과 [env.production.example](/home/ubuntu/youth-welfare/env.production.example:1) 로 분리했다. 운영형 compose는 `app + redis` 만 띄우고 DB는 RDS endpoint를 직접 보게 하며, bootstrap은 [bootstrap-rds-runtime.sh](/home/ubuntu/youth-welfare/deploy/postgres/bootstrap-rds-runtime.sh:1) 로 `schema.sql -> runtime roles/grants -> patch` 순서를 EC2 shell에서 수행하게 고정했다.
...
- 문제: 기존 [WelfareServiceMapper.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/collect/mapper/WelfareServiceMapper.java:1) 는 `www.` 를 `https://` 로 보정하는 정도였고, 일부 source는 `RawFieldValidator.normalize(servDtlLink)` 값을 그대로 `detailUrl` 에 넣었다. 프론트 [PolicyDetailPage.jsx](/home/ubuntu/youth-welfare/frontend/src/pages/PolicyDetailPage.jsx:1) 도 `href` 와 `window.open()` 에 그 값을 거의 그대로 넘겼다. 이 상태에선 `javascript:`, `data:`, `intent:` 같은 비정상 scheme이 수집 데이터로 들어오면 렌더링/오픈 경계가 느슨해진다.
...
- 문제: [NotificationController.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/notification/controller/NotificationController.java:1) 는 공개 링크 `/api/notifications/unsubscribe` 에서 `jwtUtil.getSubjectAllowExpired(token)` 으로 userKey를 읽고 있었다. 이 메서드는 [JwtUtil.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/global/util/JwtUtil.java:1) 에서 `ExpiredJwtException` 이어도 claims에서 subject를 꺼내 반환하므로, 서명만 맞으면 만료된 unsubscribe 링크도 계속 성공한다.
...
- 해결: [SecurityConfig.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/global/config/SecurityConfig.java:89) 의 public 정책 조회 그룹에 두 endpoint를 명시적으로 추가했다.
...
- 해결: integration은 endpoint 대신 [PolicySearchKeywordReadRepository.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/policy/repository/PolicySearchKeywordReadRepository.java:12) 를 직접 호출하고, `windowStart` 를 짧게 잡아 fixture만 읽는 방식으로 바꿨다. HTTP contract는 별도 WebMvc 테스트로 유지했다.
...
- 문제: [policy-next-active-track-priority.md](/home/ubuntu/youth-welfare/docs/policy/policy-next-active-track-priority.md:1) 는 `Gov24 canonical promotion` 을 다음 active track처럼 서술하고 있었지만, 실제 코드와 [phase-plan.md](/home/ubuntu/youth-welfare/docs/phase-plan.md:1) 은 이미 label-first canonical 경계가 read-model/admin/presentation/test까지 닫힌 상태였다. 이 차이 때문에 “다음 로컬 작업”을 고를 때 이미 끝난 구현을 또 열어야 하는 것처럼 읽힐 수 있었다.
...
- 해결: 코드 경로를 다시 확인해 [PolicyViewLogCommandRepositoryImpl.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/policy/repository/PolicyViewLogCommandRepositoryImpl.java:1) 는 `INSERT ... ON CONFLICT DO UPDATE` 만, [UserRecentViewedPolicyReadService.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/user/service/UserRecentViewedPolicyReadService.java:1) 는 read만 쓰는 bounded surface임을 확정했다. 그 뒤 `app_core_rw` 에서 `recent_policy_views DELETE` 만 revoke하는 migration/patch를 추가하고, fresh init([z90-create-runtime-db-users.sh](/home/ubuntu/youth-welfare/deploy/postgres/init/z90-create-runtime-db-users.sh:1)), RDS bootstrap([bootstrap-rds-runtime.sh](/home/ubuntu/youth-welfare/deploy/postgres/bootstrap-rds-runtime.sh:1)), 운영 verify([verify-rds-runtime-privileges.sh](/home/ubuntu/youth-welfare/deploy/postgres/verify-rds-runtime-privileges.sh:1)) 까지 같이 맞췄다. 즉 이 경계는 전용 cleanup role을 늘리지 않고도 bounded revoke로 닫는 편이 더 단순하다.

## 1041) `recommendation_review_gate_promotion_approvals` 는 admin 전용 단일 테이블이라 별도 command role로 떼는 편이 bounded했다
- 문제: 이 테이블은 recommendation lane reopen 판단을 위한 admin-only approval record인데, 초기 migration/patch는 편의상 `app_core_rw` 에 `SELECT/INSERT/UPDATE/DELETE` 를 통째로 줬다. 하지만 실제 write 경로는 [AdminRecommendationReviewGatePromotionApprovalRecordService.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/admin/dashboard/service/AdminRecommendationReviewGatePromotionApprovalRecordService.java:1) 의 upsert/delete 두 경로뿐이고, read도 이미 admin read-only datasource를 타므로 primary runtime role에 남길 이유가 약했다.
- 해결: [RecommendationReviewGatePromotionApprovalCommandRepositoryImpl.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/admin/dashboard/repository/RecommendationReviewGatePromotionApprovalCommandRepositoryImpl.java:1) 를 `recommendation_review_gate_command_rw` 전용 datasource로 옮기고, `app_core_rw` 에서는 `recommendation_review_gate_promotion_approvals` `ALL` 권한을 제거했다. 동시에 fresh init/로컬 patch/RDS bootstrap/runtime verify/preflight/env example을 같이 맞춰, “코드는 secondary datasource를 기대하지만 실제 role/grant/login은 없는 drift” 까지 한 번에 닫는다.
...
- 문제: `recommendation_persistence_command_rw` 전용 datasource를 추가하면서 [RecommendationPersistenceCommandDataSourceConfig.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/global/config/RecommendationPersistenceCommandDataSourceConfig.java:1) 에 `recommendationPersistenceCommandTransactionManager` bean이 생겼다. 이 뒤 서버 signup/login sanity에서 `NoSuchBeanDefinitionException: No bean named 'transactionManager' available` 가 발생했고, 일반 `@Transactional` 서비스가 500으로 깨졌다.
- 해결: [PrimaryDataSourceConfig.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/global/config/PrimaryDataSourceConfig.java:1) 에 primary JPA transaction manager를 `@Bean(name = {"transactionManager", "primaryTransactionManager"})` + `@Primary` 로 명시했다. 이제 기본 JPA write/read 서비스는 다시 `transactionManager` 이름을 해석하고, bounded secondary command datasource는 자기 전용 transaction manager만 명시적으로 쓴다.
...
- 해결: fresh init([z90-create-runtime-db-users.sh](/home/ubuntu/youth-welfare/deploy/postgres/init/z90-create-runtime-db-users.sh:1)), RDS bootstrap([bootstrap-rds-runtime.sh](/home/ubuntu/youth-welfare/deploy/postgres/bootstrap-rds-runtime.sh:1)), runtime drift patch([V2026_05_30_06__grant_recommendation_persistence_user_key_select.sql](/home/ubuntu/youth-welfare/deploy/postgres/patches/V2026_05_30_06__grant_recommendation_persistence_user_key_select.sql:1)), verifier([verify-rds-runtime-privileges.sh](/home/ubuntu/youth-welfare/deploy/postgres/verify-rds-runtime-privileges.sh:1)) 기준을 모두 `SELECT(user_key)` bounded grant 포함 형태로 맞췄다.
...
- 문제: `frontend-browser-smoke` job은 `npm ci` 단계에서 먼저 실패하면 아직 `docker compose up` 을 하지 않았더라도 마지막 cleanup step의 `docker compose down -v` 는 그대로 실행된다. 이 저장소의 [docker-compose.yml](/home/ubuntu/youth-welfare/docker-compose.yml:1) `app` 서비스는 `env_file: .env` 를 참조하므로, CI workspace 루트에 `.env` 가 없으면 cleanup 자체가 또 한 번 빨갛게 남는다.
- 해결: [.github/workflows/ci.yml](/home/ubuntu/youth-welfare/.github/workflows/ci.yml:1) 에 `Prepare CI compose env file` step을 추가해 early stage에서 `touch .env` 를 수행하도록 했다. 이렇게 하면 install 단계가 먼저 실패해도 cleanup의 compose parse는 더 이상 `.env not found` 로 깨지지 않는다.
...
- 문제: `frontend-browser-smoke` job의 전역 `APP_BASE_URL` 은 Playwright와 Vite용 `http://127.0.0.1:5173` 이었다. 그런데 [bootstrap-playwright-smoke-data.sh](/home/ubuntu/youth-welfare/frontend/scripts/bootstrap-playwright-smoke-data.sh:1) 는 `APP_BASE_URL` 을 백엔드 API base와 health endpoint 기준으로 읽어 `HEALTH_URL=${APP_BASE_URL}/actuator/health` 를 만들기 때문에, CI에서 `127.0.0.1:5173/actuator/health` 를 60회 재시도하다가 실패했다.
- 해결: [.github/workflows/ci.yml](/home/ubuntu/youth-welfare/.github/workflows/ci.yml:1) 의 `Bootstrap frontend smoke data` step에만 `APP_BASE_URL=http://127.0.0.1:8082`, `HEALTH_URL=http://127.0.0.1:8082/actuator/health` 를 명시적으로 덮어썼다. 브라우저 smoke 자체는 계속 `PLAYWRIGHT_BASE_URL=5173`, `VITE_API_BASE_URL=8082` 계약을 유지한다.
...
- 문제: `frontend-browser-smoke` job은 fake Gmail credential로 backend를 띄우므로 `POST /api/auth/password-reset/request` 가 [PasswordResetService.java](/home/ubuntu/youth-welfare/backend/src/main/java/com/example/welfare/user/service/PasswordResetService.java:1) 내부 `EmailClient.send(...)` 실패 후 `A009` 로 끝난다. Playwright smoke는 그 응답을 무조건 200으로 기대해 query/hash reset-password 2개가 계속 깨졌다.
- 해결: [frontend/e2e/smoke.spec.js](/home/ubuntu/youth-welfare/frontend/e2e/smoke.spec.js:1) 의 `issuePasswordReset()` 은 기본적으로 real API request를 유지하되, 응답이 `A009` 일 때만 새 [issue-password-reset-token.sh](/home/ubuntu/youth-welfare/frontend/scripts/issue-password-reset-token.sh:1) helper로 Redis에 reset token을 직접 seed해서 브라우저 flow를 계속 검증하게 바꿨다.
