...
이 스크립트는 `admin login -> ROLE_ADMIN 확인 -> /api/admin/dashboard/recommendation-breakdowns -> traffic mix + real-user traffic gate + fallback/click/repeat exposure userCohort 계약` 을 한 번에 확인합니다.
