...
계정 잠금 임계치/잠금 응답 반복 검증은 아래 스크립트를 우선 사용합니다.

```bash
deploy/smoke/run-local-account-lockout-smoke.sh
```

이 스크립트는 `signup -> wrong password login x5(401/A004) -> users/auth_users login_fail_count=5 + locked_until set 확인 -> correct password login while locked(401/A005)` 를 한 번에 확인합니다.
기본 임계치는 현재 구현과 같은 `5` 이고, 필요하면 `LOCK_THRESHOLD` 로 덮어쓸 수 있습니다.
...
이 스크립트는 `admin login -> ROLE_ADMIN 확인 -> /api/admin/dashboard/summary -> summary window + trend window + recommendation weight progress + recommendation traffic mix + real-user traffic gate + recommendationReviewGate + latest batch concentration snapshot(top1LeaderUserMix/top1LeaderSignalSummary 포함) + reviewGateStaleness + recentWindowLatestBatch + reviewGatePolicyCandidateStatus/reviewGatePolicyCandidateReason + reviewGatePolicyPromotionStatus/reviewGatePolicyPromotionReason + reviewGatePolicyPromotionActionStatus/reviewGatePolicyPromotionActionReason + reviewGatePolicyPromotionReadinessStatus/reviewGatePolicyPromotionReadinessReason + reviewGatePolicyPromotionExecutionStatus/reviewGatePolicyPromotionExecutionReason + reviewGatePolicyPromotionApprovalStatus/reviewGatePolicyPromotionApprovalReason + reviewGatePolicyPromotionApprovalCriteriaStatus/reviewGatePolicyPromotionApprovalCriteriaReason + reviewGatePolicyPromotionApprovalDecisionStatus/reviewGatePolicyPromotionApprovalDecisionReason + reviewGatePolicyPromotionApprovalRecordStatus/reviewGatePolicyPromotionApprovalRecordReason 계약` 을 한 번에 확인합니다.
...
이 스크립트는 `admin login -> ROLE_ADMIN 확인 -> /api/admin/dashboard/recommendation-breakdowns -> traffic mix + real-user traffic gate + recommendationReviewGate + latest batch concentration snapshot(top1LeaderUserMix/top1LeaderSignalSummary 포함) + reviewGateStaleness + recentWindowLatestBatch + reviewGatePolicyCandidateStatus/reviewGatePolicyCandidateReason + reviewGatePolicyPromotionStatus/reviewGatePolicyPromotionReason + reviewGatePolicyPromotionActionStatus/reviewGatePolicyPromotionActionReason + reviewGatePolicyPromotionReadinessStatus/reviewGatePolicyPromotionReadinessReason + reviewGatePolicyPromotionExecutionStatus/reviewGatePolicyPromotionExecutionReason + reviewGatePolicyPromotionApprovalStatus/reviewGatePolicyPromotionApprovalReason + reviewGatePolicyPromotionApprovalCriteriaStatus/reviewGatePolicyPromotionApprovalCriteriaReason + reviewGatePolicyPromotionApprovalDecisionStatus/reviewGatePolicyPromotionApprovalDecisionReason + reviewGatePolicyPromotionApprovalRecordStatus/reviewGatePolicyPromotionApprovalRecordReason + topRepeatedServices/top1Services(userMix 포함) + fallback/click/repeat exposure userCohort 계약` 을 한 번에 확인합니다.
...
3. account lockout smoke
4. withdraw smoke
5. admin forced logout smoke
...
현재 `auth/session smoke wrapper` 내부 순서는 `runtime logout -> login failure tracking -> account lockout -> withdraw -> admin forced logout` 입니다.
