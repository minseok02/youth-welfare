...
이 스크립트는 `admin login -> ROLE_ADMIN 확인 -> /api/admin/dashboard/summary -> summary window + trend window + recommendation weight progress + recommendation traffic mix + real-user traffic gate 계약` 을 한 번에 확인합니다.
...
이 스크립트는 `admin login -> ROLE_ADMIN 확인 -> /api/admin/dashboard/recommendation-breakdowns -> traffic mix + real-user traffic gate + latest batch concentration snapshot + fallback/click/repeat exposure userCohort 계약` 을 한 번에 확인합니다.
