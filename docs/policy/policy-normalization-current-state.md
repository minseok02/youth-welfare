...
[V2026_06_01_01__seed_gov24_taxonomy_codes.sql](../../backend/src/main/resources/db/migration/V2026_06_01_01__seed_gov24_taxonomy_codes.sql)
에 seed 되어 있고,
[Gov24TaxonomyCodeSupport.java](../../backend/src/main/java/com/example/welfare/collect/support/Gov24TaxonomyCodeSupport.java)
...
남은 것은 이 값을 public UX나 rule 소비로 넓힐지 여부뿐이다.
그 다음 후보 `mrgSttsCd` 는 signal이 `71건` 으로 작지만,
multi-code가 없고 `기혼/미혼/제한없음` scalar 의미가 명확해서 가장 좁고 안전한 official fact 후보다.
...
서버 `7ca6a6c` 검증에서도 이 경계는 그대로 닫혔다.
`GET /api/policies?sourceType=YOUTH&page=0&size=10&sort=LATEST&statusFilter=ALL` summary 응답은
예시 `serviceId=4686` 에 대해
`youthIncomeConditionTypeLabel=기타`,
...
`youthMaritalStatusLabel=제한없음`
을 실제로 반환했다.
프론트는 `npm run build` 뒤 Nginx가 최신 `PoliciesPage` 번들을 서빙하는 것이 확인됐고,
같은 raw label 조합에서 card badge 계산 결과는
`['소득 기타', '재직자', '중소기업']`
처럼 `제한없음`, `무관` 을 숨기고 실제 신호만 최대 3개 남기는 형태였다.
...
- `/api/policies`, `/api/policies/search` 는 `gov24UserType` query param도 받는다.
...
- recommendation scoring도 이제 같은 canonical term/projection truth를 soft signal로 읽는다.
  ...
  - [DefaultPriorityMatcherTest.java](../../backend/src/test/java/com/example/welfare/recommend/service/DefaultPriorityMatcherTest.java) 도 `gov24ServiceFieldLabel/gov24UserTypeTokens/gov24BenefitTypeTokens` 만으로는 matcher가 열리지 않는다는 회귀를 고정한다.
