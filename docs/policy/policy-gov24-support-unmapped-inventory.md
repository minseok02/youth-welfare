...
1. 현재 남은 `979`건 gap이 어떤 code 군집 때문에 생기는지 고정
...
반대로 현재 missing fact 집합에는 아래 개인 eligibility 축이 사실상 남지 않습니다.
...
3. 이 축은 지금 제품의 사용자 입력, 우선순위 matcher, 추천 score에서
   직접 소비하는 경로가 거의 없습니다.
4. 남아 있는 개인 eligibility 잔여(`JA0313~JA0316`)는 서비스 수가 `2/1/1/1` 수준으로 매우 작고,
   `JA0322`, `JA0410` 은 `해당사항없음` 이라 positive fact로 올릴 실익이 낮습니다.
...
- `JA0313~JA0316` 농/어/축/임업인: 현재 gap 설명력 기준으로는 low-priority deferred
- `JA0322`, `JA0410` 해당사항없음: positive fact 승격 대상 아님
