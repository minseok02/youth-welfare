...
- 2026-06-21 프론트 deployed-origin 기준: `FRONTEND_PUBLIC_BASE_URL=https://youthmoa.kr` 관측 스위트에서 사용자 Playwright smoke 31개가 통과했고, admin 포함 재실행에서는 `RUN_FRONTEND_ADMIN_E2E=true`, deployed-origin 기준 Playwright 52개가 통과했다. 최신 admin 포함 artifact는 `tmp/frontend-observation/20260621T135933Z`, `decision_class=BASELINE_HEALTHY` 다.
- 2026-06-21 운영 관측 후속 기준: `run-local-ops-observation-suite.sh` 최신 artifact `tmp/ops-observation/20260621T135553Z` 는 `BASELINE_HEALTHY`, collect 실패/partial/circuit `0`, policy duplicate/link open queue `0`, chat observability `CHAT_BASELINE_HEALTHY` 로 통과했다. 이 실행에서 stale notification target 경고가 다시 보여 `RECOMMENDATION_DIGEST` `/policies/3324` 2건을 확인했고, 모두 `EXAMPLE_SMOKE` 라서 `hide-stale` 로 bounded 정리했다. 재검증 artifact `tmp/notification-stale-target-audit/20260621T135828Z` 는 `stale_14d_total=0`, `NO_STALE_TARGETS`, attention feed는 warning `0` 이다. 남은 알림은 `unread_total=26`, `stale_unread_7d=14`, `stale_unread_14d=0` 의 최근 digest tail로 관찰한다.
