...
- 2026-06-27 AWS/RDS 운영 확인 blocker: 현재 EC2 실행 역할 `youth-welfare-ops-monitor-v2-role` 은 실제 AWS IAM에서 `iam:PutRolePolicy` 와 `rds:DescribeDBInstances` 가 허용되지 않아 RDS backup retention/latest restorable time/deletion protection/encryption/Multi-AZ/public access 확인과 restore rehearsal을 진행할 수 없다. repo의 `deploy/ops/aws-ops-monitor-role-policy.json` 에는 `rds:DescribeDBInstances` 가 포함되어 있으므로 privileged AWS actor가 실제 역할 정책을 적용한 뒤 [db-backup-restore-rehearsal-runbook.md](core/db-backup-restore-rehearsal-runbook.md) 순서로 이어간다.
- 2026-06-29 운영 closeout 기준: 정책 오류 제보 OPEN 5건은 모두 GOV24 중앙부처/공공기관 정책의 지역명 동형어/일반명사 오탐으로 review 처리했고 `service_regions` 는 전국 노출 상태로 유지했다. 오래된 추천 saved batch 지역 mismatch 10명은 bounded refresh repair로 갱신해 최신 `run-local-recommendation-region-mismatch-audit.sh` 기준 `affected_users=0`, `mismatch_rows=0` 이다. `/policies/3324` 의 14일 초과 `RECOMMENDATION_DIGEST` unread 4건은 admin `hide-stale olderThanDays=14` 로 정리했고, 최신 반복 감사 기준 알림은 `unread_total=20`, `unread_digest=20`, `stale_unread_7d=12`, `stale_unread_14d=0`, failed notification `0`, `stale_14d_total=0`, `NO_STALE_TARGETS` 다. 표준코드는 `total_users=1251`, `users_with_any_standard_code=592`, `users_missing_all_standard_codes=659`, `safe_reconcile_candidate_rows=0`, `conflicting_value_gap_rows=0` 이라 자동 DB 보정 대상이 아니다. 추천은 읽기성 gate/adoption/region 감사 기준 `KEEP_BASELINE_MONITORING`, `WAIT_FOR_REAL_USER_TRAFFIC`, 최신 배치 `707`명 중 표준코드 있음 `591`, 없음 `116`, adoption share `83.59%` 다. policy triage는 `policy_error_open_reports=0`, `policy_duplicate_open_groups=0`, `policy_link_open_reviews=0`, `REVIEW_QUEUE_CLOSED_RAW_BACKLOG_REMAINS` 이며 attention feed는 warning `0`, 정보성 `standard-code-backlog`, `notification-backlog` 만 남는다. RDS backup/restore rehearsal은 2026-06-29 09:06 UTC 재확인에서도 현재 role의 `rds:DescribeDBInstances` `AccessDenied` 로 막혀 있다.
...
1. privileged AWS actor가 `youth-welfare-ops-monitor-v2-role` 에 repo의 `deploy/ops/aws-ops-monitor-role-policy.json` 를 반영해야 합니다. 이 전에는 `rds:DescribeDBInstances` 가 막혀 RDS 백업 설정 확인과 restore rehearsal을 진행할 수 없습니다.
2. IAM 적용 뒤 `aws rds describe-db-instances --region ap-northeast-2 --db-instance-identifier youth-welfare-prod-db` 로 backup retention, latest restorable time, deletion protection, encryption, Multi-AZ, public access를 확인하고 [db-backup-restore-rehearsal-runbook.md](core/db-backup-restore-rehearsal-runbook.md) 의 별도 restore DB 리허설로 넘어갑니다.
3. 다음 실제 nightly cron 뒤 `/var/log/youth-welfare/nightly-ops-handoff/nightly-summary-YYYY-MM-DD.log` 와 `artifacts/<UTC timestamp>/operational-db-audit.out` 를 확인합니다. 기대값은 `auth=passed`, `db_audit=ok`, auth smoke의 `smoke_trusted_origin=https://youthmoa.kr`, DB alert evaluator의 `OP_ALERT_STATUS=ok` 입니다.
4. 다음 작업 시작 시 server/RDS와 app runtime env를 먼저 맞춥니다. runtime read-only smoke는 `ENV_FILE=.env.runtime.production SMOKE_DB_MODE=postgres APP_BASE_URL='http://127.0.0.1:8082'` 로 맞추고, migration/admin DB bootstrap이 필요한 active baseline은 `ENV_FILE=.env.production` 을 씁니다. 관리자 password가 없으면 `ALLOW_ADMIN_JWT_MINT=true` 를 명시합니다.
5. `run-local-ops-observation-suite.sh` 를 먼저 돌려 attention warning이 다시 생겼는지 확인합니다. 최신 전체 ops 기준선은 `tmp/ops-observation/20260629T081122Z` 의 `BASELINE_HEALTHY`, attention warning `0`, attention item `standard-code-backlog,notification-backlog` 입니다. 이후 반복 read-only 재확인에서도 표준코드 bounded 보정 후보와 conflict gap은 모두 `0` 입니다.
6. attention에 남는 `standard-code-backlog`, `notification-backlog` 는 현재 관찰 항목입니다. 표준코드는 `safe_reconcile_candidate_rows` 또는 `conflicting_value_gap_rows` 가 생길 때만 직접 보정하고, 알림은 `stale_14d_total > 0` 또는 failed queue가 생길 때만 bounded hide/retry를 검토합니다. 2026-06-29 최신 알림 기준은 `unread_total=20`, `stale_unread_7d=12`, `stale_unread_14d=0`, failed notification `0`, `NO_STALE_TARGETS` 입니다.
...
8. 추천은 아직 reopen 대상이 아닙니다. 최신 읽기성 gate 기준은 `KEEP_BASELINE_MONITORING`, `WAIT_FOR_REAL_USER_TRAFFIC` 이며, promotion/review run은 실행하지 않습니다. 표준코드 미입력 backlog의 1차 UX/입력 유도는 PR #352로 배포했으므로, 다음에는 실제 입력률 변화가 생기는지 `users_missing_all_standard_codes=659`, 최신 배치 adoption share `83.59%`, 지역 mismatch `0` 을 관찰합니다.
9. 정책 데이터는 운영 review queue가 다시 열릴 때만 처리합니다. raw duplicate/link 잔량은 관찰값으로 두고, `policy_duplicate_open_groups` 또는 `policy_link_open_reviews` 가 1 이상이면 `policy-data-quality-triage-runbook.md` 순서로 review합니다.
10. 지역/좌표 이슈는 현재 닫힌 상태입니다. 새 smoke나 seed가 `sido/sgg` 있음 + `region_code` 공백을 다시 만들면 해당 smoke payload부터 고치고, 운영 row는 `RegionCodeUtil` 매핑으로 bounded 보정합니다.
