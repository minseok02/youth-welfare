...
- 2026-06-27 server/RDS active baseline 기준: compose app이 `.env.runtime.production` runtime/RDS DB를 쓰는 상태라 smoke도 `ENV_FILE=.env.runtime.production SMOKE_DB_MODE=postgres APP_BASE_URL='http://127.0.0.1:8082' ALLOW_ADMIN_JWT_MINT=true` 로 맞춰 실행한다. 최신 artifact `tmp/active-baseline-suite/20260627T123051Z` 는 backend test, frontend lint/build/E2E `33 passed`, ops baseline/observation `BASELINE_HEALTHY`, collect legacy repair까지 `active_baseline_suite=passed` 다. 추가 단독 smoke도 collect governance, notification channel, policy quality, chat application coaching matrix 모두 통과했다.
- 2026-06-27 표준코드 bounded 보정 기준: `house_tenure_code=NONE` 인 실제 사용자 1건에서 `housing_type_code=4` 가 남아 있어 `users` 와 `user_profiles` 를 각각 `housing_type_code=NONE` 으로 bounded 보정했다. 최신 user profile standard-code coverage audit은 `users_house_tenure_none_housing_type_mismatch=0`, `profiles_house_tenure_none_housing_type_mismatch=0`, `safe_reconcile_candidate_rows=0`, `conflicting_value_gap_rows=0`, `users_with_any_standard_code=462`, `users_missing_all_standard_codes=626`, `non_example_users_missing_all_standard_codes=5` 로 닫혔다. attention feed에는 `standard-code-backlog`, `notification-backlog`, `notification-stale-backlog` 3개가 남고 warning은 `standard-code-backlog` 1개지만, 현재 자동 보정 후보나 conflict gap은 없다.
...
1. 다음 작업 시작 시 server/RDS와 app runtime env를 먼저 맞춥니다. 현재 compose app 기준 smoke env는 `ENV_FILE=.env.runtime.production SMOKE_DB_MODE=postgres APP_BASE_URL='http://127.0.0.1:8082'` 이고, 관리자 password가 없으면 `ALLOW_ADMIN_JWT_MINT=true` 를 명시합니다.
2. `run-local-ops-observation-suite.sh` 를 먼저 돌려 attention warning이 다시 생겼는지 확인합니다. 현재 기준선은 `BASELINE_HEALTHY`, collect 실패/partial/circuit `0`, attention warning `1` 입니다. 이 warning은 표준코드 미입력 backlog 신호이며, bounded 보정 후보와 conflict gap은 모두 `0` 입니다.
3. attention에 남는 `standard-code-backlog`, `notification-backlog`, `notification-stale-backlog` 는 현재 관찰 항목입니다. 표준코드는 `safe_reconcile_candidate_rows` 또는 `conflicting_value_gap_rows` 가 생길 때만 직접 보정하고, 알림은 `stale_14d_total > 0` 또는 failed queue가 생길 때만 bounded hide/retry를 검토합니다.
...
5. 추천은 아직 reopen 대상이 아닙니다. `reopen_precheck_status=KEEP_OBSERVING`, real-user sample thin, `EXAMPLE_SMOKE_ONLY_LEADER` 기준이 바뀌는지 `run-local-recommendation-observation-suite.sh` 로 관찰합니다.
...
- target cluster triage는 `bash deploy/smoke/run-local-notification-stale-target-audit.sh` 로 다시 읽습니다.
  ...
  - 현재 latest 기준은 `stale_14d_total=0`, `decision_class=NO_STALE_TARGETS` 이고, 남은 unread backlog는 `unread_total=26`, `stale_unread_7d=14`, `stale_unread_14d=0` 수준의 recommendation digest tail 입니다.
  - server/RDS 최신 sample 기준 unread는 `digest=26`, `deadline=0`, `system=0`, failed notification은 `0` 입니다.
