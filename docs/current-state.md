...
- 복지로 list 수집은 현재 개발 계정으로도 운영 전 초기 snapshot 확보가 가능하지만, 복지로 detail은 개발 계정 트래픽 한도(`100`) 때문에 하루 안에 full coverage를 채우는 것을 목표로 두지 않습니다.
- 복지로 detail backlog 는 운영 서버 오픈 후 운영 계정/증설 quota 확보 전까지는 의도된 제한 상태로 유지하고, 그 전에는 `gap fill` 로 점진 채움만 수행합니다.
