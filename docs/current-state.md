...
- 2026-06-21 프론트 deployed-origin 기준: `FRONTEND_PUBLIC_BASE_URL=https://youthmoa.kr` 관측 스위트에서 사용자 Playwright smoke 31개가 통과했고, admin 포함 재실행에서는 `RUN_FRONTEND_ADMIN_E2E=true`, deployed-origin 기준 Playwright 52개가 통과했다. 최신 admin 포함 artifact는 `tmp/frontend-observation/20260621T141819Z`, `decision_class=BASELINE_HEALTHY` 다. 이후 관측 wrapper와 Playwright bootstrap은 admin E2E가 켜졌는데 admin email/password가 없으면 기본 `admin@example.com/password123!` 로 조용히 떨어지지 않고 실패하도록 tightened 했다. 기본 admin credential은 `ALLOW_DEFAULT_ADMIN_CREDENTIALS=true` 를 명시한 로컬 smoke에서만 허용한다.
