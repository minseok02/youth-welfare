...
- 최근 보안 점검 follow-up과 운영 반영 상태는 [core/security-hardening-current-state.md](./core/security-hardening-current-state.md) 를 active source of truth로 먼저 봅니다.
- 다만 `EC2 + RDS` 전환을 실제로 준비할 때 바로 쓸 수 있도록 운영용 compose/env/bootstrap/run smoke 순서는 [deployment.md](./deployment.md) 에 따로 정리했습니다. 평소 active main track으로 읽지는 않고, 운영 전환을 명시적으로 진행할 때만 봅니다.
...
- `2026-05-25` 기준 보안 follow-up active truth는 다음 네 줄로 압축합니다.
  - dependency hardening: `Tomcat 10.1.55`, `pgjdbc 42.7.11`, `Bouncy Castle 1.84`
  - logout/session: `presented_after_logout=401/A006`, `older_login_token_after_logout=401/A006`
  - edge nginx template: upstream 보안 헤더 hide + edge 단일 소유 + `/actuator/` 내부만 허용
  - admin forced logout smoke: `ADMIN_PASSWORD` 또는 `/tmp/youth-welfare-admin-smoke-password` 필요, 없으면 명확한 fail-fast 메시지 출력
...
- `2026-05-22` 기준 broad quality 재검사도 다시 green 입니다. `run-local-validation-from-env.sh --full`, `run-local-ops-baseline-suite.sh`, `run-local-policy-quality-summary.sh`, `run-local-gov24-quality-audit.sh`, `run-local-gov24-recommendation-suite.sh` 를 다시 태웠고, quality gate와 Gov24 runtime baseline 모두 유지됐습니다.
- `2026-05-22` 기준 recommendation live current truth는 `2026-05-20` 시점 문서값과 다릅니다. 최신 local ops baseline에서는 `recommendation_historical_example_dominance_detected=false`, `review_gate_policy_candidate_status=NOT_A_CANDIDATE_NO_HISTORICAL_EXAMPLE_DOMINANCE`, `review_gate_policy_promotion_status=KEEP_PRIMARY_BASELINE`, `review_gate_policy_promotion_readiness_status=NOT_READY_FOR_BOUNDED_PROMOTION_REVIEW`, `review_gate_policy_promotion_execution_status=DO_NOT_RUN_BOUNDED_PROMOTION_REVIEW` 로 읽는 편이 맞습니다. 아래쪽에 남아 있는 `RECENT_WINDOW_POLICY_CANDIDATE`, `READY_FOR_BOUNDED_PROMOTION_REVIEW`, `AWAIT_EXPLICIT_POLICY_REVIEW_DECISION` 류 값은 `2026-05-20` historical snapshot 기록으로만 읽습니다.
- `2026-05-22` 기준 Gov24 collect/runtime 품질은 유지됐고, bounded retrieval 완화 뒤 `run-local-gov24-education-signal-smoke.sh` 도 `gov24_rows=8`, `gov24_top10_rows=3`, `gov24_top2_rows=1` 로 회복됐습니다. 현재 active target은 `16490 국가장학금 Ⅰ유형 (학생직접지원형)` 이고, 예전 `7193` 은 이제 `지역화폐(강남사랑상품권)` 으로 들어와 있어 education signal target으로는 더 이상 쓰지 않는 편이 맞습니다.
...
- 같은 `2026-05-20` 기준으로 이 해석은 이제 wrapper/runbook 설명에만 머물지 않고 admin API surface에도 올라왔습니다. `GET /api/admin/dashboard/summary`, `GET /api/admin/dashboard/recommendation-breakdowns` 는 이제 full latest batch gate(`recommendationReviewGate`) 외에 `recentWindowLatestBatch`, `reviewGateStaleness`, `recentWindowRecommendationReviewReading`, `historicalExampleDominanceDetected`, `reviewGatePolicyCandidateStatus`, `reviewGatePolicyCandidateReason`, `reviewGatePolicyPromotionStatus`, `reviewGatePolicyPromotionReason`, `reviewGatePolicyPromotionActionStatus`, `reviewGatePolicyPromotionActionReason`, `reviewGatePolicyPromotionReadinessStatus`, `reviewGatePolicyPromotionReadinessReason`, `reviewGatePolicyPromotionExecutionStatus`, `reviewGatePolicyPromotionExecutionReason`, `reviewGatePolicyPromotionApprovalCriteriaStatus`, `reviewGatePolicyPromotionApprovalCriteriaReason`, `reviewGatePolicyPromotionApprovalStatus`, `reviewGatePolicyPromotionApprovalReason`, `reviewGatePolicyPromotionApprovalDecisionStatus`, `reviewGatePolicyPromotionApprovalDecisionReason`, `reviewGatePolicyPromotionApprovalRecordStatus`, `reviewGatePolicyPromotionApprovalRecordReason`, `reviewGatePolicyPromotionReviewRunStatus`, `reviewGatePolicyPromotionReviewRunReason`, `reviewGatePolicyPromotionReviewRunCriteriaStatus`, `reviewGatePolicyPromotionReviewRunCriteriaReason`, `reviewGatePolicyPromotionReviewRunDecisionStatus`, `reviewGatePolicyPromotionReviewRunDecisionReason`, `reviewGatePolicyPromotionReviewRunApprovalCriteriaStatus`, `reviewGatePolicyPromotionReviewRunApprovalCriteriaReason`, `reviewGatePolicyPromotionReviewRunApprovalDecisionStatus`, `reviewGatePolicyPromotionReviewRunApprovalDecisionReason`, `reviewGatePolicyPromotionReviewRunApprovalStatus`, `reviewGatePolicyPromotionReviewRunApprovalReason`, `reviewGatePolicyPromotionReviewRunApprovalRecordStatus`, `reviewGatePolicyPromotionReviewRunApprovalRecordReason` 를 같이 내려, historical example inertia와 recent current-live signal, 그리고 “후보이지만 아직 explicit policy change review가 필요하고 현재 action은 keep primary baseline, bounded promotion review prerequisite은 이미 충족됐지만 실행은 explicit review decision 대기, approval criteria는 이미 충족됐지만 approval/approval decision 기록은 아직 없고 approval record도 아직 pending, actual bounded review run도 아직 pending이지만 review run criteria 자체는 이미 ready이고 마지막 review run decision도 approval record 미작성 때문에 아직 pending이며 마지막 review run approval criteria도 이미 ready이고 마지막 review run approval decision도 아직 pending이고 마지막 review run approval도 아직 pending이며 마지막 review run approval record도 아직 pending”이라는 상태를 한 응답에서 함께 읽게 합니다. 최신 local smoke 기준 값은 `recommendation_review_gate=DEFERRED_NON_REAL_LEADER_SIGNAL`, `recommendation_review_gate_primary_reference_mode=ALL_TIME_LATEST_PER_USER`, `recommendation_review_gate_example_target_top1_users=272`, `recommendation_review_gate_example_target_top1_last_24h=0`, `recommendation_review_gate_real_user_latest_users=80`, `recommendation_review_gate_real_user_target_top1_users=0`, `recommendation_recent_window_review_reading=RECENT_WINDOW_CLEARS_HISTORICAL_2622_DOMINANCE`, `recommendation_historical_example_dominance_detected=true`, `recommendation_review_gate_policy_candidate_status=RECENT_WINDOW_POLICY_CANDIDATE`, `recommendation_review_gate_policy_promotion_status=REQUIRES_EXPLICIT_POLICY_CHANGE_REVIEW`, `recommendation_review_gate_policy_promotion_action_status=KEEP_PRIMARY_BASELINE`, `recommendation_review_gate_policy_promotion_readiness_status=READY_FOR_BOUNDED_PROMOTION_REVIEW`, `recommendation_review_gate_policy_promotion_execution_status=AWAIT_EXPLICIT_POLICY_REVIEW_DECISION`, `recommendation_review_gate_policy_promotion_approval_criteria_status=READY_FOR_EXPLICIT_PROMOTION_APPROVAL`, `recommendation_review_gate_policy_promotion_approval_status=PENDING_EXPLICIT_PROMOTION_APPROVAL`, `recommendation_review_gate_policy_promotion_approval_decision_status=AWAIT_EXPLICIT_PROMOTION_APPROVAL_DECISION`, `recommendation_review_gate_policy_promotion_approval_record_status=PENDING_EXPLICIT_PROMOTION_APPROVAL_RECORD`, `recommendation_review_gate_policy_promotion_review_run_status=PENDING_BOUNDED_PROMOTION_REVIEW_RUN`, `recommendation_review_gate_policy_promotion_review_run_criteria_status=READY_FOR_BOUNDED_PROMOTION_REVIEW_RUN`, `recommendation_review_gate_policy_promotion_review_run_decision_status=AWAIT_BOUNDED_PROMOTION_REVIEW_RUN_DECISION`, `recommendation_review_gate_policy_promotion_review_run_approval_criteria_status=READY_FOR_BOUNDED_PROMOTION_REVIEW_RUN_APPROVAL`, `recommendation_review_gate_policy_promotion_review_run_approval_decision_status=AWAIT_BOUNDED_PROMOTION_REVIEW_RUN_APPROVAL_DECISION`, `recommendation_review_gate_policy_promotion_review_run_approval_status=PENDING_BOUNDED_PROMOTION_REVIEW_RUN_APPROVAL`, `recommendation_review_gate_policy_promotion_review_run_approval_record_status=PENDING_BOUNDED_PROMOTION_REVIEW_RUN_APPROVAL_RECORD` 입니다.
- 같은 ladder에는 `reviewGatePolicyPromotionReviewRunApprovalRecordTransitionStatus`, `reviewGatePolicyPromotionReviewRunApprovalRecordTransitionReason`, `reviewGatePolicyPromotionReviewRunApprovalRecordWriteStatus`, `reviewGatePolicyPromotionReviewRunApprovalRecordWriteReason` 도 추가됐습니다. 현재 local 기준 값은 `AWAIT_BOUNDED_PROMOTION_REVIEW_RUN_APPROVAL_RECORD_WRITE`, `APPROVAL_RECORD_CRITERIA_MET_BUT_RECORD_NOT_WRITTEN`, `PENDING_BOUNDED_PROMOTION_REVIEW_RUN_APPROVAL_RECORD_WRITE`, `APPROVAL_RECORD_TRANSITION_READY_BUT_WRITE_NOT_EXECUTED` 입니다.
- `2026-05-20` 기준 explicit promotion approval record write path도 실제로 연결됐습니다. `POST/DELETE /api/admin/dashboard/recommendation-review-gate/promotion-approval-record` 와 `run-local-admin-recommendation-review-gate-promotion-approval-record-smoke.sh` 를 통해 baseline pending tuple(`AWAIT_EXPLICIT_PROMOTION_APPROVAL_DECISION / PENDING_EXPLICIT_PROMOTION_APPROVAL_RECORD / PENDING_BOUNDED_PROMOTION_REVIEW_RUN / PENDING_BOUNDED_PROMOTION_REVIEW_RUN_APPROVAL_RECORD_WRITE`) -> approved tuple(`APPROVED_FOR_BOUNDED_PROMOTION_REVIEW / EXPLICIT_PROMOTION_APPROVAL_RECORDED / AWAIT_BOUNDED_PROMOTION_REVIEW_RUN / BOUNDED_PROMOTION_REVIEW_RUN_APPROVAL_RECORD_WRITE_COMPLETED`) -> clear 뒤 baseline 복귀를 실제로 확인했습니다. 기존 로컬 PostgreSQL volume에서는 table/grant가 없을 수 있으므로, relation missing 또는 permission denied가 보이면 `bash deploy/postgres/apply-local-runtime-schema-patch.sh` 를 먼저 실행하는 편이 맞습니다.
- 같은 `2026-05-20` 기준으로 이제 상태값을 더 쪼개기보다, 실제 bounded promotion review go/no-go를 한 번에 보는 wrapper도 붙었습니다. `run-local-recommendation-bounded-promotion-review.sh` 는 approval record write/clear preflight, recent-window clear 여부, historical example staleness를 같이 읽고 `bounded_promotion_review_result_status` 만 요약합니다. 즉 다음 메인 작업은 더 많은 ladder 추가가 아니라, 이 wrapper 결과로 explicit policy review를 진행할지 판단하는 쪽입니다.
- 현재 explicit policy review 결론은 여기까지입니다. `bounded_promotion_review_result_status=PASS_RECENT_WINDOW_POLICY_CANDIDATE` 를 근거로 **bounded promotion review는 승인**하지만, **primary full latest batch baseline 승격 변경은 아직 보류**합니다.
- 사람 말로 풀면 이 뜻입니다. **recent-window는 실험 후보로는 충분히 괜찮지만, 운영 기본 기준을 바로 바꿀 만큼 결론이 끝난 것은 아직 아니다** 라고 읽는 편이 맞습니다.
- 이 상태에서 다음 정책 판단은 “바로 recent-window를 primary gate로 바꿀지”가 아니라, [recommendation-review-gate-policy-promotion-checklist.md](./recommendation/recommendation-review-gate-policy-promotion-checklist.md) 기준으로 explicit policy review를 먼저 거치는 쪽이 맞습니다.
...
- 같은 exact profile differential audit도 별도 wrapper로 고정했습니다. `run-local-recommendation-same-profile-origin-differential-audit.sh` 기준 exact profile latest batch user 수는 `EXAMPLE_SMOKE=454`, `REAL_USER=14`, `LOCAL_REAL_NON_EXAMPLE_SEED=3`, `BOUNDED_LOCAL=1` 이고, example 쪽 `2622` 는 `top1=272`, `top3=434`, `top10=442` 인 반면 real-user 쪽은 `top1/top3/top5/top10/any-rank=0` 입니다. 그리고 대표 example/real-user를 직접 비교하는 `run-local-recommendation-same-profile-path-differential-audit.sh` 기준 target `2622` 는 example 쪽 `drop_stage=PRESENT_IN_SAVED_BATCH`, `latest_saved_rank=1` 인 반면 real-user 쪽은 `drop_stage=NOT_IN_SQL_RETRIEVAL` 입니다. 더 나아가 `run-local-recommendation-same-profile-fresh-saved-differential-audit.sh` 로 example 대표 유저에 fresh personal refresh를 다시 태우면 그 stale saved path도 즉시 `NOT_IN_SQL_RETRIEVAL` 로 사라집니다. 그리고 `run-local-recommendation-review-gate-staleness-audit.sh` 기준 mixed leader `2622` top1 example users `272명` 은 최근 24시간에 `0명`, oldest/newest `2026-05-13 13:39:31 / 2026-05-17 11:49:50` 인 반면, real-user `80명` 은 최근 24시간에 전부 refresh돼 있습니다. 반대로 `run-local-recommendation-review-gate-recent-window-audit.sh` 기준 recent 24h latest batch만 보면 mixed leader는 `3284`, share `7.23%`, origin mix `EXAMPLE_SMOKE:1,REAL_USER:5` 이고 `2622 top1=0` 입니다. 즉 현재 남은 질문은 “주거형 real-user가 더 필요하냐”가 아니라 **historical example latest batch가 review gate 해석을 얼마나 왜곡하고 있고, recent-window current signal을 운영에서 어떤 보조 gate로 읽을지** 입니다.
...
- 현재 draft PR을 왜 유지하는지와 draft/reviewer-ready/merge-ready 경계는 [recommendation-pr-draft-exit-checklist.md](recommendation/recommendation-pr-draft-exit-checklist.md) 에 따로 정리돼 있습니다. 현재 기본 해석은 “코드 미완성”보다 **full latest batch historical inertia와 recent-window current signal을 분리해서 읽는 운영 해석 경계** 에 가깝으므로, reviewer-ready와 merge-ready를 분리해서 읽는 편이 맞습니다.
