...
5. 추천은 아직 reopen 대상이 아닙니다. `reopen_precheck_status=KEEP_OBSERVING`, real-user sample thin, `EXAMPLE_SMOKE_ONLY_LEADER` 기준이 바뀌는지 `run-local-recommendation-observation-suite.sh` 로 관찰합니다. 다음 코드 PR 후보는 score/weight/prompt 재개방이 아니라 표준코드 미입력 backlog를 줄이는 UX/입력 유도 쪽입니다.
