...
- 같은 `2026-05-20` 기준으로 이 해석은 이제 wrapper/runbook 설명에만 머물지 않고 admin API surface에도 올라왔습니다. `GET /api/admin/dashboard/summary`, `GET /api/admin/dashboard/recommendation-breakdowns` 는 이제 full latest batch gate(`recommendationReviewGate`) 외에 `recentWindowLatestBatch`, `recentWindowRecommendationReviewReading`, `historicalExampleDominanceDetected` 를 같이 내려, historical example inertia와 recent current-live signal을 한 응답에서 함께 읽게 합니다. 최신 local smoke 기준 값은 `recommendation_review_gate=DEFERRED_NON_REAL_LEADER_SIGNAL`, `recommendation_recent_window_review_reading=RECENT_WINDOW_CLEARS_HISTORICAL_2622_DOMINANCE`, `recommendation_historical_example_dominance_detected=true` 입니다.
