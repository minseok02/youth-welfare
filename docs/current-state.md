...
2. `run-local-ops-observation-suite.sh` 를 먼저 돌려 attention warning이 다시 생겼는지 확인합니다. 현재 기준선은 `BASELINE_HEALTHY`, collect 실패/partial/circuit `0`, attention warning `0` 입니다. 표준코드는 bounded 보정 후보와 conflict gap이 모두 `0` 이라 관찰 backlog로 둡니다.
3. attention에 남는 `standard-code-backlog`, `notification-backlog` 는 현재 관찰 항목입니다. 표준코드는 `safe_reconcile_candidate_rows` 또는 `conflicting_value_gap_rows` 가 생길 때만 직접 보정하고, 알림은 `stale_14d_total > 0` 또는 failed queue가 생길 때만 bounded hide/retry를 검토합니다. PR #350 배포 후 첫 예약 digest에서는 기존 `/policies/3324` unread tail 24건이 중복 증가하지 않는지만 먼저 확인합니다.
