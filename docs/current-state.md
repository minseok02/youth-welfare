...
2. `run-local-ops-observation-suite.sh` 를 먼저 돌려 attention warning이 다시 생겼는지 확인합니다. 최신 기준선은 `tmp/ops-observation/20260627T165813Z` 의 `BASELINE_HEALTHY`, collect 실패/partial/circuit `0`, attention item `standard-code-backlog,notification-backlog` 입니다. 표준코드는 bounded 보정 후보와 conflict gap이 모두 `0` 이라 관찰 backlog로 둡니다.
