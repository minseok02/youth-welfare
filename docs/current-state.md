...
- 2026-06-21 챗봇 품질 회귀셋 기준: [run-local-chat-followup-scenario-audit.sh](/home/ubuntu/youth-welfare/deploy/smoke/run-local-chat-followup-scenario-audit.sh:1) 를 4개에서 10개 시나리오로 확장하고 모든 마지막 턴이 `POLICY_GROUNDED` 여야 통과하도록 강화했다. 추가 축은 면접비, 자격증 응시료, 청년근로자 교통비, 청년창업센터 사업화자금, 서울 청년수당, 청년 동아리 활동비다. 최신 artifact `tmp/chat-followup-scenario-audit/20260621T124349Z` 는 `scenario_count=10`, `policy_grounded_last_turn_scenarios=10`, `clarification=0`, `branch_suggestion_last_turn=0`, `HOLD_LONG_TERM_MEMORY` 로 통과했다. 넓은 `서울 청년 면접비`, `서울 청년 교통비` 질의는 서울 exact 정책 부재로 `서울 청년수당` fallback clarification이 관측되어 별도 데이터/질의 품질 잔량으로 둔다.
- 2026-06-21 챗봇 운영 관측 기준: [run-local-chat-observability-audit.sh](/home/ubuntu/youth-welfare/deploy/smoke/run-local-chat-observability-audit.sh:1) 를 추가해 1/7/30일 `chat_messages` assistant reference/action link 비율과 `chat_retrieval_snapshots` branch suggestion, clarification, zero-result, avg result_count, fallback strategy를 집계한다. [run-local-ops-observation-suite.sh](/home/ubuntu/youth-welfare/deploy/smoke/run-local-ops-observation-suite.sh:1) 에도 `RUN_CHAT_OBSERVABILITY_AUDIT=true` 기본 옵션으로 연결했다. 최신 단독 artifact `tmp/chat-observability-audit/20260621T124749Z` 와 ops wrapper artifact `tmp/ops-observation/20260621T125104Z` 기준 7일 창은 assistant `7`, reference rate `71.43%`, clarification `0.00%`, zero-result `50.00%` 이며, zero-result는 최근 branch suggestion/smoke성 snapshot 영향이라 `CHAT_ATTENTION` 으로 관측만 올리고 제품 기능 실패로 보지는 않는다.
...
- 2026-06-20 프론트 deployed-origin 기준: `FRONTEND_PUBLIC_BASE_URL=https://youthmoa.kr` 관측 스위트에서 사용자 Playwright smoke 31개가 통과했고 `decision_class=BASELINE_HEALTHY` 다. 최신 artifact는 `tmp/frontend-observation/20260620T150550Z` 다.
