...
- 2026-07-17 nightly handoff 재확인 기준: `2026-07-17T01:10Z` 정기 cron은 backend drift fix 배포 전 코드라 `current-priority -> active baseline -> backend_tests` 에서 멈춰 summary가 start line만 남았다. drift closeout 배포 뒤 같은 wrapper를 수동 재실행했고 `/var/log/youth-welfare/nightly-ops-handoff/artifacts/20260717T085519Z` 기준 `ops=passed`, `policy=passed`, `policy_triage=passed`, `collect=passed`, `auth=passed`, `frontend=skipped`, `db_audit=ok`, auth smoke `smoke_trusted_origin=https://youthmoa.kr`, `OP_ALERT_STATUS=ok` 로 끝까지 append 됐다. 다음 자동 cron은 같은 결과가 정기 스케줄에서 반복되는지만 확인한다.
...
1. 다음 실제 nightly cron 뒤 `/var/log/youth-welfare/nightly-ops-handoff/nightly-summary-YYYY-MM-DD.log` 와 `artifacts/<UTC timestamp>/operational-db-audit.out` 를 확인합니다. 2026-07-17 수동 재실행은 끝까지 통과했으므로, 다음 정기 cron에서는 같은 값이 자동 스케줄에서도 반복되는지만 봅니다. 기대값은 `ops=passed`, `policy=passed`, `policy_triage=passed`, `collect=passed`, `auth=passed`, `db_audit=ok`, auth smoke의 `smoke_trusted_origin=https://youthmoa.kr`, DB alert evaluator의 `OP_ALERT_STATUS=ok` 입니다.
