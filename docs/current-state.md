...
- `2026-05-20` 기준 recommendation 의 `savedAi=0` 이슈는 여전히 AI exclusion evidence 정리 단계지만, local generic-domain signup으로 만든 분산 `REAL_USER` 샘플 30명으로 live readiness gate는 실제로 열렸습니다. 현재 `run-local-real-user-exclusion-readiness-check.sh` 기준 `dashboard_real_user_gate=READY_REAL_USER_TRAFFIC`, `breakdown_real_user_cohort_gate=READY_REAL_USER_COHORT`, `real_user_distribution_executed=true` 이고, `zero_ai_rows=22`, `zero_ai_users=7`, `zero_ai_reason_buckets=AUDIENCE_MISMATCH:3,INCOME_MISMATCH:5,OTHER:7,STUDENT_AUDIENCE_MISMATCH:7` 까지 다시 잡힙니다. latest batch review gate는 아직 `recommendation_review_gate=DEFERRED_NON_REAL_LEADER_SIGNAL` 로 남아 있지만, real-user 자체 concentration은 `top1_share_pct=10.00`, `concentration_readiness=NO_PRIORITY_DOMINANT` 로 더 분산됐습니다. artifact baseline 쪽 latest reading은 계속 `drift_class=VOLATILE_ONLY_DRIFT`, `recommended_reading=READ_LATEST_AS_VOLATILE_OBSERVATION` 이며, stable baseline은 여전히 old pointer 기준 `baseline_zero_ai_reason_buckets=AUDIENCE_MISMATCH:4,STUDENT_AUDIENCE_MISMATCH:4`, `dashboard_real_user_gate=DEFERRED_NO_REAL_USER_TRAFFIC`, `breakdown_real_user_cohort_gate=DEFERRED_NO_REAL_USER_COHORT` 로 읽습니다.
- 같은 `2026-05-20` 기준으로 mixed latest batch review gate blocker도 직접 좁혔습니다. `run-local-recommendation-review-gate-blocker-audit.sh` 결과 mixed latest batch는 `users=498`, `example_users=464`, `real_user_users=30`, `top1_leader=청년월세 지원사업`, `top1_leader_share_pct=55.62`, `top1_leader_real_user_users=0` 인 반면, real-user-only latest batch는 `top1_leader=드림나래(인천청년 면접복장 지원)`, `top1_leader_share_pct=10.00`, `concentration_readiness=NO_PRIORITY_DOMINANT` 입니다. 즉 현재 blocker는 recommendation rank bug보다 **mixed batch non-real dominance** 로 읽는 편이 맞습니다.
- 더 좁히면 현재 mixed leader 서비스 `2622(청년월세 지원사업)` 는 real-user 쪽에서 `top1/top3/top5/top10` 에 한 번도 안 나타납니다. 즉 `real_user_mixed_leader_top10_count=0` 이고, 현재 30명 분산 샘플만으로는 mixed leader transition path 자체가 안 보입니다. 지금 해석은 단순 “더 기다림”보다 **housing-like targeted real-user signal이 없으면 review gate 전이가 늦어질 수 있음** 쪽이 더 정확합니다.
