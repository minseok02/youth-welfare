...
외부 API 수집 DB 이력은 [collect-external-api-smoke-runbook.md](./collect-external-api-smoke-runbook.md) 와 `bash deploy/smoke/run-local-collect-external-api-smoke.sh` 로 먼저 봅니다. 이 smoke는 외부 API를 재호출하지 않고 `api_sync_logs`, `raw_api_payloads`, `collect_runtime_statuses`, `collect_execution_locks` 를 읽어 `NO_COLLECT_HISTORY` 와 실제 실패/open circuit/active lock을 분리합니다.
