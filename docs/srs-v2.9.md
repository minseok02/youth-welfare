...
| 작성일 | 2026-04-03 |
| 프로젝트 유형 | 졸업 프로젝트 (2인) |
| 변경 이력 | v2.8→v2.9: **확장형 MVP 구조** → 1차(11개) / 2차(9개) 테이블 분리. **Cold Start 전략** → `score_weights` 테이블 추가, 추천 이력 기반 rule/ai 가중치 자동 전환. **AI 점수 구조 수정** → `welfare_services.ai_score` 제거(유저별 점수를 서비스 공통값으로 두는 설계 오류 수정), AI 점수는 `user_recommendations`에만 존재. **스키마 무결성** → `service_tags` UNIQUE KEY 추가(중복 태그 → 점수 이중합산 버그 방지), `user_attributes.attr_type` ENUM→VARCHAR(30). **컬럼 수정** → `batch_date DATE`→`recommended_at DATETIME`, `reason`→`ai_reason`, `rule_weight_used`·`ai_weight_used` 추가. **unified_category** → 3개 API 분류 통합 필터용 컬럼 추가. **FR 수정** → FR-05-02 분야 필터를 unified_category 기반으로, FR-07-09 점수 가중치를 score_weights 기반으로, FR-07-14 CLOSED 처리를 user_recommendations 기준으로 수정 |
...
| Re-ranking | 개인적 우선순위 가중치로 후보를 재정렬하는 2단계 |
...
| 알림 (메인) | 카카오 알림톡 (CoolSMS) | |
| 알림 (폴백) | Spring Mail + Gmail SMTP | |
...
| FR-03-02 | 즉시 비식별화 (이메일→'withdrawn', 주소·소득·취업상태 등 모두 NULL) | 필수 |
...
| ID | 요구사항 | 우선순위 |
|----|---------|:--------:|
| FR-06-01 | 대상의 상세·선정기준·지원내용·신청방법 표시 (`welfare_service_details`) | 필수 |
| FR-06-02 | `welfare_service_details` JOIN 조회 (실시간 API 호출 없음) | 필수 |
| FR-06-03 | 30분 이내 재조회 시 조회수 미증가 (애플리케이션 레이어 처리. 2차에서 service_view_logs로 정교화) | 필수 |
...
```
[새벽 1시] 군집(2D) + JSONL + Batch 제출
[폴링]     completed → Re-ranking / failed → fallback
[새벽 6시] 하드 데드라인 Fallback (rule만으로 Re-ranking)
[오전 8시] 알림 슬롯 배치 발송
```
...
| FR-07-06 | **신규 정책 가점 포함**: 수집 후 24시간 이내 신규 정책 중 `rule_base_score` 최솟값 M=5건을 후보 필수 가점 추가. 유저 조정 없이 슬롯 배치에서만 노출 보장 | 필수 | 1차 |
...
| FR-07-13 | 2차: Partial 완료 처리 → expired 후 부분 결과 먼저 저장 후 완료 군집 즉시 Re-ranking | 필수 | 2차 |
...
| FR-07-17 | 북마크 가능. 사용자당 최대 200건 유지, 30일+미북마크 삭제 | 필수 | 1차 |
...
| FR-09-02 | 카카오 알림톡(CoolSMS) 고정 템플릿 발송. 정책명 + ai_reason 삽입 | 필수 | 1차 |
| FR-09-03 | 알림톡 실패 시 Spring Mail + Gmail SMTP 폴백 | 필수 | 1차 |
...
| FR-09-09 | 발송 실패 시 30분/2시간 후 최대 2회 재시도 | 필수 | 1차 |
| FR-09-10 | 수신 동의 일시 기록, 2년 경과 시 자동 제외 | 필수 | 1차 |
| FR-09-11 | 수신 거부 링크 포함, 거부 즉시 반영 | 필수 | 1차 |
...
| FR-11-03 | 추천 목록, 북마크, 알림 이력 조회 | 필수 |
...
| NFR-02-02 | 이메일 확인 + UNIQUE + HTTPS |
| NFR-02-03 | 전화번호 AES-256 암호화 |
| NFR-02-04 | HTTPS 전 구간 |
...
| NFR-02-12 | AI 프롬프트: 군집 범주값만 전송 (개인 식별 제외) |
...
| NFR-03-03 | 신규 정책은 유저 조정 없이 슬롯 배치로 노출 보장 |
...
| 카카오 알림톡 (CoolSMS) | 추천 알림 | ~8원/건 |
| Kakao 주소 검색 | 구조화 주소 | 무료 |
| Gmail SMTP | 알림 폴백 | 무료, 500건/일 |
...
| C-08 | 카카오 알림톡 템플릿 사전 2~5영업일 |
| C-09 | OpenAI Batch API 완료 최대 24시간 |
...
| 결정 | 선택 | 이유 |
|------|------|------|
| 확장형 MVP | 1차/2차 구조 분리 | 처음부터 서비스 경계 확정 → 재작성 없이 내부만 교체. 발표 시 "확장성 고려한 설계"로 설명 가능 |
| Cold Start | `score_weights` 테이블 (rule 0.8→0.4) | 초기 AI 데이터 부족 시 rule 우선. 이력 추적 후 자동 전환. 가중치 변화가 recommendation_logs에 기록되어 분석 가능 |
| ai_score 위치 | `user_recommendations`에만 | AI 점수는 유저×서비스 단위. `welfare_services`에 두면 모든 유저가 동일 점수 공유 → 개인화 불가 |
| user_attributes.attr_type | VARCHAR(30) | ENUM은 신규 속성(LIFE_STAGE, 학력 등) 추가 시 ALTER TABLE 필요 → 경직성 문제 |
| service_tags UNIQUE KEY | `uq_st (service_id, tag_type, tag_value)` | 중복 태그 삽입 → rule_base_score 이중 합산 버그 방지. RDBMS 무결성 제약 활용 |
| batch_date → recommended_at | DATETIME | 실시간 방식에서 DATE만으로 같은 날 중복 구분 불가. DATETIME이 배치 시간도 포괄하여 2차 호환 |
| ai_reason | `user_recommendations`에 저장 | 유저별 AI 추천 이유. 추천 UI 및 알림톡 템플릿에 직접 표시 |
| unified_category | `welfare_services` 별도 컬럼 | 3개 API 분류 체계 불일치. 필터 UI 통합용 수집 시 매핑. 원본 category_main/service_tags 보존 |
...
| FastAPI | 제거 (Spring 단일화) | 딥러닝 서빙 없음. 2인 졸업에서 불필요 |
| 챗봇 | 제거 | 13주 내 품질 확보 불가 |
