...
이 next question을 직접 보는 wrapper도 고정했습니다. `run-local-recommendation-same-profile-origin-differential-audit.sh` 기준 exact profile latest batch user 수는 `EXAMPLE_SMOKE=454`, `REAL_USER=14`, `LOCAL_REAL_NON_EXAMPLE_SEED=3`, `BOUNDED_LOCAL=1` 이고, target service `2622` 는 example 쪽 `top1=272`, `top3=434`, `top10=442` 인 반면 real-user 쪽은 `top1/top3/top5/top10/any-rank=0` 입니다. 즉 current blocker는 단순히 “mixed batch에서 example이 많다”가 아니라 **same-profile인데도 example/local-seed/bounded-local 에서는 `2622` path가 강하게 열리고 generic-domain real-user 에서는 완전히 닫혀 있는 상태** 입니다. 이 상태의 다음 기술적 단계는 표본 수 확대보다 `account_origin`, signup flow, 최신 batch composition 차이가 retrieval/scoring/rerank path를 어떻게 바꾸는지 추적하는 쪽입니다.
