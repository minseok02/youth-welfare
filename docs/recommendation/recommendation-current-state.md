...
다만 `2026-05-20` 에 local generic-domain signup 계정 13개(`account_origin=REAL_USER`)로 추천 refresh + click 표본을 실제로 쌓은 뒤, live readiness gate는 이미 열렸습니다. `run-local-real-user-exclusion-readiness-check.sh` 결과는 `dashboard_real_user_gate=READY_REAL_USER_TRAFFIC`, `breakdown_real_user_cohort_gate=READY_REAL_USER_COHORT`, `real_user_distribution_executed=true` 였고, 분산 표본에서는 `zero_ai_rows=14`, `zero_ai_users=4`, `zero_ai_reason_buckets=AUDIENCE_MISMATCH:1,INCOME_MISMATCH:5,OTHER:5,STUDENT_AUDIENCE_MISMATCH:3` 도 다시 잡혔습니다. 대신 admin recommendation 쪽 review gate는 아직 `recommendation_review_gate=DEFERRED_NON_REAL_LEADER_SIGNAL` 로 남아 있고, mixed latest batch 기준 `recommendation_top1_leader_real_user_users=0`, `recommendation_top1_leader_signal_summary=LOCAL_SEED_WITHOUT_REAL_USER_LEADER` 가 같이 확인됐습니다.
...
같은 날 10개 분산 프로필(`birthDate/sido/sgg/incomeLevel/employmentStatus/householdType` 전부 다르게 준 `REAL_USER`)을 추가로 만들어 보니 recommendation 상단도 실제로 분산됩니다. 계정별 latest top1은 `3790(대전 청년 월세지원)`, `3730(대학생 학자금대출 이자지원 사업)`, `2884(4차산업칼리지 청년인턴 지원사업)`, `2852(광주청년구직활동지원사업)`, `3254(인천 중구 청년 자격시험 응시료 지원사업)`, `3218(청년사회진입 활동비 지원)`, `2766(청년 어학·자격시험 응시료 지원사업)`, `3908(대구형 청년수당)`, `2960(구직활동비 지원사업)`, `16490(국가장학금 Ⅰ유형)` 처럼 서로 갈렸고, real-user-only concentration도 `top1_share_pct=23.08`, `concentration_readiness=NO_PRIORITY_DOMINANT` 로 더 이상 한 정책에 과집중되지 않았습니다. 즉 homogeneous 3계정에서 보이던 `zero_ai_rows=0` 는 분산 표본이 좁아서였고, 분산 `REAL_USER` 샘플을 늘리면 exclusion bucket이 다시 드러난다고 읽는 편이 맞습니다.
