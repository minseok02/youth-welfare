...
같은 latest export JSON은 이제 `operator_next_step` 도 같이 남깁니다. 다만 이 값은 older baseline artifact를 그대로 요약한 포인터이므로, 현재 local truth를 단독으로 대표하지는 않습니다. 지금은 live readiness와 review gate audit까지 같이 읽어, **full latest batch historical baseline + recent-window current signal** 을 함께 해석하는 편이 맞습니다.
...
이 해석은 이제 wrapper/runbook 메모에만 남아 있지 않습니다. `GET /api/admin/dashboard/summary`, `GET /api/admin/dashboard/recommendation-breakdowns` 도 full latest batch gate(`recommendationReviewGate`) 외에 `recentWindowLatestBatch`, `recentWindowRecommendationReviewReading`, `historicalExampleDominanceDetected` 를 함께 내려, historical example inertia와 recent current-live signal을 같은 admin surface에서 같이 읽게 했습니다. 최신 local smoke 기준으로는 `recommendation_review_gate=DEFERRED_NON_REAL_LEADER_SIGNAL`, `recent_window_recommendation_review_reading=RECENT_WINDOW_CLEARS_HISTORICAL_2622_DOMINANCE`, `historical_example_dominance_detected=true` 입니다.

그래서 현재 `latest-overview` 는 두 층을 같이 보여 줍니다. baseline artifact 기반 `operator_next_step` 은 historical snapshot 포인터로 남지만, readiness 포함 실행에서는 `effective_operator_next_step=WAIT_FOR_REAL_USER_LEADER_SIGNAL`, `readiness_override_detected=true`, `readiness_override_reason=LIVE_REAL_USER_GATES_READY_BUT_REVIEW_GATE_PENDING` 를 같이 보여 줍니다. 즉 지금 남은 blocker는 더 이상 “real-user traffic/cohort 없음”이 아니라 **real-user leader/review signal이 아직 mixed latest batch top1에 안 올라온 상태**로 읽는 편이 맞습니다.
