...
`2026-06-06` 추천 메모 코드 품질 점검에서는 `aiReason` 이 AI 응답에서 캐시, 저장 row, API 응답, `MainPage` 표시까지 이어지는 것을 확인했습니다. 결함 가능성이 큰 지점은 OpenAI prompt가 `reason 20자 이내` 를 요구해도 서버가 응답을 신뢰하고 그대로 저장/출력하던 경계였습니다. 현재는 [recommendation-ai-reason-memo-contract.md](./recommendation-ai-reason-memo-contract.md) 기준으로 `RecommendationAiReasonSanitizer` 를 도입해 blank reason 은 null 로, 개행/탭/중복 공백은 한 줄로, 장문 reason 은 20 code point 이내로 정규화합니다. 이 정규화는 `RealtimeAiGateway`, `AiScoringService`, `JpaClusterAiScoreCache`, `RecommendationPersistenceService`, `RecommendationResponse` 에 모두 걸려 있어 신규 AI 응답뿐 아니라 기존 DB row 응답도 같은 품질 기준을 탑니다.
