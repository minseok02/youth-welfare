...
- `review_gate_policy_promotion_readiness_status`
  - explicit policy review를 아직 거치지 않았더라도 bounded promotion review의 prerequisite 자체는 이미 충족됐는지 보여 주는 값입니다. 현재 local 기준으로는 `READY_FOR_BOUNDED_PROMOTION_REVIEW` 입니다.
- `review_gate_policy_promotion_readiness_reason`
  - 위 readiness 상태를 만든 직접 이유입니다. 현재 local 기준으로는 `EXPLICIT_POLICY_REVIEW_PENDING_WITH_BOUNDED_REVIEW_PREREQUISITES_MET` 입니다.
- `review_gate_policy_promotion_execution_status`
  - readiness 다음 실제 실행 상태입니다. 현재 local 기준으로는 `AWAIT_EXPLICIT_POLICY_REVIEW_DECISION` 입니다.
- `review_gate_policy_promotion_execution_reason`
  - 위 execution status를 만든 직접 이유입니다. 현재 local 기준으로는 `READINESS_MET_BUT_EXPLICIT_POLICY_REVIEW_DECISION_IS_STILL_PENDING` 입니다.
- `review_gate_policy_promotion_approval_status`
  - execution readiness와 별도로, bounded promotion review 승인 자체가 기록됐는지 보여 주는 값입니다. 현재 local 기준으로는 `PENDING_EXPLICIT_PROMOTION_APPROVAL` 입니다.
- `review_gate_policy_promotion_approval_reason`
  - 위 approval status를 만든 직접 이유입니다. 현재 local 기준으로는 `EXECUTION_READY_BUT_EXPLICIT_PROMOTION_APPROVAL_NOT_RECORDED` 입니다.
- `review_gate_policy_promotion_approval_criteria_status`
...
- `review_gate_policy_promotion_review_run_status`
  - explicit approval record 다음 실제 bounded promotion review run이 이미 실행됐는지, 아니면 아직 pending인지 보여 주는 값입니다. 현재 local 기준으로는 `PENDING_BOUNDED_PROMOTION_REVIEW_RUN` 입니다.
- `review_gate_policy_promotion_review_run_reason`
  - 위 review run status를 만든 직접 이유입니다. 현재 local 기준으로는 `EXPLICIT_APPROVAL_RECORD_NOT_WRITTEN_FOR_BOUNDED_REVIEW_RUN` 입니다.
- `review_gate_interpretation_class`
