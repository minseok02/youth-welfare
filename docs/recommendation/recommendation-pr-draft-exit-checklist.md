...
9. review gate candidate / promotion / readiness / approval criteria / approval / approval decision 판단(`RECENT_WINDOW_POLICY_CANDIDATE` / `REQUIRES_EXPLICIT_POLICY_CHANGE_REVIEW` / `READY_FOR_BOUNDED_PROMOTION_REVIEW` / `READY_FOR_EXPLICIT_PROMOTION_APPROVAL` / `PENDING_EXPLICIT_PROMOTION_APPROVAL` / `AWAIT_EXPLICIT_PROMOTION_APPROVAL_DECISION`)도 reviewer 문서/PR surface에 같이 고정돼 있음
