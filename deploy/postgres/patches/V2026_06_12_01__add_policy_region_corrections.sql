...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'app_core_rw') THEN
        GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.policy_region_corrections TO app_core_rw;
        GRANT USAGE, SELECT ON SEQUENCE public.policy_region_corrections_id_seq TO app_core_rw;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'admin_dashboard_ro') THEN
        GRANT SELECT ON TABLE public.policy_region_corrections TO admin_dashboard_ro;
        GRANT SELECT ON SEQUENCE public.policy_region_corrections_id_seq TO admin_dashboard_ro;
    END IF;
END $$;
