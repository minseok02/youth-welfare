...
MATRIX_ROOT="${MATRIX_ROOT:-${ROOT_DIR}/tmp/housing-standard-code-matrix-audit}"
KEEP_ARTIFACTS="${KEEP_ARTIFACTS:-false}"
RUN_TS_UTC="$(smoke_now_ts_utc)"
ARTIFACT_DIR="${ARTIFACT_DIR:-${MATRIX_ROOT}/${RUN_TS_UTC}}"
SUMMARY_OUT="${ARTIFACT_DIR}/housing-standard-code-matrix-summary.txt"
JSON_OUT="${ARTIFACT_DIR}/housing-standard-code-matrix-summary.json"
LATEST_ARTIFACT_LINK="${MATRIX_ROOT}/latest"
LATEST_SUMMARY_LINK="${MATRIX_ROOT}/latest-housing-standard-code-matrix-summary.txt"
...
run_scenario() {
  ...
  smoke_print_step "housing standard code matrix ${label}"
...
run_scenario "house_tenure_only" env \
  SMOKE_HOUSEHOLD_TYPE="1인 가구" \
  SMOKE_HOUSE_TENURE_CODE="3" \
  SMOKE_HOUSING_TYPE_CODE=""

run_scenario "housing_type_only" env \
  SMOKE_HOUSEHOLD_TYPE="1인 가구" \
  SMOKE_HOUSE_TENURE_CODE="" \
  SMOKE_HOUSING_TYPE_CODE="4"

run_scenario "tenure_and_type_combo" env \
  SMOKE_HOUSEHOLD_TYPE="1인 가구" \
  SMOKE_HOUSE_TENURE_CODE="3" \
  SMOKE_HOUSING_TYPE_CODE="4"

run_scenario "family_combo" env \
  SMOKE_HOUSEHOLD_TYPE="다자녀" \
  SMOKE_HOUSE_TENURE_CODE="3" \
  SMOKE_HOUSING_TYPE_CODE="4"
...
summary_lines = [
    "housing_standard_code_matrix_audit=passed",
