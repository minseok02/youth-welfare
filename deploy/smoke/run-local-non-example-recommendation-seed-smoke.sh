...
SMOKE_EMAIL_PREFIX="${SMOKE_EMAIL_PREFIX:-non.example.seed}"
SMOKE_EMAIL_DOMAIN="${SMOKE_EMAIL_DOMAIN:-smoke.local}"
SUMMARY_WINDOW_DAYS="${SUMMARY_WINDOW_DAYS:-14}"

ARTIFACT_DIR="${ARTIFACT_DIR:-$(mktemp -d)}"
CLICK_OUTPUT="${ARTIFACT_DIR}/recommendation-click.out"
CTR_OUTPUT="${ARTIFACT_DIR}/ctr-bounded-local.out"
CONCENTRATION_OUTPUT="${ARTIFACT_DIR}/concentration-bounded-local.out"
...
smoke_print_step "seed non-example recommendation click (${SMOKE_EMAIL_DOMAIN})"
...
if [[ -z "${SEED_EMAIL}" || -z "${SEED_LOG_ID}" ]]; then
  echo "failed to extract non-example seed outputs" >&2
  cat "${CLICK_OUTPUT}" >&2
  exit 1
fi

if [[ "${SEED_EMAIL}" == *@example.com ]]; then
  echo "seed email unexpectedly classified as example: ${SEED_EMAIL}" >&2
  exit 1
fi
...
smoke_print_step "verify bounded-local cohort counts in ${SUMMARY_WINDOW_DAYS}d window"
smoke_db_query \
  "SELECT COUNT(*),
...
WINDOW_BOUNDED_LOCAL_USERS="$(awk -F $'\t' 'NR==1 {print $2}' "${WINDOW_ROW_OUTPUT}")"
WINDOW_BOUNDED_LOCAL_CLICKED_USERS="$(awk -F $'\t' 'NR==1 {print $3}' "${WINDOW_ROW_OUTPUT}")"

if [[ "${WINDOW_BOUNDED_LOCAL_LOGS}" == "0" || "${WINDOW_BOUNDED_LOCAL_USERS}" == "0" ]]; then
  echo "bounded-local cohort is still empty after seed" >&2
  cat "${WINDOW_ROW_OUTPUT}" >&2
  exit 1
fi

smoke_print_step "rerun ctr audit for bounded-local cohort"
USER_COHORT=bounded_local bash "${ROOT_DIR}/deploy/smoke/run-local-ctr-readiness-audit.sh" | tee "${CTR_OUTPUT}"
...
smoke_print_step "rerun concentration audit for bounded-local cohort"
USER_COHORT=bounded_local bash "${ROOT_DIR}/deploy/smoke/run-local-recommendation-concentration-audit.sh" | tee "${CONCENTRATION_OUTPUT}"
...
CONCENTRATION_BOUNDED_LOCAL_USERS="$(extract_key_value "${CONCENTRATION_OUTPUT}" "latest_batch_bounded_local_users")"
...
echo "window_bounded_local_logs=${WINDOW_BOUNDED_LOCAL_LOGS}"
echo "window_bounded_local_users=${WINDOW_BOUNDED_LOCAL_USERS}"
echo "window_bounded_local_clicked_users=${WINDOW_BOUNDED_LOCAL_CLICKED_USERS}"
...
echo "latest_batch_bounded_local_users=${CONCENTRATION_BOUNDED_LOCAL_USERS}"
