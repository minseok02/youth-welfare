...
python3 - "${REFRESH_RESPONSE}" "${DIAGNOSTICS_RESPONSE}" "${TARGET_SERVICE_IDS_CSV}" <<'PY'
import json
import sys
...
top_rows = [services[int(row["serviceId"])] for row in refresh_rows if row.get("serviceId") is not None and int(row["serviceId"]) in services]

zero_rows = [row for row in top_rows if row.get("latestSavedAiStatus") == "SCORED" and (row.get("latestSavedAiScore") or 0.0) == 0.0]
positive_rows = [row for row in top_rows if row.get("latestSavedAiStatus") == "SCORED" and (row.get("latestSavedAiScore") or 0.0) > 0.0]
...
print(f"METRIC ai_zero_categories={bucket(row.get('category') for row in zero_rows)}")
print(f"METRIC ai_positive_sources={bucket(row.get('sourceType') for row in positive_rows)}")
print(f"METRIC ai_positive_categories={bucket(row.get('category') for row in positive_rows)}")

print()
print("[ZERO VS POSITIVE CATEGORY COUNTS]")
all_keys = sorted(set((row.get("sourceType"), row.get("category")) for row in zero_rows + positive_rows))
for key in all_keys:
    source, category = key
    zero_count = sum(1 for row in zero_rows if row.get("sourceType") == source and row.get("category") == category)
    pos_count = sum(1 for row in positive_rows if row.get("sourceType") == source and row.get("category") == category)
    print(f"{source}\t{category}\tzero={zero_count}\tpositive={pos_count}")
...
for row in zero_rows:
    ...
    base = (
        f"zero={row.get('serviceId')}:{row.get('title')}:{row.get('sourceType')}:{row.get('category')}:"
        f"savedRank={row.get('latestSavedRank')}:rerankRank={row.get('rerankCurrentRank')}:"
        f"savedAi={row.get('latestSavedAiScore')}:savedFinal={row.get('latestSavedFinalScore')}"
    )
    if comp is None:
        print(base + "\tcomp=NONE")
    else:
        print(
            base +
            "\tcomp=" +
            f"{comp.get('serviceId')}:{comp.get('title')}:{comp.get('sourceType')}:{comp.get('category')}:"
            f"savedRank={comp.get('latestSavedRank')}:rerankRank={comp.get('rerankCurrentRank')}:"
            f"savedAi={comp.get('latestSavedAiScore')}:savedFinal={comp.get('latestSavedFinalScore')}"
        )

print()
print("[TARGET ZERO MEMBERSHIP]")
for service_id in sorted(target_ids):
    row = services.get(service_id)
    if row is None:
        print(f"{service_id}\tmissing=true")
        continue
    is_zero = row.get("latestSavedAiStatus") == "SCORED" and (row.get("latestSavedAiScore") or 0.0) == 0.0
    print(
        f"{service_id}\t"
        f"inAiZero={is_zero}\t"
