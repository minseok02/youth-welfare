...
top_rows = [services[sid] for sid in refresh_ids if sid in services]
zero_rows = [
...
print(f"METRIC ai_zero_cohort_count={len(zero_rows)}")
...
print()
print("[AI ZERO COHORT]")
for row in zero_rows:
    print(
        f"{row.get('serviceId')}\t"
        f"target={int(row.get('serviceId')) in target_ids}\t"
        ...
        f"youthIncome={row.get('youthIncomeConditionTypeLabel')}\t"
        f"employment={','.join(row.get('youthEmploymentRequirementLabels') or [])}\t"
        f"education={','.join(row.get('youthEducationRequirementLabels') or [])}\t"
        f"special={','.join(row.get('youthSpecialRequirementLabels') or [])}\t"
        f"marital={row.get('youthMaritalStatusLabel')}\t"
