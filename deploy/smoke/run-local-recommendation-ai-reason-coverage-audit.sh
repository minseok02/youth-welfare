...
scored_rows = [row for row in top_rows if row.get("latestSavedAiStatus") == "SCORED"]
blank_rows = [row for row in scored_rows if not (row.get("latestSavedAiReason") or "").strip()]
non_blank_rows = [row for row in scored_rows if (row.get("latestSavedAiReason") or "").strip()]

blank_source = Counter((row.get("sourceType") or "UNKNOWN") for row in blank_rows)
blank_category = Counter((row.get("category") or "UNKNOWN") for row in blank_rows)
...
print(f"METRIC ai_scored_count={len(scored_rows)}")
print(f"METRIC ai_scored_blank_reason_count={len(blank_rows)}")
print(f"METRIC ai_scored_non_blank_reason_count={len(non_blank_rows)}")
print(f"METRIC blank_reason_source_distribution={encode_counter(blank_source)}")
print(f"METRIC blank_reason_category_distribution={encode_counter(blank_category)}")

print()
print("[BLANK REASON ROWS]")
