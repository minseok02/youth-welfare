...
build_block() {
  cat <<EOF
${BLOCK_START}
${NIGHTLY_SCHEDULE} ENV_FILE=${ENV_FILE} SMOKE_DB_MODE=${SMOKE_DB_MODE} APP_BASE_URL='${APP_BASE_URL}' FRONTEND_E2E_MODE=${FRONTEND_E2E_MODE} FRONTEND_PUBLIC_BASE_URL='${FRONTEND_PUBLIC_BASE_URL}' ALLOW_ADMIN_JWT_MINT=${ALLOW_ADMIN_JWT_MINT} bash ${APP_ROOT}/deploy/smoke/run-nightly-ops-handoff.sh >> ${NIGHTLY_OPS_HANDOFF_LOG_ROOT}/nightly-cron.log 2>&1
EOF

  if [[ "${INSTALL_FRONTEND_WEEKLY}" == "true" ]]; then
    cat <<EOF
${FRONTEND_WEEKLY_SCHEDULE} RUN_FRONTEND_OBSERVATION=true ENV_FILE=${ENV_FILE} SMOKE_DB_MODE=${SMOKE_DB_MODE} APP_BASE_URL='${APP_BASE_URL}' FRONTEND_E2E_MODE=${FRONTEND_E2E_MODE} FRONTEND_PUBLIC_BASE_URL='${FRONTEND_PUBLIC_BASE_URL}' ALLOW_ADMIN_JWT_MINT=${ALLOW_ADMIN_JWT_MINT} bash ${APP_ROOT}/deploy/smoke/run-nightly-ops-handoff.sh >> ${NIGHTLY_OPS_HANDOFF_LOG_ROOT}/frontend-weekly-cron.log 2>&1
EOF
  fi
