...
gov24_zero_ai as (
  select * from top10 where source_type = 'GOV24' and ai_score = 0
)
select 'gov24_zero_ai_top10_count', count(*)::text
from gov24_zero_ai
union all
select 'gov24_zero_ai_top2_count', count(*)::text
from gov24_zero_ai
where rec_rank <= 2
union all
select 'gov24_zero_ai_rank_distribution',
...
union all
select 'gov24_zero_ai_reason_samples',
...
union all
select 'top2_gov24_zero_ai_score_summary',
       coalesce(string_agg(
         service_id || ':' || title || ':' ||
