...
top_rows = [services[int(row["serviceId"])] for row in refresh_rows[:len(refresh_ids)] if row.get("serviceId") is not None and int(row["serviceId"]) in services]

fresh_ai_requested = sum(1 for row in top_rows if row.get("latestSavedAiStatus") == "SCORED")
fresh_ai_zero = sum(1 for row in top_rows if row.get("latestSavedAiStatus") == "SCORED" and (row.get("latestSavedAiScore") or 0.0) == 0.0)
...
print(f"METRIC fresh_top_ai_requested_count={fresh_ai_requested}")
print(f"METRIC fresh_top_ai_zero_count={fresh_ai_zero}")

print()
print("[FRESH TOP AI]")
for idx, row in enumerate(top_rows, start=1):
    current_final = row.get("rerankCurrentFinalScore")
    saved_final = row.get("latestSavedFinalScore")
    delta = None
    if current_final is not None and saved_final is not None:
        delta = saved_final - current_final
    print(
        f"{idx}\t{row.get('serviceId')}\t"
        f"savedRank={row.get('latestSavedRank')}\t"
        f"rerankRank={row.get('rerankCurrentRank')}\t"
        f"ruleWeighted={row.get('ruleWeightedScore')}\t"
        f"currentFinal={current_final}\t"
        f"savedFinal={saved_final}\t"
        f"delta={delta}\t"
