...
CREATE INDEX IF NOT EXISTS idx_cls_source_collected ON collect_list_snapshots (source_type, collected_at DESC);
...
CREATE TABLE IF NOT EXISTS collect_list_snapshot_items (
    ...
    CONSTRAINT fk_clsi_snapshot FOREIGN KEY (snapshot_id) REFERENCES collect_list_snapshots(id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_clsi_source_id ON collect_list_snapshot_items (source_id);
...
CREATE INDEX IF NOT EXISTS idx_support_inquiries_status_created_at ON support_inquiries (status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_support_inquiries_user_key ON support_inquiries (user_key);
